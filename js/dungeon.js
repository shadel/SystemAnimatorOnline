...
MMD_SA_options.Dungeon = (function () {
  ...
  function CombatStats(stats) {
    ...
    if (!this.weapon) this.weapon = {};
    if (!this.weapon.combo_type_RE)
      this.weapon.combo_type_RE = /bare\-handed|kick/;

    if (!this.attack) this.attack = {};
    if (this.attack.scale == null) this.attack.scale = 1;

    if (!this.defense) this.defense = {};
    if (this.defense.scale == null) this.defense.scale = 1;
  }
  ...
  return {
    ...
    get use_octree() {
      return true;
    }, //MMD_SA_options.Dungeon_options.use_octree; }
...
*/
      ...
      return {
        ...
        NewDungeon: function (options) {
          this.dungeon = new this.DG({
            ...
            room_count:
              options.DG_room_count ||
              Math.round(Math.sqrt(options.width * options.height) * 0.5),
          });
          ...
          this.dungeon.children.forEach(function (c, idx) {
            ...
            if (c.tag) {
              for (
                var y = pos[1] + 1, y_max = pos[1] + c.size[1] - 1;
                y < y_max;
                y++
              ) {
                for (
                  var x = pos[0] + 1, x_max = pos[0] + c.size[0] - 1;
                  x < x_max;
                  x++
                ) {
                  g[y][x] = room_index;
    ...
    character: {
      ...
      bb_translate: {
        ...
        update: function () {
          ...
          this.z =
            0.5 +
            (MMD_SA.get_bone_position(
              mesh,
              (mesh.bones_by_name["上半身2"] && "上半身2") || "上半身",
              mesh
            ).z -
              MMD_SA.get_bone_position(mesh, "センター", mesh).z) /
              (mesh.geometry.boundingSphere.radius *
                (Math.max(mesh.scale.x, mesh.scale.y, mesh.scale.z) || 1));
      ...
      get _obj() {
        return THREE.MMD.getModels()[0].mesh;
      },
      get _mesh() {
        return THREE.MMD.getModels()[0].mesh;
      },
      ...
      _combat_mode: false,
      get combat_mode() {
        return this._combat_mode;
      },
      set combat_mode(bool) {
        bool = !!bool;
        if (bool == this._combat_mode) return;

        this._combat_mode = bool;
        var d = MMD_SA_options.Dungeon;
        if (bool) {
          MMD_SA_options.motion_shuffle_list_default =
            MMD_SA_options.Dungeon._motion_shuffle_list_default_combat.slice();
          MMD_SA_options._motion_shuffle_list_default =
            MMD_SA_options.motion_shuffle_list_default.slice();
          ...
          MMD_SA_options.motion_shuffle_list_default =
            MMD_SA_options.Dungeon._motion_shuffle_list_default.slice();
          MMD_SA_options._motion_shuffle_list_default =
            MMD_SA_options.motion_shuffle_list_default.slice();
          ...
          if (this.boundingBox_scale) this.update_boundingBox();
      ...
      pos_update: function () {
        ...
        for (var i = 1, i_max = d.PC_light_max; i < i_max; i++) {
          ...
          if (light.obj.target) {
            light.obj.target.position
              .copy(light._target_pos_base)
              .add(this.pos);
          }
        }
        ...
        d.PC_follower_list.forEach(function (para) {
          ...
          if (obj.rot_base) {
            obj._obj.rotation
              .copy(MMD_SA.TEMP_v3.copy(obj.rot_base))
            ...
            if (that.ground_normal && obj.follow_PC_ground_normal) {
              ...
              if (
                MMD_SA.TEMP_v3.crossVectors(
                  obj._ground_normal,
                  that.ground_normal
                ).lengthSq() >
                0.2 * 0.2
              ) {
                if (moved) obj._ground_normal.lerp(that.ground_normal, 0.1);
              } else obj._ground_normal.copy(that.ground_normal);
              const ground_q = new THREE.Quaternion().setFromRotationMatrix(
                MMD_SA.TEMP_m4.lookAt(
                  MMD_SA.TEMP_v3.set(0, 0, 0),
                  MMD_SA._v3a.set(0, 0, -1),
                  obj._ground_normal
                )
              );
      ...
      camera_update: (function () {
        ...
        return function () {
          ...
          if (d.camera_y_default_non_negative) pos.y = Math.max(this.pos.y, 0);
          ...
          var blocked = d.check_grid_blocking(
            MMD_SA.TEMP_v3,
            d.grid_blocking_camera_offset
          );
      ...
      hp_add: function (num, check_hp) {
        ...
        if (check_hp && check_hp(this)) return;
      ...
      mount: function (para) {
        ...
        d.PC_follower_list.push({
          obj: {
            ...
            follow_PC_ground_normal:
              para.target.use_PC_ground_normal_when_following !== false,
          },
          onupdate: para.onupdate,
          onidle: para.onidle,
        });

        if (para.PC_hidden) MMD_SA._skip_render_list.push("#mikuPmx0");
        ...
        if (para.mount_position || para.mount_rotation) {
          this._mount_position = function (e) {
            ...
            mesh._bone_morph["全ての親"] = {
              0: {
                pos_v3: para.mount_position || new THREE.Vector3(),
                rot_q:
                  (para.mount_rotation &&
                    new THREE.Quaternion().setFromEuler(
                      MMD_SA.TEMP_v3.copy(para.mount_rotation).multiplyScalar(
                        Math.PI / 180
                      )
                    )) ||
      ...
      dismount: function () {
        ...
        if (para.PC_hidden) {
          MMD_SA._skip_render_list = MMD_SA._skip_render_list.filter(function (
            p
          ) {
            return p != "#mikuPmx0";
          });
        ...
        if (para.camera_target_offset) MMD_SA.center_view_lookAt_offset = null;
        if (para.mount_position || para.mount_rotation) {
        ...
        if (para.dismount_position) {
          this.pos.add(
            MMD_SA.TEMP_v3.copy(para.dismount_position).applyQuaternion(
              THREE.MMD.getModels()[0].mesh.quaternion
            )
          );
          this.pos_update();
        }
        ...
        var saved =
          MMD_SA_options.Dungeon_options.options_by_area_id[d.area_id]._saved
            .object_by_index;
        saved = saved[para.target._index] = saved[para.target._index] || {};
        if (!saved.position) saved.position = {};
        if (!saved.rotation) saved.rotation = {};
      ...
      update_boundingBox: function (scale) {
        ...
        if (!this.boundingBox)
          this.boundingBox =
            (MMD_SA_options.model_para_obj.boundingBox &&
              new THREE.Box3().set(
                MMD_SA_options.model_para_obj.boundingBox.min,
                MMD_SA_options.model_para_obj.boundingBox.max
              )) ||
            model_mesh.geometry.boundingBox.clone();
        model_mesh.geometry.boundingBox = new THREE.Box3().set(
          this.boundingBox.min,
          this.boundingBox.max
        );
        model_mesh.geometry.boundingBox_list = [
          model_mesh.geometry.boundingBox,
        ];

        if (!scale) scale = MMD_SA._v3a.set(1, 1, 1);
        var bb_scale = this.boundingBox_scale || MMD_SA._v3b.set(1, 1, 1);
        var size = this.boundingBox.size(MMD_SA.TEMP_v3);
        model_mesh.geometry.boundingBox.expandByVector(
          new THREE.Vector3(
            size.x * (bb_scale.x * scale.x - 1) * 0.5,
            size.y * (bb_scale.y * scale.y - 1) * 0.5,
            size.z * (bb_scale.z * scale.z - 1) * 0.5
          )
        );
      },
      ...
      swap_character: function (character) {
        ...
        if (para_SA.is_PC_candidate) {
          if (!para_SA.character.combat_stats)
            para_SA.character.combat_stats = new CombatStats(
              para_SA.character.combat_stats_base
            );
      ...
      assign_keys: function () {
        ...
        if (MMD_SA_options.Dungeon_options.combat_mode_enabled) {
          const key_map_new = {};
          Object.keys(d.key_map)
            .filter((k) => k < 10000)
            .forEach((k) => {
              key_map_new[k] = d.key_map[k];
            });
          d.key_map = key_map_new;

          MMD_SA_options.Dungeon_options.attack_combo_list.forEach(function (
            combo
          ) {
            combo._RE = new RegExp(
              "(^|\\,)" + combo.combo_RE.replace(/\,/g, "\\,") + "(\\,|$)"
            );
            combo._RE_simple = new RegExp(
              "(^|\\,)" +
                (/^(123|456|789)$/.test(combo.combo_RE)
                  ? combo.combo_RE
                      .replace(/123/, "3\\,3")
                      .replace(/456/, "6\\,6")
                      .replace(/789/, "9\\,9")
            ...
            d.key_map[combo.keyCode] = {
              order: combo.keyCode,
              id: "combo-" + combo.keyCode,
              type_combat: true,
        ...
        Object.keys(d.key_map)
          .map((key) => d.key_map[key])
          .concat(d.key_map_combat || [])
          .concat(d.key_map_parry || [])
          .forEach(
            (function () {
              var p_to_sync = [
                "combat_para",
              ...
              return function (key_map) {
                modes.forEach(function (mode) {
                  ...
                  if (!("motion_filename" in km))
                    Object.defineProperty(km, "motion_filename", {
                      get: function () {
                        return (
                          this.motion_id &&
                          MMD_SA_options.Dungeon.motion_filename_by_id[
                            this.motion_id
                          ]
                        );
                      },
      ...
      reset: function () {
        ...
        for (
          var i = 1, i_max = MMD_SA_options.Dungeon.PC_light_max;
          i < i_max;
          i++
        ) {
          var light = MMD_SA.light_list[i];
          if (
            !light._pos_base ||
            !(light.obj instanceof THREE.DirectionalLight)
          )
        ...
        MMD_SA_options.Dungeon.PC_follower_list.forEach(function (para) {
          ...
          if (!obj) return;
          if (!obj.pos_base) obj.pos_base = obj._obj.position.clone();
    ...
    inventory: (function () {
      ...
      INV.prototype = {
        ...
        add: function (item_id, stock, swap_if_necessary) {
          ...
          var stock_added =
            this.item.stock_max && stock + stock_ini > this.item.stock_max
              ? this.item.stock_max - stock_ini
              : stock;
          this.stock = stock_ini + stock_added;
          this.item_id = item_id;

          if (stock_added) this.update_UI();
        ...
        update_info: function (
          info,
          display,
          page_index = inventory.get_page_index(this.index),
          index
        ) {
          if (page_index > -1 && page_index != inventory.page_index) return;

          if (index == null) index = inventory.get_UI_index(this.index);
          ...
          if (info) {
            ...
            className = display
              ? "Dungeon_inventory_item_info_display"
          ...
          d.setAttribute(
            "data-info_short",
            this.item.info_short || "Item" + index
          );
        ...
        update_UI: function () {
          ...
          var icon = document.getElementById(
            "Ldungeon_inventory_item" + index + "_icon"
          );
          icon.src = this.item.icon.src;
          icon.style.opacity = 1;

          document.getElementById(
            "Ldungeon_inventory_item" + index + "_border"
          ).src = item_border[this.item.rarity].src;

          var d_stock = document.getElementById(
            "Ldungeon_inventory_item" + index + "_stock"
          );
          d_stock.textContent = this.stock;
          d_stock.style.visibility =
            this.stock && this.item.stock_max != 1 ? "inherit" : "hidden";

          const d = document.getElementById("Ldungeon_inventory_item" + index);

          this.update_info(null, false, page_index, index);

          d.style.visibility = this.item.is_always_visible
            ? "visible"
            : "inherit";
        },
        ...
        action_check: (function () {
          ...
          return async function () {
            ...
            if (!action) {
              MMD_SA_options.Dungeon.sound.audio_object_by_name[
                (
                  (this.item.sound &&
                    this.item.sound.find(function (i) {
                      return i.is_no_action;
                    })) ||
                  sound_item_deny
                ).name
              ].play();
            ...
            if (!action.anytime && inventory.action_disabled) {
              const branch = MMD_SA_options.Dungeon.dialogue_branch_mode?.find(
                (b) => b.is_closing_event
              );
              if (!branch) {
                MMD_SA_options.Dungeon.sound.audio_object_by_name[
                  sound_item_deny.name
                ].play();
                return false;
              }

              const key = Array.isArray(branch.key)
                ? branch.key[0]
                : branch.key;
              let code, keyCode;
              ...
              document.dispatchEvent(
                new KeyboardEvent("keydown", { code: code, keyCode: keyCode })
              );
      ...
      System._browser.load_file(
        System.Gadget.path +
          "/images/_dungeon/item_icon.zip#/inventory/RarityBorders/mono_L-50_V11.png",
        item_border.inactive
      );
      item_border.normal = new Image();
      System._browser.load_file(
        System.Gadget.path +
          "/images/_dungeon/item_icon.zip#/inventory/RarityBorders/monoV11.png",
        item_border.normal
      );
      ...
      inventory = {
        ...
        UI: {
          ...
          get muted() {
            return UI_muted || System._browser.overlay_mode;
          },
          set muted(v) {
            UI_muted = v;
          },
        ...
        get_UI_index: function (idx) {
          return idx < this.max_base
            ? idx
            : this.max_base +
                ((idx - this.max_base) % (this.max_base * (this.max_row - 1)));
        },

        get_inventory_index: function (idx) {
          return idx < this.max_base || idx > this.max_base * this.max_row
            ? idx
            : idx + this.page_index * this.max_base * (this.max_row - 1);
        },
        ...
        get item_selected_index() {
          return this._item_selected_index;
        },
        set item_selected_index(v) {
          ...
          if (typeof this._item_selected_index === "number")
            document.getElementById(
              "Ldungeon_inventory_item" +
                this.get_UI_index(this._item_selected_index)
            ).style.opacity = 1;
          ...
          if (typeof v === "number")
            document.getElementById(
              "Ldungeon_inventory_item" + this.get_UI_index(v)
            ).style.opacity = 0.75;
        },

        get action_disabled() {
          return MMD_SA_options.Dungeon.event_mode;
        },
        ...
        add: (function () {
          ...
          function sort(a, b) {
            ...
            if (a_is_item && !b_is_item) return -1;
            if (!a_is_item && b_is_item) return 1;
          ...
          return function (id, stock) {
            item_id = id;
            var success = this.list
              .filter(filter)
              .sort(sort)
              .some(function (inv, idx) {
        ...
        find: function (item_id, page_index) {
          ...
          for (let i = ini; i < end; i++) {
            if (inv[i].item_id == item_id) return inv[i];
          }
        ...
        unshift: function (source_index, page_index, forced) {
          ...
          if (forced) return this.unshift(source_index);
        },
        ...
        update_page: function (page_index) {
          ...
          var ini =
            this.max_base + page_index * this.max_base * (this.max_row - 1);
          var end = ini + this.max_base * (this.max_row - 1);
          //DEBUG_show(ini+'/'+end,0,1)
          for (let i = ini; i < end; i++) this.list[i].update_UI();
        },
        ...
        reset: function () {
          ...
          for (var id in MMD_SA_options.Dungeon.item_base) {
            ...
            if (item.index_default >= 0) {
              this.list[item.index_default].add(
                id,
                item.stock_default || 1,
                true
              );
          ...
          if (options.inventory && options.inventory.list) {
            options.inventory.list.forEach(function (item) {
              if (item.index == null) that.add(item.item_id, item.stock);
              else that.list[item.index].add(item.item_id, item.stock);
            });
          }
        },
      };
    ...
    check_grid_blocking: (function () {
      ...
      return function (pos, offset, dir) {
        ...
        if (
          this.ceil_material_index_default != -1 &&
          pos.y > this.ceil_height - 20
        ) {
          return true; //{ limited:{y:1} }
        }
        var is_camera = offset == this.grid_blocking_camera_offset;
        var no_ceil_camera_high =
          this.ceil_material_index_default == -1 &&
          is_camera &&
          pos.y > this.ceil_height;
        ...
        for (var x in X) {
          for (var y in Y) {
            if (
              x < 0 ||
              x >= this.RDG_options.width ||
              y < 0 ||
              y >= this.RDG_options.height ||
              (!no_ceil_camera_high && this.grid_array[y][x] == 1)
            ) {
              if (!dir) return true;
              //DEBUG_show(blk_v3.set(_x-x,0,_y-y).dot(dir))
              if (blk_v3.set(_x - x, 0, _y - y).dot(dir) < 0) return true;
            } else if (
              !is_camera &&
              this.get_ground_y(
                { x: x * grid_size, y: 0, z: y * grid_size },
                -999
              ) >
                pos.y + 10
            ) {
    ...
    GOML_dungeon_blocks: function () {
      ...
      for (var i = 0, i_max = p.length; i < i_max; i++) {
        var p_obj = p[i];
        if (p_obj.disabled) continue;

        if (p_obj.waveBaseSpeed != null) use_waveNoiseTexture = true;
        ...
        if (p_obj.map && p_obj.map_id == null) {
          ...
          tex_list.push({
            tag: "txr",
            id: "DungeonPlane" + i + "TXR",
            src: p_obj.map,
            para: { repeat: [1, 1] },
          });
        }
        p_obj.normalMap_id =
          p_obj.normalMap && txr_normalMap_id[p_obj.normalMap];
        if (p_obj.normalMap && txr_normalMap_id[p_obj.normalMap] == null) {
          ...
          tex_list.push({
            tag: "txr",
            id: "DungeonPlane" + i + "TXR_N",
            src: p_obj.normalMap,
            para: { repeat: [1, 1] },
          });
        }
        p_obj.specularMap_id =
          p_obj.specularMap && txr_specularMap_id[p_obj.specularMap];
        if (
          p_obj.specularMap &&
          txr_specularMap_id[p_obj.specularMap] == null
        ) {
          ...
          tex_list.push({
            tag: "txr",
            id: "DungeonPlane" + i + "TXR_S",
            src: p_obj.specularMap,
            para: { repeat: [1, 1] },
          });
        }
        ...
        if (
          ...
          (!MMD_SA_options.mirror_obj ||
            !MMD_SA_options.mirror_obj[p_obj.mirrorTextureIndex])
        ) {
...
*/
        ...
        if (p_obj.renderDepth != null)
          mtl_param_common.renderDepth = p_obj.renderDepth;
        if (p_obj.side) mtl_param_common.side = p_obj.side;
        if (p_obj.map)
          mtl_param_common.map = "DungeonPlane" + p_obj.map_id + "TXR";
        if (p_obj.normalMap)
          mtl_param_common.normalMap =
            "DungeonPlane" + p_obj.normalMap_id + "TXR_N";
        //'color' : 'ambient'
        if (p_obj.ambient) mtl_param_common.ambient = p_obj.ambient;
        if (p_obj.specularMap) {
          mtl_param_common.specularMap =
            "DungeonPlane" + p_obj.specularMap_id + "TXR_S";
        ...
        if (p_obj.mirrorTextureIndex != null)
          mtl_param_common_extra.mirrorTextureIndex = p_obj.mirrorTextureIndex;
        if (p_obj.waveBaseSpeed != null)
          mtl_param_common_extra.waveBaseSpeed = p_obj.waveBaseSpeed;
        if (typeof p_obj.uniTexture == "object")
          mtl_param_common_extra.uniTexture = p_obj.uniTexture;
          ...
          if (p_obj.random_terrain) {
            ...
            if (!t_map) {
              ...
              var terrain = generateTerrain(
                w - 1,
                h - 1,
                p_obj.random_terrain.smoothness || 1
              );
              //console.log(terrain)
              var z_min = 1;
              var z_max = -1;
              ...
              for (var y = 0; y < h; y++) {
                for (var x = 0; x < w; x++) {
                  ...
                  data[index + 0] =
                    data[index + 1] =
                    data[index + 2] =
                      ~~(((terrain[y][x] - z_min) / z_scale) * 255); //random(256)//
                  data[index + 3] = 255;
                }
              }
...
*/
            ...
            p_obj.uDisplacementCustomUVScale =
              1 / (p_obj.random_terrain.divider || 1);
          ...
          if (d_map_draw) {
            ...
            tex_list.push({
              tag: "txr",
              id: "DungeonPlane" + d_map_id + "TXR_D",
              src: p_obj.displacementMap,
              para: { repeat: [1, 1] },
            });
          }
          //    mtl_head +=
          ...
          const mtl_para = Object.assign(
            {
              displacementMap: "DungeonPlane" + d_map_id + "TXR_D",
              uDisplacementScale: p_obj.uDisplacementScale || 1,
              uDisplacementBias: p_obj.uDisplacementBias || 0,
            },
            mtl_param_common
          );
          const mtl_para_extra = Object.assign(
            {
              uDisplacementCustomUVScale: p_obj.uDisplacementCustomUVScale || 0,
            },
            mtl_param_common_extra
          );
          mtl_list.push({
            tag: "mtl",
            id: "DungeonPlane" + i + "MTL_D",
            type: "MeshPhong",
            para: mtl_para,
            para_extra: mtl_para_extra,
          });
        }
        ...
        mtl_list.push({
          tag: "mtl",
          id: "DungeonPlane" + i + "MTL",
          type: "MeshPhong",
          para: mtl_param_common,
          para_extra: mtl_param_common_extra,
        });

        for (
          var lvl = 0, lvl_max = p_obj.geo_by_lvl.length;
          lvl < lvl_max;
          lvl++
        ) {
          ...
          geo_dim[geo_id] = true;
          var instanced_drawing =
            (p_obj.instanced_drawing_by_lvl &&
              p_obj.instanced_drawing_by_lvl[lvl]) ||
            0;
          //    mesh_scene +=
          ...
          mesh_scene_list.push({
            tag: "mesh",
            id: "DungeonPlane" + i + "MESH_LV" + lvl,
            geo: "DungeonGEO_" + geo_id,
            mtl:
              "DungeonPlane" +
              i +
              "MTL" +
              (p_obj.displacementMap && geo_id != "1x1" ? "_D" : ""),
            instanced_drawing: instanced_drawing || null,
            style: { scale: 0, opacity: p_obj.opacity || null },
          });
          MMD_SA_options.mesh_obj.push({
            id: "DungeonPlane" + i + "MESH_LV" + lvl,
          });
        }
      }
      ...
      if (use_waveNoiseTexture) {
        ...
        tex_list.push({
          tag: "txr",
          id: "waveNoiseTexture",
          src: System.Gadget.path + "/images/watershader_cloud.png",
          para: {},
        });
      }
      ...
      for (var geo in geo_dim) {
        if (/^(\d+)x(\d+)$/.test(geo)) {
          ...
          geo_list.push({
            ...
            id: "DungeonGEO_" + geo,
            type: "Plane",
            para: [1, 1, parseInt(RegExp.$1), parseInt(RegExp.$2)],
          });
        }
      }

      MMD_SA_options.GOML_head += tex_head + mtl_head;

      MMD_SA_options.GOML_scene +=
        (MMD_SA_options.Dungeon_options.use_point_light == false
          ? ""
          : '<light id="pointlight_main" type="Poi" style="lightIntensity: 1.0; lightDistance: ' +
            0 +
            "; position: " +
            [0, 0, 0].join(" ") +
            '; lightColor:#ffffff;" />\n') + mesh_scene;

      MMD_SA_options.GOML_head_list = (
        MMD_SA_options.GOML_head_list || []
      ).concat(tex_list, geo_list, mtl_list);
      MMD_SA_options.GOML_scene_list = (
        MMD_SA_options.GOML_scene_list || []
      ).concat(mesh_scene_list);
...
*/
      ...
      return function (pos, fixed_value_for_random_terrain) {
        ...
        if (material_id == -1) return ground_y;

        var p_obj = this.grid_material_list[material_id];
        if (!p_obj.random_terrain) return ground_y;
        ...
        var xx =
          ((x % p_obj.random_terrain.divider) +
            (pos.x - x * this.grid_size) / this.grid_size) *
          p_obj.uDisplacementCustomUVScale *
          image_data.width; //    - 0.5
        // y inverted
        var yy_offset =
          p_obj.random_terrain.divider - (y % p_obj.random_terrain.divider);
        // clumsy, but it works lol
        if (yy_offset == p_obj.random_terrain.divider) yy_offset = 0;
        var yy =
          image_data.height -
          (yy_offset + (1 - (pos.z - y * this.grid_size) / this.grid_size)) *
            p_obj.uDisplacementCustomUVScale *
            image_data.height; // - 0.5
        /*
if (yy < 0) {
...
*/
        ...
        var height =
          dummy_destImg.data[(~~((yy % 1) * 10) * 10 + ~~((xx % 1) * 10)) * 4];
        //DEBUG_show([x,y]+'\n'+[xx+0.5,yy+0.5]+'\n'+[(pos.x - x * this.grid_size) / this.grid_size,(pos.z - y * this.grid_size) / this.grid_size]+'\n'+[height_by_x, height_by_y])
        ground_y +=
          p_obj.uDisplacementBias + (height / 255) * p_obj.uDisplacementScale;
    ...
    get_para: (function () {
      ...
      return function (x, y, full) {
        ...
        if (x < 0 || x >= ro.width || y < 0 || y >= ro.height) return dummy;

        var grid_id = this.grid_array[y][x];
        if (full) {
          return Object.assign(
            {},
            (this.para_by_map && this.para_by_map(x, y)) || dummy,
            this.para_by_grid_id[grid_id] || dummy,
            this.para_by_xy[x + "x" + y] || dummy
          );
        }

        return (
          this.para_by_xy[x + "x" + y] ||
          this.para_by_grid_id[grid_id] ||
          (this.para_by_map && this.para_by_map(x, y)) ||
          dummy
        );
      };
    })(),
    ...
    update_dungeon_blocks: (function () {
      ...
      function is_visible_wall(x, y) {
        ...
        if (d[y][x] == 0) {
          is_hidden_door = TRBL.some(function (dir) {
            var _x = x + dir.xy[0];
            var _y = y + dir.xy[1];
            if (_y < 0 || _y >= h || _x < 0 || _x >= w) return false;
      ...
      var grid_cache = {
        ...
        release: function (xy, visibility_kept) {
          if (!this.xy[xy]) return;

          this.xy[xy].forEach(function (cache) {
            if (!visibility_kept) cache.list[cache.index].visible = false;
            cache.obj[
              cache.list == cache.obj.list
                ? "reusable_list"
        ...
        release_outdated: function () {
          MMD_SA_options.Dungeon.grid_material_list.forEach(function (p_obj) {
            if (p_obj.disabled || p_obj.lvl.length == 1) return;
          ...
          for (var xy in this.xy) {
            ...
            if (timestamp != RAF_timestamp) this.release(xy, timestamp == -1);
          }
        },
      ...
      return function (forced) {
        ...
        if (MMD_SA_options.Dungeon_options.multiplayer) {
          ...
          for (
            var i = 1,
              i_max =
                MMD_SA_options.Dungeon_options.multiplayer.OPC_list.length;
            i <= i_max;
            i++
          ) {
            let ds = document.getElementById("Ldungeon_map_spot_OPC" + i).style;
            let OPC_index =
              i - 1 + MMD_SA_options.Dungeon_options.multiplayer.OPC_index0;
            let OPC_mesh = THREE.MMD.getModels()[OPC_index].mesh;
            if (OPC_mesh.visible) {
              ds.posLeft =
                8 +
                ~~(OPC_mesh.position.x / grid_size) * map_display_scale +
                ~~(map_display_scale / 4);
              ds.posTop =
                8 +
                ~~(OPC_mesh.position.z / grid_size) * map_display_scale +
                ~~(map_display_scale / 4);
              ds.visibility = "inherit";
            } else ds.visibility = "hidden";

            let id = this.object_id_translated["OPC-" + (i - 1)];
            if (!/^object(\d+)_(\d+)$/.test(id)) continue;
            ...
            if (obj._obj_proxy.hidden) continue;
            let cache = obj._obj;
        ...
        window.dispatchEvent(
          new CustomEvent("SA_Dungeon_update_dungeon_blocks")
        );
        ...
        if (
          grid_para_last.onexit &&
          grid_para_last.onexit({
            x: _xx,
            y: _yy,
            grid_id_changed: grid_id_changed,
          })
        )
          return;
        if (
          grid_para_now.onenter &&
          grid_para_now.onenter({
            x: xx,
            y: yy,
            grid_id_changed: grid_id_changed,
        ...
        Ldungeon_map_spot.style.posLeft =
          8 + xx * map_display_scale + ~~(map_display_scale / 4);
        Ldungeon_map_spot.style.posTop =
          8 + yy * map_display_scale + ~~(map_display_scale / 4);

        Cdungeon_map_compass_canvas.style.posLeft =
          Ldungeon_map_spot.style.posLeft -
          16 +
          (Ldungeon_map_spot.style.pixelWidth - 1) * 0.5;
        Cdungeon_map_compass_canvas.style.posTop =
          Ldungeon_map_spot.style.posTop -
          16 +
          (Ldungeon_map_spot.style.pixelHeight - 1) * 0.5;

        no_wall =
          this.para_by_grid_id[1] && this.para_by_grid_id[1].hidden_on_map;
        //DEBUG_show(c.xy+'/'+no_wall+'\n'+(this.get_para(xx,yy,true).map_grid_color||""))
        var mirror_active = {};

        //var grid_cache_hit = 0
        for (
          var y = yy - view_radius, y_max = yy + view_radius;
          y < y_max;
          y++
        ) {
          ...
          for (
            var x = xx - view_radius, x_max = xx + view_radius;
            x < x_max;
            x++
          ) {
            ...
            if (y >= 0 && y < h && x >= 0 && x < w && !m[y][x]) {
              ...
              if (
                no_wall ||
                grid_id == c_grid_id ||
                (grid_id == 0 && !is_hidden_door)
              ) {
                var draw_grid = false;
                if (
                  !no_wall &&
                  grid_id == 0 &&
                  Math.abs(x - xx) + Math.abs(y - yy) > 1
                ) {
                ...
                if (draw_grid) {
                  ...
                  if (!is_hidden_door) {
                    context.fillStyle =
                      this.get_para(x, y, true).map_grid_color || "green";
                    context.fillRect(
                      x * map_display_scale,
                      y * map_display_scale,
                      map_display_scale,
            ...
            var ground_y_visible =
              this.get_para(x, y, true).ground_y_visible || 0;
            if (
              y < 0 ||
              y >= h ||
              x < 0 ||
              x >= w ||
              (is_visible_wall(x, y) && (x != xx || y != yy))
            ) {
              ...
              TRBL.forEach(function (dir, idx) {
                var _x = x + dir.xy[0];
                var _y = y + dir.xy[1];
                if (_y < 0 || _y >= h || _x < 0 || _x >= w || d[_y][_x] == 1)
                  //is_visible_wall(_x,_y))//
                  return;

                var grid_para = (_TRBL_grid_para[idx] = that.get_para(_x, _y));
                if (
                  grid_para.no_wall &&
                  (grid_para.no_wall.all || grid_para.no_wall[dir.id_opposite])
                ) {
                  return;
                }
              ...
              TRBL.forEach(function (dir, idx) {
                ...
                if (lvl == null) {
                  ...
                  for (
                    var i = 0, i_max = p_obj.distance_by_lvl.length;
                    i < i_max;
                    i++
                  ) {
                    if (dis <= p_obj.distance_by_lvl[i]) break;
                    lvl--;
                  }
                ...
                plane.position.set(
                  (x + dir.posXZ[0]) * grid_size,
                  grid_size * 0.5,
                  (y + dir.posXZ[1]) * grid_size
                );
                ...
                plane.material.edgeScaleDisplacementU =
                  that.ceil_material_index_default != -1 ? 1 : 0;
                plane.material.uDisplacementScale =
                  (p_obj.uDisplacementScale || 1) / grid_size;
                plane.material.uDisplacementBias =
                  (p_obj.uDisplacementBias || 0) / grid_size;
                ...
                if (!obj_y_extended) obj_y_extended = {};
                obj_y_extended[idx] = {
                  obj: p_obj.lvl[0],
                  ground_y: ground_y_visible,
                  ceil_height: that.ceil_height,
                };
              });

              if (
                this.ceil_material_index_default == -1 &&
                this.wall_material_index_default != -1
              ) {
              ...
              p_obj_list.forEach(function (_p_obj, idx) {
                ...
                var plane;
                if (
                  p_obj.random_terrain &&
                  p_obj.uDisplacementCustomUVScale < 1 &&
                  p_obj.geo_by_lvl[lvl] != "1x1"
                ) {
                  plane = obj.get_obj(obj.list_material_cloned);
                  grid_cache.add(xy, obj, obj.list_material_cloned, plane[1]);
                  plane = plane[0];

                  plane.material.uDisplacementCustomUVScale =
                    p_obj.uDisplacementCustomUVScale;
                  // y inverted
                  plane.material.uDisplacementCustomUVOffset
                    .set(
                      x % p_obj.random_terrain.divider,
                      p_obj.random_terrain.divider -
                        (y % p_obj.random_terrain.divider)
                    )
                    .multiplyScalar(p_obj.uDisplacementCustomUVScale);
                } else {
                  plane = obj.get_obj(obj.list);
                  grid_cache.add(xy, obj, obj.list, plane[1]);
                  plane = plane[0];
                ...
                if (!p_obj1 || idx == 1)
                  obj_y_extended = {
                    0: { obj: p_obj.lvl[0], ground_y: _ground_y },
                  };
              });
            }

            if (!obj_y_extended)
              // && ((this.ceil_material_index_default != -1) || (this.wall_material_index_default != -1)))
              continue;
            ...
            TRBL.forEach(function (dir, idx) {
              ...
              if (obj_ground_y > _ground_y) {
                ...
                for (
                  var i = 0, i_max = Math.ceil(pitfall_y / grid_size);
                  i < i_max;
                  i++
                ) {
                  var plane = obj.get_obj(obj.list);
              ...
              if (
                _obj.ceil_height &&
                _obj.ceil_height > grid_size &&
                this.ceil_material_index_default != -1
              ) {
                for (
                  var i = 1, i_max = Math.ceil(_obj.ceil_height / grid_size);
                  i < i_max;
                  i++
                ) {
                  var plane = obj.get_obj(obj.list);
...
*/
        MMD_SA._THREE_mirror.forEach(function (mirror, idx) {
          mirror.visible = MMD_SA_options.mesh_obj_by_id[
            "Mirror" + idx + "MESH"
          ]._obj.visible = !!mirror_active[idx];
        ...
        this.object_list.forEach(function (obj) {
          ...
          if (obj._obj_proxy._hidden) return;
          var cache = obj._obj;
          d_sq = cache.position.distanceToSquared(c.pos);
          var v = (obj._obj_proxy.visible =
            d_sq < lengthSq * obj.view_distance_sq);
          if (obj._obj_LOD_far)
            obj._obj_LOD_far_proxy.visible = !v && d_sq < lengthFarSq;
        });

        this.object_list_in_view = this.object_list.filter(function (obj) {
          if (obj.is_dummy) return false;

          if (obj.no_collision) return false;

          return obj._obj.visible;
        });
        this.object_list_in_view = this.object_list_in_view.concat(
          this.grid_blocks.objs
        );
      };
    })(),
    ...
    restart: (function () {
      class Object3D_proxy extends Object3D_proxy_base {
        ...
        set hidden(v) {
          this._hidden = v;
          if (v) {
            MMD_SA_options.Dungeon.sound.detach_positional_audio(
              this._parent._obj
            );
          }
        }
        ...
        set visible(v) {
          ...
          cache.init();
          if (v) {
            if (!p[this._obj_name].is_Object3D_proxy) return;
            ...
            if (cache.reusable_list.length) {
              ...
              if (!obj_mesh) {
                ...
                if (obj_mesh.bones) {
                  ...
                  obj_mesh.updateMatrixWorld = (function () {
                    var updateMatrixWorld =
                      obj_mesh._mesh_parent.updateMatrixWorld;
                    return function () {
                ...
                if (!cache.is_LOD_far) {
                  MMD_SA.get_bounding_host(obj_mesh).boundingBox_list =
                    MMD_SA.get_bounding_host(cache0).boundingBox_list;
                }
            ...
            if (!cache.is_LOD_far) {
              ...
              p._mesh =
                obj_mesh.geometry || obj_mesh.isObject3D
                  ? obj_mesh
                  : obj_mesh.children[0];
              if (p.cache_index == 0)
                MMD_SA_options.Dungeon.object_base_list[
                  p.object_index
                ]._obj.scale = obj_source.scale.x;
            }
            ...
            if (cache.is_LOD_far)
              obj_mesh.position.add(
                MMD_SA.TEMP_v3.copy(cache.obj_base.LOD_far.center).multiply(
                  obj_source.scale
                )
              );
            ...
            if (!cache.is_LOD_far) p._mesh = this;
          }

          obj_mesh.visible = v;
          if (!MMD_SA.THREEX.enabled)
            obj_mesh.children.forEach(function (c) {
              c.visible = v;
            });
        }
      }
      ...
      var animate_combat_default = function () {
        ...
        if (!c.combat_mode || d.event_mode) return;

        var npc_model = THREE.MMD.getModels()[this.character_index];
        var npc_motion_para =
          MMD_SA.motion[npc_model.skin._motion_index].para_SA;
        var model_para =
          MMD_SA_options.model_para_obj_all[this.character_index];
        if (
          npc_motion_para.NPC_motion_command_disabled ||
          npc_motion_para.motion_command_disabled
        ) {
          //DEBUG_show(Date.now())
          this.motion = null;

          if (
            npc_motion_para.duration_NPC &&
            npc_model.skin.time > npc_motion_para.duration_NPC
          ) {
            model_para._motion_name_next =
              model_para.motion_name_default_combat;
          }
          ...
          if (npc_motion_para.mov_speed)
            mov.copy(
              d._mov_delta(
                npc_model,
                npc_motion_para,
                Math.max(npc_model.skin.time - (this._skin_time_last || 0), 0)
              )
            );
          this._skin_time_last = npc_model.skin.time;
          //DEBUG_show(mov.toArray().join("\n"))
          var pos_delta = npc_motion_para.bone_to_position
            ? npc._bone_to_position_last.pos_delta
            : null;
          if (pos_delta) {
          ...
          if (npc_motion_para.combat_para) {
            ...
            d.combat_para_process(
              { obj: this, mass: this.mass },
              npc_motion_para,
              npc_model.skin.time * 30
            );
          }

          d._mov[this._index] =
            mov.x || mov.y || mov.z ? mov.applyQuaternion(npc.quaternion) : mov;
        ...
        MMD_SA.TEMP_v3.set(
          ...
          Math.PI / 2 -
            Math.atan2(
              pc.position.z - npc.position.z,
              pc.position.x - npc.position.x
            ),
          0
        );
        ...
        if (action_obj.type == "STAY") {
          ...
          if (inside_zone) {
            ...
            if (
              ...
              if (dir_final[0]) {
                let y_center_offset =
                  grid_size * 0.5 - (npc.position.z % grid_size);
                if (Math.abs(y_center_offset) > mov_scale)
                  dir_final = [0, y_center_offset > 0 ? 1 : -1];
              } else {
                let x_center_offset =
                  grid_size * 0.5 - (npc.position.x % grid_size);
              ...
              MMD_SA.TEMP_v3.set(
                ...
                Math.PI / 2 -
                  Math.atan2(
                    y_target - npc.position.z,
                    x_target - npc.position.x
                  ),
                0
              );
              npc.quaternion.setFromEuler(MMD_SA.TEMP_v3);
              ...
              if (dis > 64) dir = ["U"];
              else if (dis > 32) dir = ["U", "L", "R"];
              else if (dis < 8) dir = ["D", "L", "R"];
            }
          }
          if (!dir) dir = ["U", "D", "L", "R"];
          ...
          var end_pt = npc.position
            .clone()
            .add(
              v3_dir.multiplyScalar(mov_scale).applyQuaternion(npc.quaternion)
            );

          ["x", "z"].forEach(function (p) {
            if (end_pt[p] < zm.min[p]) end_pt[p] = zm.min[p];
            else if (end_pt[p] > zm.max[p]) end_pt[p] = zm.max[p];
          });
      ...
      return function (area_id, refresh_state) {
        if (!refresh_state) refresh_state = 0;
        ...
        var options_start =
          MMD_SA_options.Dungeon_options.options_by_area_id["start"];
        var options;

        if (this.area_id) {
          // unload
          options =
            MMD_SA_options.Dungeon_options.options_by_area_id[this.area_id];
          ...
          if (this.ceil_material_index_default != -1) {
            var ceil =
              this.grid_material_list[this.ceil_material_index_default].lvl[0]
                .list[1];
            if (ceil) ceil.visible = false;
          }
          ...
          for (var index in options._saved.object_by_index) {
            ...
            for (var p in saved) {
              ...
              if (p == "rotation") {
                saved.rotation.data =
                  obj._obj[
                    obj._obj.useQuaternion ? "quaternion" : "rotation"
                  ].clone();
              }
            }
          }
          ...
          if (refresh_state == 0) {
            ...
            for (var id in MMD_SA_options.Dungeon_options.options_by_area_id) {
              var area_options =
                MMD_SA_options.Dungeon_options.options_by_area_id[id];
        ...
        this.camera_y_default_non_negative =
          options.camera_y_default_non_negative !== false;

        this.para_by_grid_id =
          (options.para_by_grid_id && Object.clone(options.para_by_grid_id)) ||
          {};
        this.para_by_xy =
          (options.para_by_xy && Object.clone(options.para_by_xy)) || {};
        this.para_by_map =
          options.para_by_map && Object.clone(options.para_by_map);
        /*
...
*/
        this.ceil_material_index_default =
          options.ceil_material_index_default != null
            ? options.ceil_material_index_default
            : 0;
        this.floor_material_index_default =
          options.floor_material_index_default != null
            ? options.floor_material_index_default
            : 1;
        this.wall_material_index_default =
          options.wall_material_index_default != null
            ? options.wall_material_index_default
            : 2;

        var d_options = this.RDG_options;
        var grid_array = (this.grid_array =
          (this.RDG_options.grid_array &&
            this.RDG_options.grid_array.map(function (x) {
              return x.slice();
            })) ||
          this.RDG.NewDungeon(d_options));
        //console.log(this.grid_array)

        this.grid_size = options.grid_size || options_start.grid_size || 64;
        this.view_radius =
          options.view_radius || options_start.view_radius || 8;
...
*/
        ...
        for (var y = 0, y_max = d_options.height; y < y_max; y++) {
          for (var x = 0, x_max = d_options.width; x < x_max; x++) {
            this.room_max_default = Math.max(
              this.room_max_default,
              grid_array[y][x]
            );
          }
        }
        ...
        if (this.para_by_grid_id[1] && this.para_by_grid_id[1].hidden_on_map) {
          ...
          if (!para_new_room) {
            para_new_room = this.para_by_grid_id[this.room_max_default] =
              Object.clone(this.para_by_grid_id[1]);
          }
        ...
        for (var y = 0, y_max = d_options.height; y < y_max; y++) {
          ...
          for (var x = 0, x_max = d_options.width; x < x_max; x++) {
            ...
            map_str +=
              index < 10 ? index : String.fromCharCode(65 + (index - 10));
          }
          map_str += "\n";
        }
        ...
        for (var id in this.para_by_grid_id) {
          if (/^room_max_default([\+\-])(\d+)$/.test(id))
            this.para_by_grid_id[
              this.room_max_default +
                (RegExp.$1 == "+" ? 1 : -1) * parseInt(RegExp.$2)
            ] = this.para_by_grid_id[id];
        }
          ...
          if (/^\d+$/.test(id)) {
            ...
            if (para.para_by_xy) {
              ...
              para.para_by_xy.forEach(function (para_xy) {
                let x =
                  ~~(Math.min(para_xy.x, 0.9999) * room.width) + room.x_min;
                let y =
                  ~~(Math.min(para_xy.y, 0.9999) * room.height) + room.y_min;
                that.para_by_xy[x + "x" + y] = Object.assign(
                  that.para_by_xy[x + "x" + y] || {},
                  para_xy.para
                );
        ...
        MMD_SA_options.trackball_camera_limit.max.length =
          this.grid_size * 3 * (options.camera_limit_scale || 1);

        window.dispatchEvent(
          new CustomEvent("SA_Dungeon_after_map_generation", {
            detail: { refresh_state: refresh_state, options: options },
          })
        );

        this.PC_light_max = Math.min(
          options.PC_light_max || options_base.PC_light_max || 3,
          MMD_SA.light_list.length
        );

        MMD_SA_options.ambient_light_color =
          options.ambient_light_color || MMD_SA_options.ambient_light_color;
        jThree("#MMD_AmbLight").three(0).color = new THREE.Color(
          MMD_SA_options.ambient_light_color
        );

        var dir_light = jThree("#MMD_DirLight").three(0);
        MMD_SA_options.light_color =
          options.light_color || MMD_SA_options.light_color;
        dir_light.color = new THREE.Color(MMD_SA_options.light_color);
        MMD_SA_options.light_position =
          options.light_position ||
          options_base.light_position ||
          MMD_SA_options._light_position;
        dir_light.position.fromArray(MMD_SA_options.light_position);
        MMD_SA.light_list.find(function (light) {
          return light.obj == dir_light;
        })._pos_base = null;

        var point_light = jThree("#pointlight_main").three(0);
        if (point_light) {
          point_light.intensity =
            (options.point_light && options.point_light.intensity) || 1;
          point_light.color = new THREE.Color(
            (options.point_light && options.point_light.color) || "#ffffff"
          );
          point_light.distance =
            (options.point_light && options.point_light.distance) ||
            this.grid_size * 2;
          point_light.position.copy(
            (options.point_light && options.point_light.position) || {
              x: 0,
              y: this.grid_size * 0.5,
              z: 0,
            }
          );
        }
        ...
        Object.assign(
          ...
          options_base.events_default || {}
        );

        if (options.object_list && !options._object_list_initialized) {
          options._object_list_initialized = true;
          options.object_list = options.object_list.map(function (obj) {
            return (obj._constructor && obj._constructor()) || obj;
          });
        }
        this.object_list =
          (options.object_list && Object.clone(options.object_list)) || [];
        ...
        if (fog) {
          ...
          if (scene.fog instanceof THREE.Fog) {
            scene.fog.near =
              fog.near ||
              this.grid_size * this.view_radius * (fog.near_ratio || 0.5);
            scene.fog.far =
              fog.far ||
              this.grid_size * this.view_radius * (fog.far_ratio || 0.9);
          }

          scene.fog.color = new THREE.Color(fog.color || "#000");
          if (use_MatrixRain && MMD_SA.matrix_rain.greenness) {
            let gray =
              0.3 * scene.fog.color.r +
              0.59 * scene.fog.color.g +
              0.11 * scene.fog.color.b;
            scene.fog.color.setRGB(
              scene.fog.color.r * (1 - MMD_SA.matrix_rain.greenness),
              gray * MMD_SA.matrix_rain.greenness +
                scene.fog.color.g * (1 - MMD_SA.matrix_rain.greenness),
              scene.fog.color.b * (1 - MMD_SA.matrix_rain.greenness)
            );
          }
          console.log(
            "Fog:" +
        ...
        if (options_base.skydome) {
          ...
          if (options.skydome) {
            dome_obj.scale =
              (MMD_SA._trackball_camera.object.far * 0.5) / (64 * 4); //(this.grid_size * (this.view_radius * 4)) / (64*4)
            this.ceil_material_index_default =
              options.ceil_material_index_default = -1;
            if (use_MatrixRain) {
              dome_obj._obj._uniTexture_fadeout =
                0.5 +
                ((options.skydome.fog && options.skydome.fog.height) || 0.025);
              dome_obj._obj._uniTexture_scale = 10;
            }

            if (!options.skydome.texture_setup)
              options.skydome.texture_setup =
                options_base.skydome.texture_setup;
        ...
        if (this.ceil_material_index_default != -1) {
          ...
          if (ceil) {
            ...
            var ceil = (p[this.ceil_material_index_default].lvl[0].list[1] =
              ceil0.clone(
                new THREE.Mesh(ceil0.geometry, ceil0.material.clone())
              ));
            if (MMD_SA.THREEX.enabled) {
...
*/
            ...
            var obj =
              this.grid_blocks.objs[this.grid_blocks.objs.length - 1]._obj;
            var geo = obj.geometry;
            obj.position.copy(ceil.position);
            geo.boundingBox.set(
              MMD_SA._v3a.set(-w * grid_size * 0.5, 0, -h * grid_size * 0.5),
              MMD_SA._v3b.set(w * grid_size * 0.5, 999, h * grid_size * 0.5)
            );
        ...
        var _object_list = this.object_list.map(function (obj) {
          return [obj];
        });
        ...
        d.object_base_list.forEach(function (obj_base, obj_index) {
          if (
            obj_base.parent_object_list &&
            !_object_list.some(
              (obj) =>
                obj_index == obj[0].object_index ||
                (obj_base.id && obj_base.id == obj[0].object_id)
            )
          ) {
            obj_base.parent_object_list.some(function (p) {
              if (p != "PC") {
                ...
                if (
                  !_object_list.some(
                    (obj) =>
                      p_index == obj[0].object_index ||
                      (p_id && p_id == obj[0].object_id)
                  )
                )
                  return false;
              }
        ...
        _object_list.forEach(function (obj_list, i) {
          var obj = obj_list[0];
          var obj_index = (obj.object_index = obj.object_id
            ? d.object_base_index_by_id[obj.object_id]
            : obj.object_index);
          var obj_base = d.object_base_list[obj_index];

          var placement = obj.placement || obj_base.placement || {};
          var clone_list =
            placement.grid_id_filter &&
            placement.grid_id_filter(_object_list, i);
          if (clone_list) {
            clone_list.forEach(function (para, idx) {
              var clone = Object.clone(obj);
              clone._clone_index = idx;
              if (!clone.placement) clone.placement = Object.clone(placement);
        ...
        this.object_id_translated[
          (MMD_SA_options.model_para_obj.character &&
            MMD_SA_options.model_para_obj.character.name) ||
            "PC"
        ] = "PC";
        ...
        this.object_list.forEach(function (obj, idx) {
          ...
          if (obj.character_index) {
            const c =
              MMD_SA_options.model_para_obj_all[obj.character_index].character;
            if (c && c.name) that.object_id_translated[c.name] = id;
          }

          if (obj._clone_index == null) obj._clone_index = -1;
          ...
          obj_mesh =
            ...
            obj._mesh =
            obj._obj_proxy =
              new Object3D_proxy(obj, obj_base.cache);
          if (obj_base.LOD_far)
            obj_LOD_far_mesh =
              obj._obj_LOD_far =
              obj._obj_LOD_far_proxy =
                new Object3D_proxy(obj, obj_base.cache_LOD_far);

          var placement = (obj.placement =
            obj.placement ||
            (obj_base.placement && Object.clone(obj_base.placement)) ||
            {});
          if (!placement.position) placement.position = { x: 0, y: 0, z: 0 };
          ...
          obj_mesh.scale.x =
            ...
            obj.placement.scale =
            obj_base._obj.scale =
              placement.scale || (obj_base.path ? 10 : 1);
          ...
          if (
            (placement.setup && placement.setup(obj)) ||
            placement.grid_pos_absolute
          ) {
            ...
            if (placement.grid_xy) {
              ...
              pos[0] =
                ~~(Math.min(placement.grid_xy[0], 0.9999) * room.width) +
                room.x_min;
              pos[1] =
                ~~(Math.min(placement.grid_xy[1], 0.9999) * room.height) +
                room.y_min;
            } else {
              ...
              if (!grid_list.length) {
                console.error(
                  "object index:" + obj_index + " / gird sorting failed"
                );
                grid_list = d.grid_by_index_free[placement.grid_id];
              }
              pos =
                grid_list[Math.floor(MT.random() * grid_list.length)].slice();
            }
          ...
          if (placement.rotation) {
            ...
            for (var dir in rot) {
              if (rot[dir] == null) rot[dir] = MT.random() * 360;
            }
          ...
          if (grid_occupied) {
            if (rot) {
              var rot_q = MMD_SA.TEMP_q.setFromEuler(
                MMD_SA.TEMP_v3.copy(rot).multiplyScalar(Math.PI / 180)
              );
              grid_occupied = grid_occupied.map(function (xy) {
                if (xy[0] || xy[1]) {
                  MMD_SA.TEMP_v3.set(xy[0], 0, xy[1]).applyQuaternion(rot_q);
                  return [
                    Math.round(MMD_SA.TEMP_v3.x),
                    Math.round(MMD_SA.TEMP_v3.z),
                  ];
                }
                return xy;
              });
            ...
            var x = pos[0];
            var y = pos[1];
            d.grid_by_index_free[placement.grid_id] = d.grid_by_index_free[
              placement.grid_id
            ].filter(function (xy) {
              var occupied = grid_occupied.some(function (xy_mod) {
                ...
                if (hit) d.grid_array_free[xy[1]][xy[0]] = false;
                return hit;
              });
              return !occupied;
            });
          ...
          obj._zone_of_movement =
            obj.zone_of_movement || obj_base.zone_of_movement;
          if (obj._zone_of_movement) {
            ...
            obj.zone_of_movement = new THREE.Box3()
              .copy(obj._zone_of_movement)
              .translate(
                MMD_SA.TEMP_v3.set(
                  (pos[0] + 0.5) * d.grid_size,
                  0,
                  (pos[1] + 0.5) * d.grid_size
                )
              );
          }
          ...
          obj_mesh.position
            ...
            .add(
              MMD_SA.TEMP_v3.set(
                d.grid_size / 2 + placement.position.x,
                placement.position.y,
                d.grid_size / 2 + placement.position.z
              )
            );
          obj_mesh.position.y += placement.grounded
            ? d.get_ground_y(obj_mesh.position)
            : Math.max(d.get_para(~~pos[0], ~~pos[1], true).ground_y || 0, 0);
          ...
          if (obj.onclick) {
            obj.onclick.forEach(function (e) {
              var _clone_index = e.clone_event_by_index
                ? obj_base.object_list.length - 1
                : obj._clone_index;
              if (_clone_index == -1) return;
          ...
          obj.use_PC_ground_normal_when_following =
            obj_base.use_PC_ground_normal_when_following;
          if (obj.mass == null) obj.mass = obj_base.mass;

          if (obj.hp == null) obj.hp = obj_base.hp;
          if (obj.hp_max == null) obj.hp_max = obj_base.hp_max;
          if (obj.hp) {
            if (!obj.hp_max) obj.hp_max = obj.hp;
          } else {
            obj.hp = obj.hp_max;
          }
          if (obj.hp) {
            obj.combat_stats = new CombatStats(
              Object.assign({}, obj_base.combat_stats, obj.combat_stats)
            );
            obj.hp_add = hp_add;
          }
          ...
          obj.collision_by_mesh_material_index_max =
            obj_base.collision_by_mesh_material_index_max;
          obj.collision_by_mesh_enforced = obj_base.collision_by_mesh_enforced;
          obj.collision_by_mesh_face_grounded =
            obj_base.collision_by_mesh_face_grounded;
          obj.collision_by_mesh_sort_range =
            obj_base.collision_by_mesh_sort_range;
          obj.collision_by_mesh_drop_limit =
            obj_base.collision_by_mesh_drop_limit;
          obj.collision_by_mesh_ground_limit =
            obj_base.collision_by_mesh_ground_limit;
          ...
          if (obj.collision_by_mesh) {
            ...
            if (!bb_host._boundingBox)
              bb_host._boundingBox = bb_host.boundingBox.clone();
          ...
          obj.oncollisioncheck =
            obj.oncollisioncheck || obj_base.oncollisioncheck;

          obj.motion = obj.motion || obj_base.motion;
          if (obj.motion) obj.motion = new d.PathMotion(obj);

          obj.animate = obj.animate || obj_base.animate;
          if (
            obj.animate &&
            typeof obj.animate == "string" &&
            obj.animate == "combat_default"
          )
          ...
          obj.matrixAutoUpdate = !!(
            obj.matrixAutoUpdate ||
            obj_base.matrixAutoUpdate ||
            obj.character_index != null ||
            obj.motion ||
            obj.animate
          );
          ...
          obj.user_data =
            obj.user_data ||
            (obj_base.user_data && Object.clone(obj_base.user_data)) ||
            {};
        ...
        for (var index in options._saved.object_by_index) {
          ...
          if (para.rotation && para.rotation.data) {
            obj._obj[obj._obj.useQuaternion ? "quaternion" : "rotation"].copy(
              para.rotation.data
            );
          }
        }
        ...
        window.dispatchEvent(
          new CustomEvent("SA_Dungeon_after_object_placement")
        );
        ...
        if (!pos) {
          ...
          if (options.starting_position_full) {
            ...
            if (options.starting_position_full.grid_id)
              xy =
                this.grid_by_index[options.starting_position_full.grid_id][0];
            ...
            if (xy) {
              ...
              pos.set(
                (xy[0] + 0.5) * this.grid_size,
                ground_y,
                (xy[1] + 0.5) * this.grid_size
              );
            }
            ...
            pos = pos.set(
              (x + 0.5) * this.grid_size,
              Math.max(d.get_para(x, y, true).ground_y || 0, 0),
              (y + 0.5) * this.grid_size
            );
          }
        }
        ...
        if (rot) {
          ...
          c.rot.add(rot);
          THREE.MMD.getModels()[0].mesh.quaternion.multiply(
            MMD_SA.TEMP_q.setFromEuler(rot)
          );
        } else c.pos_update();

        THREE.MMD.getModels()[0].resetMotion();
        MMD_SA_options.motion_shuffle_list_default =
          MMD_SA_options._motion_shuffle_list_default.slice();
        MMD_SA._force_motion_shuffle = true;
        //console.log(MMD_SA_options.motion_shuffle_list_default)
        this.shadow_camera_width =
          options.shadow_camera_width ||
          options_base.shadow_camera_width ||
          64 * 4;
...
*/
          ...
          MMD_SA.THREEX.enabled &&
          MMD_SA.THREEX.get_model(0).type == "VRM" &&
          (MMD_SA.THREEX.get_model(0).is_VRM1
            ? MMD_SA.THREEX.get_model(0).model.meta.thumbnailImage
            : MMD_SA.THREEX.get_model(0).model.meta.texture)
        ) {
          ...
          ctx.drawImage(
            model.is_VRM1
              ? model.model.meta.thumbnailImage
              : model.model.meta.texture.source.data,
            0,
            0,
            64,
            64
          ...
          const icon_canvas =
            (para_SA._icon_canvas =
            para_SAX._icon_canvas =
          ...
          System._browser.on_animation_update.add(
            function () {
              ...
              dir_light.position
                .set(1, 1, 1)
                .multiplyScalar(MMD_SA_options.light_position_scale)
                .add(c.pos);
              ...
              var head_pos = MMD_SA._v3a
                .copy(head_pos_absolute)
                .sub(mesh.position);
              ...
              camera_obj.position.set(
                head_pos_absolute.x,
                head_pos_absolute.y - 0.25,
                head_pos_absolute.z + 4 + 12
              );
          ...
          System._browser.on_animation_update.add(
            function () {
              ...
              (function () {
                ...
                while (y--) {
                  ...
                  while (x--) {
                    ...
                    dstBuff[dstOff + 1] =
                      g * mix + srcBuff[dstOff + 1] * (1 - mix);
                    dstBuff[dstOff + 2] =
                      b * mix + srcBuff[dstOff + 2] * (1 - mix);
              ...
              MMD_SA_options.model_para_obj._icon_canvas =
                document.createElement("canvas");
              MMD_SA_options.model_para_obj._icon_canvas.width =
                MMD_SA_options.model_para_obj._icon_canvas.height = 64;
              MMD_SA_options.model_para_obj._icon_canvas
                .getContext("2d")
              ...
              if (point_light) {
                point_light.color = new THREE.Color(
                  (options.point_light && options.point_light.color) ||
                    "#ffffff"
                );
              }
    ...
    update_shadow_para: function () {
      ...
      MMD_SA_options.shadow_para.shadowCameraFar =
        MMD_SA_options.light_position_scale * 3; //shadow_camera_width*1.5;//
    ...
    init: function () {
                ...
                UI_and_overlays: {
                  user_interface: {
                    ...
                    UI_off: {
                      _translation_: {
                        _default_:
                          "User interface is now OFF. Press Esc to toggle the bottom menu display.",
      ...
      if (!options) options = MMD_SA_options.Dungeon_options = {};

      if (
        use_SA_browser_mode &&
        !is_SA_child_animation &&
        (!webkit_electron_mode || !options.transparent_background)
      ) {
      ...
      document.addEventListener("DOMContentLoaded", function (e) {
        ...
        for (var item_name in d.item_base) {
          ...
          if (item.sound) {
            item.sound.forEach(function (sound) {
              if (
                options.sound.findIndex(function (s) {
                  return s.url == sound.url;
                }) == -1
              ) {
                options.sound.push({
                  url: sound.url,
        ...
        if (options.NPC_physics_disabled) {
          for (
            var i = 1, i_max = MMD_SA_options.model_para_obj_all.length;
            i < i_max;
            i++
          )
      ...
      window.addEventListener("jThree_ready", function () {
        var c = (MMD_SA_options.Dungeon.character.icon =
          document.createElement("canvas"));
        c.width = c.height = 64;
        ...
        MMD_SA_options.model_para_obj_all.forEach(function (para_SA, idx) {
          if (para_SA.is_PC_candidate || idx == 0) {
            ...
            if (!para_SA.character) para_SA.character = {};
            if (!para_SA.character.combat_stats_base)
              para_SA.character.combat_stats_base =
                idx == 0
                  ? MMD_SA_options.Dungeon.character.combat_stats_base || {}
                  : {};
            if (!para_SA.character.combat_stats_base.attack_combo_list)
              para_SA.character.combat_stats_base.attack_combo_list =
                MMD_SA_options.Dungeon_options._attack_combo_list;
          }

          const para_SAX = MMD_SA.THREEX.get_model(idx).model_para;
          if (!para_SAX.icon_path) return;
          //System.Gadget.path + '\\icon_SA_512x512.png'

          if (!/^\w+\:/.test(para_SAX.icon_path))
            para_SAX.icon_path =
              MMD_SA.THREEX.get_model(idx).model_path.replace(/[^\/\\]+$/, "") +
              para_SAX.icon_path;
        ...
        MMD_SA_options.Dungeon.character.combat_stats_base =
          MMD_SA_options.model_para_obj.character.combat_stats_base;
      });

      window.addEventListener("SA_Dungeon_onstart", function () {
        //https://github.com/yoannmoinet/nipplejs
        if (Ljoystick.style.visibility == "hidden") return;
        ...
        (function () {
          ...
          function keyboard_event(e_type, key_id) {
            ...
            if (e_type == "keyup" && !d._key_pressed[keyCode]) return;

            var e = new KeyboardEvent(e_type, {
              bubbles: true,
              cancelable: true,
              keyCode: keyCode,
            });
          ...
          function joystick_resize() {
            ...
            Ljoystick.style.posTop =
              B_content_height - (Ljoystick.style.pixelWidth + 32);
            //Ljoystick.style.transform = "scale(" + css_scale + ")"
            Ljoystick.style.visibility = "inherit";
          }

          Ljoystick.style.pixelWidth = Ljoystick.style.pixelHeight =
            256 * css_scale;
          ...
          d.nipplejs_manager.on("move", function (ev, data) {
            ...
            if (dis > nipple_radius / 10) {
              ...
              if (x_abs > threshold) {
                scale = d.character.TPS_mode
                  ? (x_abs / dis) * Math.min((dis / nipple_radius) * 2, 1)
                  : (x_abs / nipple_radius - 0.1) / 0.9;
                if (x > 0) key_pressed.right = scale;
                else key_pressed.left = scale;
              }
              if (y_abs > threshold) {
                scale = d.character.TPS_mode
                  ? (y_abs / dis) * Math.min((dis / nipple_radius) * 2, 1)
                  : Math.min((y_abs / nipple_radius) * 2, 1);
                if (y > 0) key_pressed.down = scale;
                else key_pressed.up = scale;
              }
            }
            ...
            for (var key_id in key_pressed) {
              ...
              } else keyboard_event("keyup", key_id);
            }
          });

          d.nipplejs_manager.on("end", function (ev) {
            var key_pressed = { up: 0, down: 0, left: 0, right: 0 };
            for (var key_id in key_pressed) keyboard_event("keyup", key_id);
          });
      ...
      if (!MMD_SA_options.GOML_scene) MMD_SA_options.GOML_scene = "";
      if (!MMD_SA_options.mesh_obj) MMD_SA_options.mesh_obj = [];
      ...
      if (!MMD_SA_options.shadow_para) MMD_SA_options.shadow_para = {};
      ...
      if (
        !MMD_SA_options.fog &&
        (!MMD_SA_options.WebXR || !MMD_SA_options.WebXR.AR)
      )
      ...
      if (!options.game_version) options.game_version = "1.0";
      if (!options.chapter_id) options.chapter_id = "1";

      if (!options.sound) options.sound = [];

      if (
        !options.sound.some(function (s) {
          return s.name == "interface_item_access";
        })
      ) {
        options.sound.push({
          url:
            System.Gadget.path +
            "/sound/SFX_pack01.zip#/RPG Sound Pack/interface/interface1.aac",
      ...
      if (
        !options.sound.some(function (s) {
          return s.name == "interface_item_deny";
        })
      ) {
        options.sound.push({
          url:
            System.Gadget.path +
            "/sound/SFX_pack01.zip#/RPG Sound Pack/interface/interface6.aac",
      ...
      if (
        !options.sound.some(function (s) {
          return s.name == "interface_item_drop";
        })
      ) {
        options.sound.push({
          url:
            System.Gadget.path +
            "/sound/SFX_pack01.zip#/RPG Sound Pack/interface/interface3.aac",
      ...
      if (
        !options.sound.some(function (s) {
          return s.name == "footstep_default";
        })
      ) {
        options.sound.push({
          url:
            System.Gadget.path +
            "/sound/SFX_pack01.zip#/251788__vkproduktion__footstep-01-mono.aac",
      ...
      if (
        !options.sound.some(function (s) {
          return s.name == "footstep_water";
        })
      ) {
        options.sound.push({
          url:
            System.Gadget.path +
            "/sound/SFX_pack01.zip#/270429__littlerobotsoundfactory__footstep-water-03.aac",
          name: "footstep_water",
          channel: "SFX",
          can_spawn: true,
        });
      ...
      if (
        !options.sound.some(function (s) {
          return s.name == "footstep_grass";
        })
      ) {
        options.sound.push({
          url:
            System.Gadget.path +
            "/sound/SFX_pack01.zip#/456273__soundfx-studio__footsteps-grass.aac",
          name: "footstep_grass",
          channel: "SFX",
          can_spawn: true,
        });
      ...
      if (
        !options.sound.some(function (s) {
          return s.name == "footstep_sand";
        })
      ) {
        options.sound.push({
          url:
            System.Gadget.path +
            "/sound/SFX_pack01.zip#/456273__soundfx-studio__footsteps-sand.aac",
          name: "footstep_sand",
          channel: "SFX",
          can_spawn: true,
        });
      ...
      if (
        !options.sound.some(function (s) {
          return s.name == "car_engine01";
        })
      ) {
        options.sound.push({
          url:
            System.Gadget.path +
            "/sound/SFX_pack01.zip#/car_engine_loop_5x5.aac",
          name: "car_engine01",
          channel: "SFX",
          can_spawn: true,
          loop: true,
      ...
      if (options.use_PC_click_reaction_default) {
        if (
          ...
          options.sound.push({
            url:
              System.Gadget.path +
              "/sound/SFX_pack01.zip#/162370__lewisisminted__punch-1.aac",
            name: "hit-1",
            channel: "SFX",
            can_spawn: true,
          });
        ...
        if (
          ...
          options.sound.push({
            url:
              System.Gadget.path +
              "/sound/SFX_pack01.zip#/104183__ekokubza123__punch.aac",
            name: "hit-3",
            channel: "SFX",
            can_spawn: true,
          });
        ...
        if (
          !options.sound.some(function (s) {
            return s.name == "anime_wow";
          })
        ) {
          options.sound.push({
            url:
              System.Gadget.path + "/sound/SFX_pack01.zip#/Anime_Wow_Sound.oga",
            name: "anime_wow",
            channel: "SFX",
            //     ,can_spawn: true
          });
        ...
        MMD_SA_options.custom_default = function () {
          ...
          if (
            ...
            MMD_SA_options.custom_action.indexOf("cover_undies") == -1
          )
            custom_action_new.push("cover_undies");
          ...
          var _hit_head = {
            ...
            motion: {
              path: "MMD.js/motion/motion_rpg_pack01.zip#/hit/h01_何かにぶつかる小.vmd",
              match: {
                skin_jThree: {
                  test: function (name) {
                    return !(
            ...
            animation_check:
              MMD_SA.custom_action_default["cover_undies"].animation_check,
          };
          ...
          var _cover_chest = {
              ...
              condition: function (is_bone_action, objs) {
                ...
                if (MMD_SA._hit_chest_ || this._cover_chest_) {
                  if (is_bone_action && !this.frame) {
                    MMD_SA.copy_first_bone_frame(this.motion_index, objs, {
                      bone_group: ["腕"],
                      skin_jThree:
                        /^(\u5DE6|\u53F3)(\u80A9|\u8155|\u3072\u3058|\u624B\u9996|\u624B\u6369)/,
                    });
            ...
            motion: {
              path: "MMD.js/motion/motion_basic_pack01.zip#/cover_chest_v02b.vmd",
              match: {
                bone_group: ["腕", "指"],
                all_morphs: true,
                skin_jThree:
          ...
          MMD_SA_options.custom_action = MMD_SA_options.custom_action
            ? custom_action_new.concat(MMD_SA_options.custom_action)
            : custom_action_new;
          ...
          Object.defineProperty(MMD_SA, "_hit_body_but_chest_", {
            get: function () {
              return MMD_SA._hit_head_ || MMD_SA._hit_hip_ || MMD_SA._hit_legs_;
            },
          });
          Object.defineProperty(MMD_SA, "_hit_body_but_hip_", {
            get: function () {
              return (
                MMD_SA._hit_head_ || MMD_SA._hit_chest_ || MMD_SA._hit_legs_
              );
            },
          });
          ...
          window.addEventListener("SA_Dungeon_character_clicked", function (e) {
            if (MMD_SA.music_mode || System._browser.camera.ML_enabled)
              return false;
            ...
            for (var i = 0, i_max = d.key_map_list.length; i < i_max; i++) {
              ...
              var id = k.id;
              if (key_map.down && !/^(up|left|down|right)$/.test(id)) return;

              if (key_map.down) {
                pressed = true;
                _key_pressed_when_character_clicked[k.keyCode] = true;
                if (/^(up|down)$/.test(id)) moving = true;
              }
            }
            ...
            if (intersected.y > bb_max_y * 0.8) {
              ...
              if (!pressed) {
                ...
                MMD_SA_options._motion_shuffle_list = [
                  MMD_SA_options.motion_index_by_name["w01_すべって尻もち"],
                ...
                // use ._motion_shuffle_list instead, because we have multiple motions running in order, but .motion_shuffle_list_default can be shuffled.
                MMD_SA_options._motion_shuffle_list = [
                  MMD_SA_options.motion_index_by_name["r01_普通に転ぶ"],
                  MMD_SA_options.motion_index_by_name["OTL→立ち上がり"],
                ];
                MMD_SA_options.motion_shuffle_list_default = null;
                MMD_SA._force_motion_shuffle = true;
              } else {
            ...
            if (_cursor_timerID) clearTimeout(_cursor_timerID);
            e.detail.target.style.cursor =
              (webkit_mode ? "-webkit-" : "") + "grab";
            _cursor_timerID = setTimeout(function () {
              e.detail.target.style.cursor =
                (webkit_mode ? "-webkit-" : "") + "grabbing";
              _cursor_timerID = setTimeout(function () {
      ...
      if (options.combat_mode_enabled) {
        ...
        if (
          ...
          options.sound.push({
            url:
              System.Gadget.path +
              "/sound/SFX_pack01.zip#/216198__rsilveira-88__cartoon-punch-02.aac",
            name: "hit-2",
            channel: "SFX",
            can_spawn: true,
          });
        ...
        if (
          ...
          options.sound.push({
            url:
              System.Gadget.path +
              "/sound/SFX_pack01.zip#/RPG Sound Pack/battle/swing.aac",
            name: "swing",
            channel: "SFX",
            can_spawn: true,
          });
        ...
        if (
          !options.sound.some(function (s) {
            return s.name == "hit_slash";
          })
        ) {
          options.sound.push({
            url:
              System.Gadget.path +
              "/sound/SFX_pack01.zip#/35213__abyssmal__slashkut.aac",
            name: "hit_slash",
            channel: "SFX",
            can_spawn: true,
          });
      ...
      if (sd) {
        if (!sd.texture_path_list)
          sd.texture_path_list = [
            System.Gadget.path +
              "/images/_dungeon/tex/ryntaro_nukata/angel_staircase.jpg",
          ];
        ...
        if (!sd.width_segments) sd.width_segments = 64 * 2;
        if (!sd.height_segments) sd.height_segments = 64 * 2;
        ...
        if (!sd.texture_setup) {
          sd.texture_setup = (function () {
            ...
            return function () {
              ...
              var dome_tex =
                MMD_SA.THREEX.mesh_obj.get_three("DomeMESH").material.map;
              if (MMD_SA.THREEX.enabled && MMD_SA.THREEX.use_sRGBEncoding)
                dome_tex.colorSpace = THREE.SRGBColorSpace;
              dome_tex.needsUpdate = true;

              var img =
                MMD_SA_options.Dungeon_options.skydome.texture_cache_list[
              ...
              var fill_color =
                fog.color ||
                (MMD_SA_options.Dungeon.fog &&
                  MMD_SA_options.Dungeon.fog.color) ||
                "#000000";
              ...
              gradient.addColorStop(
                0,
                "rgba(" + hexToRgb(fill_color).join(",") + ",0)"
              );
        ...
        MMD_SA_options.mesh_obj_preload_list.push({
          id: "DomeMESH",
          create: function () {
            const THREE = MMD_SA.THREEX.THREE;
        ...
        window.addEventListener("MMDStarted", function () {
          MMD_SA_options.mesh_obj_by_id["DomeMESH"]._obj.renderDepth = 99999;
        });

        if (!options.PC_follower_list) options.PC_follower_list = [];
...
*/
      ...
      this.grid_material_list = options.grid_material_list || [
        // ceil
        {
          map:
            System.Gadget.path +
            "\\images\\_dungeon\\tex\\3dtextures.me\\Stone Wall 002\\Stone_Wall_002_COLOR_AO.jpg",
          normalMap:
            System.Gadget.path +
            "\\images\\_dungeon\\tex\\3dtextures.me\\Stone Wall 002\\Stone_Wall_002_NRM.jpg",
          geo_by_lvl: [[1, 1]],
          distance_by_lvl: [],
        },
        // floor
        {
          map:
            System.Gadget.path +
            "/images/_dungeon/tex/3dtextures.me/Stone Floor 003/Pavement_006_COLOR_AO-50.jpg",
          normalMap:
            System.Gadget.path +
            "/images/_dungeon/tex/3dtextures.me/Stone Floor 003/Pavement_006_NRM_c80.jpg",
          specularMap:
            System.Gadget.path +
            "/images/_dungeon/tex/3dtextures.me/Stone Floor 003/Pavement_006_SPEC_c80.jpg",
        ...
        // wall
        {
          map:
            System.Gadget.path +
            "/images/_dungeon/tex/3dtextures.me/Stone Wall 004/Wall Stone 004_COLOR_AO.jpg",
          normalMap:
            System.Gadget.path +
            "/images/_dungeon/tex/3dtextures.me/Stone Wall 004/Wall Stone 004_NRM_c80.jpg",
          specularMap:
            System.Gadget.path +
            "/images/_dungeon/tex/3dtextures.me/Stone Wall 004/Wall Stone 004_SPEC_c90.jpg",
      ...
      this.item_base._empty_ = Object.assign(
        {
          icon_path:
            System.Gadget.path + "/images/_dungeon/item_icon.zip#/empty.gif",
          rarity: "inactive",
          info_short: "Empty",
        },
      ...
      this.item_base._backpack_ = Object.assign(
        {
          icon_path:
            System.Gadget.path +
            "/images/_dungeon/item_icon.zip#/fantasy_icon/backpack_64x64.png",
          get info_short() {
            return System._browser.translation.get("Dungeon.UI.backpack");
          },
          ...
          sound: [
            {
              url:
                System.Gadget.path +
                "/sound/SFX_pack01.zip#/RPG Sound Pack/interface/interface2.aac",
              name: "item_backpack",
              is_drag: true,
            },
          ],
            ...
            func: function () {
              if (System._browser.overlay_mode) {
                if (Ldungeon_inventory.style.visibility == "hidden")
                  ...
                  Ldungeon_inventory_backpack.style.visibility != "hidden"
                )
                  Ldungeon_inventory.style.visibility =
                    Ldungeon_inventory_backpack.style.visibility = "hidden";
                else Ldungeon_inventory_backpack.style.visibility = "inherit";
              } else {
                Ldungeon_inventory_backpack.style.visibility =
                  Ldungeon_inventory_backpack.style.visibility != "hidden"
                    ? "hidden"
                    : "inherit";
              }
              ...
              if (MMD_SA_options.Dungeon.nipplejs_manager)
                Ljoystick.style.visibility =
                  Ldungeon_inventory_backpack.style.visibility != "hidden" ||
                  options.joystick_disabled
                    ? "hidden"
                    : "inherit";
            },
            anytime: true,
      ...
      const Bag = (() => {
        function clone(inv) {
          ...
          if (!_inv) {
            inventory.copy(
              inv.index,
              inventory.max_base +
                page_index * inventory.max_base * (inventory.max_row - 1)
            );
          }
        }
        ...
        function action(item, inv) {
          ...
          if (inventory.page_index != page_index) {
            ...
            MMD_SA_options.Dungeon.inventory.update_page(
              Math.max(this._page_index_, 0)
            );
          }
        }
        ...
        const Bag = function (page_index) {
          ...
          this.icon_path =
            System.Gadget.path +
            "/images/_dungeon/item_icon.zip#/misc_icon/bag_64x64.png";
          this.info_short = "Bag";
          ...
          this.action = {
            func: (item, inv) => {
              return action.call(this, item, inv);
            },
            anytime: true,
          };
        };
        ...
        Bag.prototype.on_drop = function (item, inv) {
          ...
          inventory.unshift(
            inv.index,
            inventory.page_index != page_index ? page_index : this._page_index_
          );
          /*
System._browser.on_animation_update.add(()=>{
  item.update_UI();
...
*/
      ...
      this.item_base.bag02 = Object.assign(
        new Bag(2),
        this.item_base.bag02 || {}
      );
      ...
      this.item_base._map_ = Object.assign(
        {
          icon_path:
            System.Gadget.path +
            "/images/_dungeon/item_icon.zip#/fantasy_icon/map_64x64.png",
          info_short: "Map",
          index_default: MMD_SA_options.Dungeon.inventory.max_base - 2,
          stock_max: 1,
          action: {
            func: function () {
              Ldungeon_map.style.visibility =
                Ldungeon_map.style.visibility != "hidden"
                  ? "hidden"
                  : "inherit";
            },
            anytime: true,
      ...
      this.item_base.menu = Object.assign(
        {
          icon_path:
            System.Gadget.path +
            "/images/_dungeon/item_icon.zip#/fantasy_icon/tome_64x64.png",
          ...
          sound: [
            {
              url:
                System.Gadget.path +
                "/sound/SFX_pack01.zip#/RPGsounds_Kenney/bookOpen.ogg",
              name: "item_book_open",
              is_drag: true,
            },
          ],
          action: {
            func: function () {
              MMD_SA_options.Dungeon.run_event("_MENU_", 0);
            },
            // ,anytime: true
          },
        },
        ...
        [
          {
            message: {
              content:
                "This will restart the game, preserving all procedurally generated maps and content.\n1. OK\n2. Cancel",
              bubble_index: 3,
              para: { scale: 1.5, font_scale: 1 },
              branch_list: [{ key: 1, branch_index: 3 }, { key: 2 }],
            },
          },
        ],
        //3
        ...
        [
          {
            message: {
              content:
                "This will restart the game, resetting all procedurally generated maps and content.\n1. OK\n2. Cancel",
              bubble_index: 3,
              para: { scale: 1.5, font_scale: 1 },
              branch_list: [{ key: 1, branch_index: 5 }, { key: 2 }],
            },
          },
        ],
        //5
      ...
      if (!this.item_base.coin) {
        this.item_base.coin = {
          icon_path:
            System.Gadget.path +
            "/images/_dungeon/item_icon.zip#/fantasy_icon/coin_64x64.png",
          ...
          sound: [
            {
              url:
                System.Gadget.path +
                "/sound/SFX_pack01.zip#/RPG Sound Pack/inventory/coin3.aac",
              name: "item_coin",
              is_drag: true,
              is_no_action: true,
            },
      ...
      if (!this.item_base.potion_hp_50) {
        this.item_base.potion_hp_50 = {
          icon_path:
            System.Gadget.path +
            "/images/_dungeon/item_icon.zip#/potions/pt1_64x64.png",
          ...
          sound: [
            {
              url:
                System.Gadget.path +
                "/sound/SFX_pack01.zip#/RPG Sound Pack/inventory/bubble2.aac",
              name: "item_potion_drink01",
            },
            {
              url:
                System.Gadget.path +
                "/sound/SFX_pack01.zip#/RPG Sound Pack/inventory/bottle.aac",
              name: "item_potion01",
              is_drag: true,
            },
          ],
      ...
      [
        System.Gadget.path +
          "/images/_dungeon/item_icon.zip#/inventory/BlankSlot.png",
      ...
      window.addEventListener(
        "SA_MMD_trackball_camera_limit_adjust",
        (function () {
          var para = {
            filter: function (obj) {
          ...
          return function (e) {
            ...
            var d = MMD_SA_options.Dungeon;
            var camera_limit_scale =
              MMD_SA_options.Dungeon_options.options_by_area_id[d.area_id]
                .camera_limit_scale || 1;

            // feel less glitchy than using MMD_SA.camera_position
            var camera_pos = MMD_SA._v3a_.copy(d.character.pos);
            camera_pos.y += 10; //MMD_SA.camera_position;
            var rv =
              (d.ceil_material_index_default != -1 &&
                eye.y > d.ceil_height * camera_limit_scale * 0.8) ||
              d.check_grid_blocking(
                MMD_SA.TEMP_v3.copy(eye).add(d.character.pos),
                d.grid_blocking_camera_offset
              ) ||
              (!d.no_camera_collision &&
                d.check_ray_intersection(camera_pos, eye, para));
            if (rv) {
        ...
        (function () {
          ...
          ss.sheet.insertRule(
            [
              ...
              "transform: scale(" +
                MMD_SA_options.Dungeon.inventory.UI.info.scale +
                ");",
              "}",
            ].join("\n"),
            0
          ...
          _s.transform =
            "scale(" + MMD_SA_options.Dungeon.inventory.UI.info.scale + ")";
          ...
          var drop_item = function (index_source, index) {
            ...
            if (
              inv_target.item.is_base_inventory &&
              index_source >= inv.max_base
            ) {
              if (!MMD_SA_options.Dungeon.inventory.UI.muted)
                MMD_SA_options.Dungeon.sound.audio_object_by_name[
                  "interface_item_deny"
            ...
            if (!MMD_SA_options.Dungeon.inventory.UI.muted)
              MMD_SA_options.Dungeon.sound.audio_object_by_name[
                (
                  (inv_source.item.sound &&
                    inv_source.item.sound.find(function (i) {
                      return i.is_drag;
                    })) || { name: "interface_item_drop" }
                ).name
              ].play();
          ...
          for (let r = 0, r_max = inv.max_row; r < r_max; r++) {
            for (let i = 0, i_max = inv.max_base; i < i_max; i++) {
              ...
              ds.backgroundImage =
                "url(" +
                MMD_SA_options.Dungeon.blob_url.get("BlankSlot.png") +
                ")";
              ...
              d.addEventListener(
                ...
                function (e) {
                  ...
                  inv_item.item.onmouseover &&
                    inv_item.item.onmouseover(e, _idx);

                  // not accessing .info directly as it may be a getter function
                  if ("info" in inv_item.item) {
                    this.className = System._browser.overlay_mode
                      ? "Dungeon_inventory_item_info_short"
                      : "Dungeon_inventory_item_info";
                    this.setAttribute(
                      "data-info",
                      inv_item.item.info_short + ":\n" + inv_item.item.info
                    );
                  }
                },
                true
...
*/
              ...
              d.addEventListener(
                ...
                async function (e) {
                  ...
                  if (
                    !MMD_SA_options.Dungeon.inventory.UI.muted &&
                    !inv_item.item.action.muted
                  )
                    MMD_SA_options.Dungeon.sound.audio_object_by_name[
                      (
                        (inv_item.item.sound && inv_item.item.sound[0]) || {
                          name: "interface_item_access",
                        }
                      ).name
                    ].play();
                  ...
                  if (inv_item.item.stock_max != 1) {
                    if (--inv_item.stock == 0) inv_item.clear();
                    else
                      document.getElementById(
                        "Ldungeon_inventory_item" + idx + "_stock"
                      ).textContent = inv_item.stock;
                  }
                },
                true
              );
...
*/
              d.addEventListener(
                ...
                function (e) {
                  const _idx = inv.get_inventory_index(idx);
                  if (!inv.list[_idx].item_id) return;
                  e.stopPropagation();
              ...
              if (r) Ldungeon_inventory_backpack.appendChild(d);
              else Ldungeon_inventory.appendChild(d);
          ...
          var draw_dungeon_map = function (e) {
            ...
            var scale =
              Math.min(B_content_width, B_content_height) /
              (Math.max(d.RDG_options.width, d.RDG_options.height) * 2);
            if (scale >= 8) scale = 8;
            ...
            if (d.map_display_scale != scale) {
              ...
              context.clearRect(
                0,
                0,
                Cdungeon_map_canvas.width,
                Cdungeon_map_canvas.height
              );
              for (var y = 0, y_max = d.RDG_options.height; y < y_max; y++) {
                for (var x = 0, x_max = d.RDG_options.width; x < x_max; x++) {
                  if (d.get_para(x, y, true).hidden_on_map) {
                    ...
                    context.fillStyle =
                      d.get_para(x, y, true).map_grid_color || "green";
            ...
            Ldungeon_map_spot.style.pixelWidth =
              Ldungeon_map_spot.style.pixelHeight = spot_size;
            if (MMD_SA_options.Dungeon_options.multiplayer) {
              for (
                var i = 1,
                  i_max =
          ...
          MMD_SA_options.Dungeon.update_status_bar = (function () {
            ...
            var update_status_bar = function (always_update) {
              ...
              if (!always_update && hp_width == _hp_width) return;
              hp_width = _hp_width;
              ...
              if (c.hp > 75) {
                ...
                const g =
                  c.hp > 50 ? 255 : Math.round(64 + (c.hp / 50) * (255 - 64));
          ...
          var place_inventory = function (e) {
            Ldungeon_inventory.style.posLeft =
              Ldungeon_inventory_backpack.style.posLeft =
                (B_content_width - inv.max_base * 64) *
                (System._browser.overlay_mode &&
                Ldungeon_inventory.style.visibility == "hidden"
                  ? 1
                  : 0.5);
            ...
            Ldungeon_inventory_backpack.style.posTop =
              B_content_height - inv.max_row * 64 - 4;
          ...
          var draw_UI = function (e) {
            ...
            place_inventory(e);
            MMD_SA_options.Dungeon.update_status_bar(true);
            Ldungeon_map.style.transform =
              Ldungeon_inventory.style.transform =
              Ldungeon_inventory_backpack.style.transform =
              Cdungeon_status_bar.style.transform =
                "scale(" + System._browser.css_scale + ")";
          };

          window.addEventListener(
            "SA_Dungeon_after_map_generation",
            function (e) {
              draw_UI();
            }
          );
          window.addEventListener("SA_resize", function (e) {
            draw_UI();
            if (MMD_SA_options.Dungeon.started)
              MMD_SA_options.Dungeon.update_dungeon_blocks(true);
          });
        })();
        // UI END
          ...
          Motion.prototype = {
            ...
            play: (function () {
              ...
              return function (t) {
                ...
                if (!this.t_ini) {
                  this.t_ini = t;
                  if (pt_ini.pos) this.pos_last.copy(pt_ini.pos);
                  if (pt_ini.rot) this.rot_last.copy(pt_ini.rot);
                  return;
                }

                if (this.ending_delay) {
                  if ((t - this.t_ini) / 1000 < this.ending_delay) return;
                } else {
                  var mesh_obj = this.obj._obj;
                  // the first frame of a new point
                  if (!this.path_alpha) {
                    if (!this.para.is_relative_path) {
                      if (pt_ini.pos) mesh_obj.position.copy(pt_ini.pos);
                      if (pt_ini.rot) {
                  ...
                  this.path_alpha = Math.min(
                    (t - this.t_ini) / 1000 / pt_ini.duration,
                    1
                  );
                  if (!pt_ini.animate || !pt_ini.animate(this)) {
                    this.path_alpha_position = this.path_alpha_rotation =
                      this.path_alpha;
                  }
                  ...
                  if (pt_ini.pos) {
                    _v3
                      .copy(pt_ini.pos)
                      .lerp(pt_end.pos, this.path_alpha_position);
                  ...
                  if (pt_ini.rot) {
                    _v3
                      .copy(pt_ini.rot)
                      .lerp(pt_end.rot, this.path_alpha_rotation);
                    this.rot_delta
                      .copy(_v3)
                      .sub(this.rot_last)
                      .multiplyScalar(Math.PI / 180);
                  ...
                  if (this.obj.is_PC) {
                    THREE.MMD.getModels()[0].mesh.quaternion.setFromEuler(
                      mesh_obj.rotation
                    );
                    c.about_turn = false;
                    c.pos_update();
                    ...
                    this.para.mounted_list.forEach(function (mounted) {
                      if (mounted == "PC") {
                        ...
                        if (mounted._obj.useQuaternion)
                          mounted._obj.quaternion.multiply(
                            _q.setFromEuler(that.rot_delta)
                          );
                      }
                    });
                  }
        ...
        window.addEventListener("SA_camera_adjust", function (e) {
          if (
            MMD_SA_options.Dungeon.check_grid_blocking(
              MMD_SA.TEMP_v3.copy(e.detail.pos_v3).add(
                MMD_SA_options.Dungeon.character.pos
              ),
              MMD_SA_options.Dungeon.grid_blocking_camera_offset
            )
          ) {
          ...
          return function (
            ...
            if (subject_is_PC) {
              ...
              if (para.bb_translate && para.bb_translate._default) {
                ...
                bb_translate_offset
                  .copy(para.bb_translate._default)
                  .sub(para.bb_translate);
              }
            ...
            if (para.bb_translate) {
              bb_translate = _v3a
                .set(subject_bs.radius, subject_bs.radius, subject_bs.radius)
                .multiply(para.bb_translate)
                .applyQuaternion(center_rotate(subject.quaternion));
            ...
            if (para.bb_expand) {
              subject_bs.radius *=
                Math.max(para.bb_expand.x, para.bb_expand.y, para.bb_expand.z) +
                1;
            }
            ...
            moved_final
              .copy(mov_delta)
              .applyQuaternion(center_rotate(subject.quaternion, true));
...
*/
            ...
            if (para.bb_translate) {
              bb_translate = _v3a
                ...
                .applyQuaternion(center_rotate(subject.quaternion));
              bb_translate.add(subject.position);
            } else bb_translate = subject.position;
            if (para.bb_expand) {
              subject_bb.expandByVector(
                _v3b.copy(_v3).multiply(para.bb_expand).multiplyScalar(0.5)
              );
            ...
            if (use_bb_translate_offset) {
              if (para.bb_expand)
                bb_translate_offset.multiply(
                  _v3a.copy(para.bb_expand).addScalar(1)
                );
              bb_translate_offset
                .multiply(_v3)
                .applyQuaternion(center_rotate(subject.quaternion));
            ...
            subject_bb.applyMatrix4(
              _m4.makeRotationFromQuaternion(center_rotate(subject.quaternion))
            ); //subject.matrixWorld)//
            ...
            ray_normal.set(
              _v3.copy(s_bb).add(_v3a.copy(d).multiplyScalar(-999)),
              d
            );
            ...
            if (!obj_list || para.check_grid_blocks) {
              obj_list =
                (obj_list && obj_list.concat(this.grid_blocks.objs)) ||
                this.object_list_in_view;
            ...
            obj_list.forEach(function (obj, idx) {
              ...
              if (
                para.collision_by_mesh_disabled &&
                obj.collision_by_mesh_enforced
              )
                return;
              ...
              if (obj.oncollisioncheck && obj.oncollisioncheck(subject)) return;

              var obj_base =
                obj._obj_base ||
                that.object_base_list[obj.object_index] ||
                obj_base_dummy;

              object_bs.copy(obj._obj_proxy.boundingSphere);
              object_bs.center.add(cache.position);

              var object_scale = Math.max(
                cache.scale.x,
                cache.scale.y,
                cache.scale.z
              );
              object_bs.radius *=
                object_scale *
                ((obj_base.construction &&
                  obj_base.construction.boundingSphere_radius_scale) ||
                  1); //((obj_base.character_index != null) ? 0.5 : 1))
              ...
              if (
                ...
                moved_dis_max +
                  r +
                  ((that.use_local_mesh_sorting &&
                    local_mesh_sorting_range_buffer) ||
                    Math.min((obj.collision_by_mesh_sort_range || 64) * 0.5, r))
              ) {
                ...
                if (
                  ...
                  _v3
                    .copy(s_bb)
                    .applyMatrix4(_m4.getInverse(cache.matrixWorld));
                  let _index =
                    _subject.obj._index != null ? _subject.obj._index + 1 : 0;
                  let mesh_sorted =
                    (obj.mesh_sorted =
                    obj.mesh_sorted_list[_index] =
                      obj.mesh_sorted_list[_index] || {});
                  if (that.use_local_mesh_sorting) {
                    _dis = mesh_sorted.position
                      ? Math.sqrt(
                          Math.pow(mesh_sorted.position.x - _v3.x, 2) +
                            Math.pow(mesh_sorted.position.z - _v3.z, 2)
                        ) +
                        mov_delta_length / object_scale
                      : 9999;
                    if (_dis > local_mesh_sorting_range_buffer / object_scale) {
                      _collision_by_mesh_sort_range =
                        Math.max(
                          Math.min(
                            Math.ceil(
                              subject_bs.radius ||
                                10 +
                                  Math.sqrt(
                                    moved_final.x * moved_final.x +
                                      moved_final.z * moved_final.z
                                  )
                            ),
                            obj.collision_by_mesh_sort_range
                          ) + local_mesh_sorting_range_buffer,
                      ...
                      if (that.use_octree) {
                        ...
                        mesh_sorted.index_list = that.mesh_sorting_worker.tree[
                          obj.object_index
                        ]
                          .search({
              ...
              if (obj._obj_proxy.boundingBox) {
                ...
                let hit_moved_once;
                let collision_by_mesh =
                  !para.collision_by_mesh_disabled && obj.collision_by_mesh;

                let skip_bb_index_list = [];
                obj._obj_proxy.boundingBox_list.some(function (bb, bb_idx) {
                  if (skip_bb_index_list.indexOf(bb_idx) != -1) return;

                  if (para.filter_obj && !para.filter_obj(obj, bb, true)) {
                    return;
                  }

                  var _skip_ground_obj_check =
                    skip_ground_obj_check || obj.skip_ground_obj_check;
                  ...
                  if (!para.collision_centered) {
                    ...
                    object_bb
                      .copy(bb)
                      .applyMatrix4(
                        _m4.makeRotationFromQuaternion(
                          _q2.copy(cache.quaternion).multiply(_q)
                        )
                      )
                      .translate(
                        _v3a
                          .copy(subject.position)
                          .add(
                            _v3
                              .copy(cache.position)
...
*/
                      _v3a
                        .copy(moved_final)
                        .multiply(_v3b.set(feedback, 1, feedback));
                      ...
                      cache.position.add(
                        _v3a
                          .setY(0)
                          .applyQuaternion(
                            center_rotate(subject.quaternion, false, true)
                          )
                      );
                  ...
                  if (
                    !_skip_ground_obj_check &&
                    ((character.ground_obj &&
                      character.ground_obj.obj == obj &&
                      character.ground_obj.bb_y_scale[bb_idx]) ||
                      Math.abs(subject_bb.min.y - object_bb.max.y) < 1)
                  ) {
                    ray.set(_v3.copy(s_bb).setY(999), _v3a.set(0, -1, 0));
                    if (ray.intersectBox(object_bb, intersection)) {
                      collision = true;
                      if (!ground_obj) ground_obj = character.ground_obj;
                  ...
                  var intersected;
                  intersected = ray_normal.intersectBox(
                    object_bb,
                    intersection
                  );
                  if (!intersected) {
                    ray.set(
                      _v3.copy(_c).add(_v3a.copy(d).multiplyScalar(999)),
                      _v3a.copy(d).negate()
                    );
                  ...
                  if (!hit && !hit_moved && !intersected) return;

                  if (!intersected) {
                    _d.copy(_c).sub(s_bb).normalize();
                    ray.set(
                      _v3.copy(s_bb).add(_v3a.copy(_d).multiplyScalar(-999)),
                      _d
                    );
                  ...
                  if (!hit) {
                    if (
                      subject_bb.distanceToPoint(intersected) > mov_delta_length
                    )
                      return;
                  }
                  ...
                  if (bb.oncollide) {
                    if (bb.skip_bb_index_list)
                      skip_bb_index_list = skip_bb_index_list.concat(
                        bb.skip_bb_index_list
                      );

                    let result = bb.oncollide({
                      subject: subject,
                      mov_delta: moved_final,
                      null_move: null_move,
                      skip_ground_obj_check: _skip_ground_obj_check,
                      obj: obj,
                      object_bb: object_bb,
                      bb_idx: bb_idx,
                      intersected: intersected,
                      _moved_final: _moved_final,
                    ...
                    if (result) {
                      ...
                      if (result.ground_obj) ground_obj = result.ground_obj;
                    }
                  } else if (
                    (!ground_obj || ground_obj.obj == obj) &&
                    Math.abs(intersection.y - object_bb.max.y) < 0.001 &&
                    intersection.y <= subject_bb.min.y + _moved_final.y + 2
                  ) {
                    ...
                    if (
                      ground &&
                      ground.obj == obj &&
                      object_bb.containsPoint(_v3.copy(c).setY(_c.y))
                    ) {
                      ...
                      let xz = Math.sqrt(
                        _moved_final.x * _moved_final.x +
                          _moved_final.z * _moved_final.z
                      );
                      if (_bb.max.x - _bb.min.x < _bb.max.z - _bb.min.z) {
                        _moved_final.x = _moved_final.x
                          ? (_moved_final.x > 0 ? 1 : -1) * xz
                          : 0;
                        _moved_final.z = 0;
                      } else {
                        _moved_final.z = _moved_final.z
                          ? (_moved_final.z > 0 ? 1 : -1) * xz
                          : 0;
                        _moved_final.x = 0;
                      }
                      if (
                        y_diff > (subject_bb.max.y - subject_bb.min.y) / 2 ||
                        y_diff > (object_bb.max.y - object_bb.min.y) / 2
                      )
                        _moved_final.y = 0;
                      _moved_final
                        .normalize()
                        .multiplyScalar(Math.max(mov_delta_length, 1));
                    }
                  } else {
                    _moved_final
                      .copy(intersected)
                      .sub(_bb.copy(subject_bb).clampPoint(intersected, _v3))
                      .multiplyScalar(0.75);
                    if (
                      _bb
                        .translate(_moved_final)
                        .isIntersectionBox(object_bb) ||
                      mov_delta_length / 4 > _moved_final.length()
                    ) {
                ...
                if (collision_by_mesh) {
                  ...
                  if (use_bb_translate_offset) {
                    subject_bb_MS = subject_bb
                      .clone()
                      .translate(bb_translate_offset);
                    moved_before_bb_check_MS = moved_before_bb_check
                      .clone()
                      .sub(bb_translate_offset);
                  }

                  null_move =
                    !moved_before_bb_check_MS.x &&
                    !moved_before_bb_check_MS.y &&
                    !moved_before_bb_check_MS.z;
                  const subject_bb_to_collide = null_move
                    ? subject_bb_MS
                    : subject_bb_moved;
                  ...
                  if (that.use_octree) {
                    ...
                    const height =
                      subject_bb_to_collide.max.y - subject_bb_to_collide.min.y;
                    const radius =
                      (height / 8) *
                      (obj_base.octree_collider_radius || 1) *
                      1.5;
                    ...
                    const c = new THREEX.Capsule(
                      new THREEX.Vector3(0, radius, 0),
                      new THREEX.Vector3(
                        0,
                        Math.max(height - radius * 2, radius),
                        0
                      ),
                      radius
                    );
                    ...
                    const mov_extended = _v3
                      .copy(moved_before_bb_check_MS)
                      .add(cache.position)
                    ...
                    const ray = new THREEX.Ray(
                      ray_origin,
                      new THREEX.Vector3(0, -1, 0)
                    ).applyMatrix4(obj_m4_inv);
                    ...
                    ray_origin
                      .copy(pos)
                      .add(mov_extended.multiplyScalar(radius));
                    ray_origin.y += ground_upper_limit;
                    grounds.push(octree.rayIntersect(ray));

                    pos
                      .applyMatrix4(obj_m4_inv)
                      .applyQuaternion(cache.quaternion);
                    ...
                    if (ground_y > -9999) {
                      ...
                      moved_before_bb_check.add(
                        mov_offset.multiply(cache.scale)
                      );
                    }
                    ...
                    if (result_octree) {
                      mov_octree.add(
                        result_octree.normal.multiplyScalar(result_octree.depth)
                      );
                      //DEBUG_show(mov_octree.toArray().join('\n'))
                      if (!grounded.length) {
                        //applyMatrix4(obj_m4_inv)
                        const face_v3 = MMD_SA.TEMP_v3.set(0, 0, 1)
                          .applyQuaternion(subject.quaternion)
                          .applyQuaternion(
                            MMD_SA.TEMP_q.copy(cache.quaternion).conjugate()
                          )
                          .normalize()
                          .multiplyScalar(result_octree.depth * 1.5)
                          .negate();
                    ...
                    result = {
                      ...
                      ground_y: ground_octree
                        ? ground_octree.position.y
                        : -9999,
                    };
                    //DEBUG_show(result.updated+'/'+(result_octree.depth||0)+'/'+result.ground_y+'\n'+pos.toArray().join('\n'))
                  } else {
                    result = subject_bb_to_collide.intersectObject(
                      obj,
                      subject_bb_MS
                    );
                  }

                  let pos_in_obj = new THREE.Vector3()
                    .copy(subject.position)
                    .applyMatrix4(_m4.getInverse(cache.matrixWorld));
                  ...
                  if (result.ground_y > -999) {
                    ground_y =
                      result.ground_y * object_scale + cache.position.y;

                    let ground_y_current = ground_obj || character.ground_obj;
                    ground_y_current =
                      ground_y_current && ground_y_current.bb_y_scale.mesh;
                    //DEBUG_show(ground_y_current+'/'+Date.now())

                    // prevent big drop in height
                    if (
                      ground_y_current != null &&
                      ground_y_current >
                        ground_y + (obj.collision_by_mesh_drop_limit || 999)
                    ) {
                      result.bb_static_collided = result.updated = false;
                    }
                    // cancel drop if the ground is below certain limit
                    else if (
                      obj.collision_by_mesh_ground_limit != null &&
                      ground_y < obj.collision_by_mesh_ground_limit
                    ) {
                      result.bb_static_collided = result.updated = false;
                    }
                    // ground_y+ fixes some possible glitches in ground level changes
                      ...
                      !ground_obj ||
                      ground_obj.bb_y_scale.mesh == null ||
                      ground_obj.bb_y_scale.mesh < ground_y + 3
                    ) {
              ...
              if (subject_bs.intersectsSphere(object_bs)) {
                ...
                moved_final
                  .copy(p)
                  .normalize()
                  .multiplyScalar((r + subject_bs.radius - dis) * 0.95);
                return;
              }
            ...
            return collision
              ? {
                  moved_final: moved_final.applyQuaternion(
                    center_rotate(subject.quaternion, false, true)
                  ),
                  obj_hit: !!obj_hit,
                  ground_obj: ground_obj,
                  collision_by_mesh_checked: collision_by_mesh_checked,
                  collision_by_mesh_failed: collision_by_mesh_failed,
                }
              : {
                  collision_by_mesh_checked: collision_by_mesh_checked,
                  collision_by_mesh_failed: collision_by_mesh_failed,
        ...
        d.check_ray_intersection = (function () {
          ...
          return function (s, dir, para) {
            ...
            this.object_list
              .concat(MMD_SA.THREEX._object3d_list_ || [])
              .forEach(function (obj, idx) {
                ...
                if (obj.no_collision) return;

                if (para.filter && para.filter(obj)) return;

                var obj_base =
                  obj._obj_base ||
                  MMD_SA_options.Dungeon.object_base_list[obj.object_index];

                object_bs.copy(obj._obj_proxy.boundingSphere);
                object_bs.center.add(cache.position);
                object_bs.radius *=
                  Math.max(cache.scale.x, cache.scale.y, cache.scale.z) *
                  ((obj_base.construction &&
                    obj_base.construction.boundingSphere_radius_scale) ||
                    (obj_base.character_index ? 0.5 : 1));
                ...
                if (s.distanceTo(c) > dis + r) return;

                if (!obj._obj_proxy.boundingBox) return;

                var skip_bb_index_list = [];
                obj._obj_proxy.boundingBox_list.forEach(function (bb, bb_idx) {
                  if (
                    obj_base.construction &&
                    obj_base.construction.boundingBox_list &&
                    obj_base.construction.boundingBox_list[bb_idx] &&
                    obj_base.construction.boundingBox_list[bb_idx]
                      .no_camera_collision
                  )
                    return;
                  ...
                  object_bb.copy(bb).applyMatrix4(cache.matrixWorld);
                  if (!ray.intersectBox(object_bb, intersection)) return;
                  ...
                  const i_obj = {
                    ...
                    obj: obj,
                    bb_index: bb_idx,
                    oncollide: !!bb.oncollide,
                  };
                  intersected.push(i_obj);
                  if (nearest.distance > i_dis) nearest = i_obj;
                });
                ...
                if (octree_result) {
                  const i_dis = s.distanceTo(
                    octree_result.position.applyMatrix4(cache.matrixWorld)
                  );
                  if (i_dis > dis) return;
            ...
            return intersected.length && !nearest.oncollide
              ? { nearest: nearest, intersected: intersected }
              : null;
          };
        })();
        ...
        d.check_mouse_on_object = (function () {
          ...
          return function (e, obj_list) {
            ...
            if (!obj_list) obj_list = this.object_list_click;
            ...
            obj_list.forEach(function (obj) {
              ...
              obj.onclick.forEach(function (click) {
                click_range.push(
                  !list_all_clickable && is_dblclick != !!click.is_dblclick
                    ? 0
                    : (click.click_range || click_range_default) +
                        (click.boundingSphere_included
                          ? obj._obj_proxy.boundingSphere.radius *
                            Math.max(
                              cache.scale.x,
                              cache.scale.y,
                              cache.scale.z
              ...
              obj._click_index = -1;
              var dis = PC_pos.distanceToSquared(
                _v3
                  .copy(obj._obj_proxy.boundingSphere.center)
                  .multiply(cache.scale)
                  .add(cache.position)
              );
            ...
            if (!list_all_clickable) {
              obj_sorted.sort(function (a, b) {
                return a._sort_weight || b._sort_weight
                  ? (b._sort_weight || 0) - (a._sort_weight || 0)
                  : a._dis_from_PC - b._dis_from_PC;
              });
            }
            ...
            for (var k = 0, k_max = obj_sorted.length; k < k_max; k++) {
              ...
              if (obj._obj_proxy.boundingBox) {
                let bb_list = obj._obj_proxy.boundingBox_list || [
                  obj._obj_proxy.boundingBox,
                ];
                ...
                bb_list.some(function (b3, bb_idx) {
                  if (bb_idx) {
                    if (
                      !click.func &&
                      !click.event_id &&
                      (!click.events || !click.events[bb_idx])
                    )
                      return;
                  }
                  ...
                  if (obj._mesh.bones) {
                    ...
                    if (b_center.pmxBone)
                      bb.translate(
                        _v3
                          .copy(b_center.position)
                          .sub(_v3a.fromArray(b_center.pmxBone.origin))
                      );
                  }
                  ...
                  if (e) {
                    ...
                    if (
                      click_range &&
                      bb.center(_v3).distanceToSquared(PC_pos) >
                  ...
                  if (
                    list_all_clickable ||
                    ray.intersectBox(bb, intersection)
                  ) {
                    let _obj = {};
                    _obj.obj = obj;
                    if (bb_list.length > 1) _obj.bb_index = bb_idx;
                ...
                if (!list_all_clickable && obj_clicked_list.length) break;
                ...
                //if(p_d > 0 || dot(p, p) < rSquared)
                // return NO_COLLISION;
                if (p_d > 0 || p.dot(p) < rSquared) continue;
            ...
            if (!obj_clicked_list.length) return;
            ...
            if (click.func) {
              ...
              this.run_event(
                (click.events &&
                  click.events[bb_index_clicked] &&
                  click.events[bb_index_clicked].id) ||
                  click.event_id
              );
            }
        ...
        (function () {
            ...
            onclick: [
              {
                is_dblclick: true,
                click_range: 320,
                func: function (e, intersected) {
                  if (!MMD_SA.MMD.motionManager.para_SA.object_click_disabled)
                    window.dispatchEvent(
                      new CustomEvent("SA_Dungeon_character_clicked", {
                        detail: { target: e.target, intersected: intersected },
                      })
                    );
                },
              },
          ...
          var e_func = function (e) {
            ...
            var obj_list;
            if (
              is_dblclick == obj_character.onclick[0].is_dblclick &&
              !MMD_SA.MMD.motionManager.para_SA.click_disabled
            ) {
          ...
          var c_host = returnBoolean("CSSTransform3DDisabledForContent")
            ? document.getElementById("Lbody_host")
            : document.getElementById("Lbody");
          ...
          c_host.addEventListener("click", function (e) {
            if (performance.now() - _mousedown_timestamp < 500) e_func(e);
            _mousedown_timestamp = 0;
          });
          ...
          (function () {
            ...
            var mov_delta = (d._mov_delta = (function () {
              ...
              DataLast.prototype.init = function (para) {
                ...
                if (!m) {
                  m = this.by_motion[para._index] = {
                    acceleration_mov_last: [],
                    t: 9999,
                  };
              ...
              DataLast.prototype.reset = function (para) {
                this.by_motion[para._index].acceleration_mov_last.forEach(
                  function (v3) {
                    v3.set(0, 0, 0);
                  }
                );
              ...
              window.addEventListener("SA_Dungeon_onrestart", function () {
                for (
                  var i = 0, i_max = MMD_SA_options.model_para_obj_all.length;
                  i < i_max;
                  i++
                ) {
...
*/
                if (t == null)
                  t =
                    MMD_SA.motion[model.skin._motion_index].para_SA == para
                      ? model.skin.time - t_diff
                      : 0;
                if (t < 0) t = 0;
                ...
                for (var i = 0, i_max = mov_speed.length; i < i_max; i++) {
                  ...
                  if (obj.acceleration) {
                    ...
                    movement_v3
                      .copy(obj.speed)
                      .multiplyScalar(_t_diff)
                      .add(
                        _v3a
                          .copy(obj.acceleration)
                          .multiplyScalar(0.5 * _t_diff * _t_diff)
                      );
                    ...
                    if (obj.acceleration.y) y_accelerated = true;
                  } else {
                    movement_v3.copy(obj.speed).multiplyScalar(t_diff);
                  }

                  if (mov_delta_finished) break;
            ...
            document.addEventListener("keyup", function (e) {
              ...
              if (key_map.onkeyup && key_map.onkeyup()) return;
            ...
            d.SA_keydown = function (e) {
              ...
              const result = {};
              window.dispatchEvent(
                new CustomEvent("SA_Dungeon_keydown", {
                  detail: { e: _e, result: result },
                })
              );
              ...
              if (msg_branch_list) {
                ...
                if (
                  !d._states.action_allowed_in_event_mode ||
                  (k >= 96 && k <= 96 + 9) ||
                  (k >= 48 && k <= 48 + 9)
                )
                  // || /Key[A-Z]/.test(k_code))
                  e.detail.result.return_value = true;
                for (
                  var i = 0, i_max = msg_branch_list.length;
                  i < i_max;
                  i++
                ) {
                  ...
                  if (!is_mobile && (_e.ctrlKey || _e.shiftKey || _e.altKey))
                    break;

                  const keys = Array.isArray(branch.key)
                    ? branch.key
                    : [branch.key];
                  ...
                  if (key_matched != null) {
                    ...
                    if (
                      MMD_SA_options.SpeechBubble_branch &&
                      MMD_SA_options.SpeechBubble_branch.confirm_keydown &&
                      key_matched != sb._branch_key_ &&
                      sb.msg_line.some(
                        (msg) =>
                          MMD_SA_options.SpeechBubble_branch.RE.test(msg) &&
                          RegExp.$1 == key_matched
                      )
                    ) {
                      ...
                      if (
                        ...
                        branch.branch_index != null ||
                        branch.event_index != null
                      )
                        d.run_event(
                          branch.event_id,
                          branch.branch_index,
                          branch.event_index || 0
                        );
                      else d.run_event();
                    }
                    break;
                ...
                if (e.detail.result.return_value) return;
              } else {
                if (k_code == "Escape") {
                  if (System._browser.overlay_mode == 0) {
                    System._browser.overlay_mode =
                      System._browser.overlay_mode_TEMP = 1;
                  } else if (System._browser.overlay_mode_TEMP) {
                    System._browser.overlay_mode =
                      System._browser.overlay_mode_TEMP = 0;
                  } else {
                    document.getElementById(
                      "Ldungeon_inventory"
                    ).style.visibility =
                      document.getElementById("Ldungeon_inventory").style
                        .visibility == "hidden"
                        ? "inherit"
                        : "hidden";
                  }
              ...
              var key_map = d.key_map[k];
              if (
                d._states.dialogue_mode &&
                !msg_branch_list &&
                (!key_map || !/^(up|left|down|right)$/.test(key_map.id))
              ) {
                d.run_event();
                e.detail.result.return_value = true;
                return;
            ...
            var _keydown = (function () {
              ...
              return function (e, key_map, t) {
                ...
                if (
                  (key_map.type_movement &&
                    (d.character_movement_disabled ||
                      e.detail.e?.altKey ||
                      e.detail.e?.ctrlKey)) ||
                  (key_map.type_combat && d.character_combat_locked)
                ) {
                  e.detail.result.return_value = true;
                  return;
                }

                if (key_map.ondown && key_map.ondown(e)) return;

                if (!key_map.down) {
                  if (
                    !d.character.grounded &&
                    (key_map.type_movement || key_map.type_combat) &&
                    !key_map.motion_can_float
                  ) {
                  ...
                  if (
                    first_press &&
                    key_map.onfirstpress &&
                    key_map.onfirstpress(e)
                  )
                    return;
                }
...
*/
            ...
            var combat_para_default = (function () {
              ...
              return {
                ...
                filter_obj: function (obj, bb, simple_mode) {
                  if (!obj.mass || !obj.hp) return false;

                  if (simple_mode) return true;

                  var attacker_index = this.combat_para.attacker.obj._index;
                  var attacker_combat_stats =
                    this.combat_para.attacker.obj.combat_stats;
                  ...
                  if (!hit_obj._combat_hit) hit_obj._combat_hit = {};
                  ...
                  var attack_ignored =
                    _hit.combat_para &&
                    (this.combat_para.para == _hit.combat_para.para
                  ...
                  if (!attack_ignored) {
                    ...
                    if (obj.character_index != null) {
                      ...
                      let combat_para =
                        this.combat_para.para[this.combat_para.index];
                      let hit_level = combat_para.hit_level || 1;
                      let model = THREE.MMD.getModels()[obj.character_index];
                      let model_para =
                        MMD_SA_options.model_para_obj_all[obj.character_index];
                      let motion_para =
                        MMD_SA.motion[model.skin._motion_index].para_SA;

                      let motion_prefix =
                        obj.character_index == 0
                          ? "PC "
                          : "NPC-" + obj.character_index + " ";

                      let super_armor_level =
                        (motion_para.super_armor &&
                          motion_para.super_armor.level) ||
                        0;

                      let hit_motion, parried_level;
                      if (motion_para.combat_para) {
                        if (
                          d._combat_para.some(function (p) {
                            return p.attacker.obj == obj;
                          })
                        ) {
                          //DEBUG_show("Double HIT!",0,1)
                          //            hit_motion = "PC combat parry broken"
                        } else {
                          let frame_before_end =
                            combat_para.frame_range[1] -
                            1 -
                            this.combat_para.frame;
                          if (frame_before_end > 0) {
                            let motion_frame =
                              model.skin.time * 30 +
                              Math.min(frame_before_end, 3);
                            if (
                              motion_para.combat_para.some(function (hit) {
                                return (
                                  motion_frame >= hit.frame_range[0] &&
                                  motion_frame <= hit.frame_range[1]
                                );
                              })
                            ) {
                      ...
                      if (!super_armor_hit) {
                        ...
                        let NPC_parried_level =
                          obj.character_index != 0 &&
                          obj.combat &&
                          obj.combat.parry_check(obj, this.combat_para);
                        if (!parried_level) {
                          if (obj.character_index == 0) {
                            parried_level = motion_para.PC_parry_level;
                          } else {
                            parried_level =
                              ((motion_para.PC_parry_level ||
                                (!motion_para.motion_command_disabled &&
                                  !motion_para.NPC_motion_command_disabled)) &&
                                NPC_parried_level) ||
                              0;
                          }
                        }
                      ...
                      if (!parried_level) {
                        ...
                        if (d._states.combat) {
                          ...
                          if (damage == null) {
                            damage = d._states.combat.enemy_list.length
                              ? Math.min(hit_level * 5, 20) *
                                Math.min(1 + this.combat_para.index * 0.2, 2)
                              : 0;
                          }
                          damage *=
                            attacker_combat_stats.attack.scale *
                            obj.combat_stats.defense.scale *
                            (super_armor_hit
                              ? motion_para.super_armor.damage_scale || 0
                              : 1);
                          obj.hp_add(-damage);
                      ...
                      if (combat_para.SFX) {
                        if (vfx) {
                          ...
                          if (combat_para.SFX.bone_to_pos) {
                            let bone_to_pos = MMD_SA.get_bone_position(
                              this.combat_para.attacker.obj._mesh,
                              combat_para.SFX.bone_to_pos
                            );
                            //console.log(bone_to_pos)
                            _v3a_cp.copy(bone_to_pos);
                            _v3a
                              .copy(bone_to_pos)
                              .sub(obj._obj.position)
                              .setY(0);
                              ...
                              _v3a
                                ...
                                .multiplyScalar(
                                  Math.max(
                                    radius / 2,
                                    radius -
                                      obj._obj.geometry.boundingSphere.radius
                                  )
                                )
                            );
                          ...
                          if (
                            combat_para.SFX.visual &&
                            combat_para.SFX.visual[vfx]
                          ) {
                            if (combat_para.SFX.visual[vfx].sprite) {
                              SFX_para.sprite = [];
                              combat_para.SFX.visual[vfx].sprite.forEach(
                                (s) => {
                                  SFX_para.sprite.push(
                                    Object.assign({}, para, s)
                                  );
                                }
                              );
                            }
                      ...
                      if (super_armor_hit && !hit_motion) return;
                      ...
                      if (obj.character_index == 0) {
                        this._attacker_list.push({
                          attacker: this._subject,
                          hit_level: hit_level,
                          motion_id:
                            d.motion[hit_motion] && d.motion[hit_motion].name,
                        });
                ...
                _bb_expand: {
                  x: (1 + 0.5) / _bb_xz_factor_ - 1,
                  y: 0,
                  z: (1 + 0.5) / _bb_xz_factor_ - 1,
                },
            ...
            if (MMD_SA_options.Dungeon_options.combat_para_default) {
              Object.assign(
                combat_para_default,
                MMD_SA_options.Dungeon_options.combat_para_default
              );
            }

            d.combat_para_process = function (
              attacker,
              combat_para_parent,
              frame
            ) {
              combat_para_parent.combat_para.some(function (hit, idx) {
                if (
                  frame >= hit.frame_range[0] &&
                  frame <= hit.frame_range[1]
                ) {
                  let hit_level =
                    (hit.hit_level && Math.min(hit.hit_level, 3)) || 1;
                  ...
                  d._combat_para.push({
                    attacker: attacker,
                    para: combat_para_parent.combat_para,
                    motion_id: combat_para_parent.motion_id,
                    index: idx,
                    frame: frame,
                  });
                  return true;
...
*/
            ...
            return function (e) {
              ...
              if (grid_para.onstay && grid_para.onstay()) return;

              if (d.check_states()) return;
              ...
              time_diff = Math.min(
                time_last ? (t - time_last) / 1000 : 1 / 30,
                1 / 20
              );
              time_last = t;
              ...
              window.dispatchEvent(
                new CustomEvent("SA_Dungeon_object_animation")
              );
              ...
              var model_para =
                MMD_SA_options.model_para_obj_all[model._model_index];
              ...
              var TPS_mode = c.TPS_mode,
                TPS_mode_in_action,
                TPS_character_rotated,
                TPS_use_last_rot,
                TPS_camera_ry,
                TPS_camera_lookAt,
                TPS_camera_lookAt_;
              ...
              if (
                TPS_mode &&
                !motion_command_disabled &&
                MMD_SA._force_motion_shuffle &&
                MMD_SA_options.motion_shuffle_list_default
              ) {
                motion_command_disabled =
                  MMD_SA.motion[MMD_SA_options.motion_shuffle_list_default[0]]
                    .para_SA.motion_command_disabled;
                DEBUG_show(
              ...
              d.key_map_list.forEach(function (k) {
                ...
                if (key_motion_disabled) {
                  if (key_map.down && key_map.motion_id) {
                    var _motion_index =
                      MMD_SA_options.motion_index_by_name[
                        d.motion[key_map.motion_id].name
                      ];
                    var _mm = MMD_SA.motion[_motion_index];
                    key_motion_running =
                      !key_map._motion_time ||
                      key_map._motion_time < _mm.lastFrame_ / 30;
                  }
                  if (key_motion_running) {
                    if (!key_map._motion_time) key_map._motion_time = 0;
                    key_map._motion_time += (t - key_map.down) / 1000;
                    motion_time = key_map._motion_time;
                  } else key_map._motion_time = 0;
                } else {
                  key_motion_running = mm.filename == key_map.motion_filename;
                  if (key_motion_running) {
                    // && (!key_map.type_movement || !d.character_movement_disabled)) {
                ...
                if (
                  key_map.down &&
                  (!key_map.motion_duration || !key_motion_running)
                ) {
                  ...
                  if (
                    key_map.type_combat &&
                    (!d.character.combat_mode ||
                      (d.character_combat_locked &&
                        d.character_combat_locked != key_map.id))
                  ) {
                    key_map.down = 0;
                  }
                }
                ...
                if (TPS_mode) key_map_by_mode = key_map.TPS_mode;
                if (!key_map_by_mode) key_map_by_mode = key_map;

                if (key_map_by_mode.motion_filename && key_map.down) {
                  let _k = (key_pressed_stats[k.keyCode] = key_pressed_stats[
                    k.keyCode
                  ] || { first_press: 0, pressed: 0 });
                  if (mm.filename != key_map_by_mode.motion_filename) {
                    _k.first_press = key_map.is_down;
                  }
                  if (key_map.is_down) {
                    if (_k.first_press) _k.pressed = t - _k.first_press;
                  } else {
                ...
                if (key_map.onupdate) {
                  if (key_map.down) {
                    motion_para =
                      key_map_by_mode.motion_id &&
                      d.motion[key_map_by_mode.motion_id].para;
                    t_diff =
                      Math.min((t - key_map.down) / 1000, time_diff) *
                      ((motion_para &&
                        motion_para.playbackRate_by_model_index &&
                        motion_para.playbackRate_by_model_index[0]) ||
                        1);
                ...
                if (result) {
                  if (result.TPS_mode != null) {
                    if (!result.TPS_mode) key_map_by_mode = key_map;
                  }
                ...
                if (key_map.down) {
                  ...
                  if (motion_duration) {
                    ...
                    t2 =
                      (key_motion_running ? motion_time - t_diff : 0) +
                      ((result && result.t2_extended) || 0);
                  ...
                  if (!motion_duration || t2 < motion_duration) {
                    ...
                    if (key_map_by_mode.mov_speed) {
                      ...
                      mov = mov_delta(
                        ...
                        t_diff,
                        t2
                      ).multiplyScalar(key_map_data.scale || 1);
                      if (_mov) {
                        let mov_length = mov.length();
                        mov
                          .lerp(_mov, 0.5)
                          .normalize()
                          .multiplyScalar(Math.max(_mov.length(), mov_length));
                      }
                    }
                    ...
                    if (TPS_mode_in_action) {
                      if (!TPS_camera_lookAt_) {
                        ...
                        TPS_camera_ry =
                          Math.PI / 2 -
                          Math.atan2(
                            camera.target.z - camera.object.position.z,
                            camera.target.x - camera.object.position.x
                          );
                      ...
                      if (
                        key_map_by_mode.mov_to_rot_absolute &&
                        mov &&
                        (mov.x || mov.z)
                      ) {
                        ...
                        rot = rotation_v3.set(
                          0,
                          Math.PI / 2 - Math.atan2(mov.z, mov.x),
                          0
                        );
                        if (!TPS_camera_lookAt_) {
                      ...
                      // case: no rotation from mov
                        ...
                        // otherwise, use PC's current rotation (minus TPS_camera_ry which will be added to rot later)
                          ...
                          if (
                            motion_id &&
                            motion_id == key_map_by_mode.motion_filename &&
                            /^(.+)(forward|right|backward|left)$/.test(
                              key_map_by_mode.motion_id
                            )
                          ) {
                    ...
                    if (key_map_by_mode.key_id_cancel_list) {
                      key_map_by_mode.key_id_cancel_list.forEach(function (
                        kc_id
                      ) {
                    ...
                    var key_id_cancel_list = key_map_by_mode.key_id_cancel_list
                      ? key_map_by_mode.key_id_cancel_list.slice()
                      : [];
                    if (key_map.type_combat) {
              ...
              if (TPS_mode_in_action) {
                if (!mov) mov = movement_v3.set(0, 0, 0);
                ...
                if (TPS_use_last_rot) {
                  ...
                  let cy =
                    Math.PI / 2 -
                    Math.atan2(
                      TPS_camera_lookAt.z - c.pos.z,
                      TPS_camera_lookAt.x - c.pos.x
                    );
...
*/
              ...
              if (mov)
                mov.multiplyScalar(
                  (c.mount_para && c.mount_para.speed_scale) || c.speed_scale
                );
              ...
              if (c.ground_obj) {
                ...
                for (var index in c.ground_obj.bb_y_scale) {
                  ground_y =
                    index == "mesh"
                      ? Math.max(
                          c.ground_obj.obj.collision_by_mesh_enforced
                            ? -999
                            : ground_y,
                          c.ground_obj.bb_y_scale.mesh
                        )
                      : Math.max(
                          ground_y,
                          g._obj_proxy.boundingBox_list[index].max.y *
                            g_obj.scale.y *
                            c.ground_obj.bb_y_scale[index] +
                            g_obj.position.y
                        );
                }
              }
              ...
              var gravity_y = 0;
              var time_to_ground;
              const falling_height_threshold =
                MMD_SA_options.Dungeon_options.falling_height_threshold || 10;
              if (!floating) {
                let v =
                  (gravity_obj.y + gravity_obj.mov_y_last) / gravity_obj.time;
                // downward is positive
                gravity_y =
                  v * time_diff + 0.5 * (98 * 1.5) * time_diff * time_diff;
                ...
                if (c.pos.y > ground_y + falling_height_threshold) {
                  ...
                  let _x0 =
                    Math.pow(Math.pow(_b, 2) - 4 * _a * _c, 0.5) / 2 / _a;
              ...
              if (!floating || c.pos.y < ground_y) {
                ...
                if (
                  c.ground_obj &&
                  c.ground_obj.mov &&
                  (Math.abs(ground_y_delta) != Math.abs(gravity_y) ||
                    gravity_y < 0.1)
                ) {
                  if (!mov) mov = movement_v3.set(0, 0, 0);
                  mov.add(
                    _v3a
                      .copy(c.ground_obj.mov)
                      .applyQuaternion(
                        _q.copy(model.mesh.quaternion).conjugate()
                      )
                  );
                }
              }
              ...
              if (!floating) gravity_obj.y = gravity_y;
              else gravity_obj.y = 0;
              ...
              _v3a.copy(mov);
              if (!TPS_mode_in_action) _v3a.applyEuler(c.rot);
              ...
              if (time_falling < 0) {
                ...
                let falling =
                  !key_motion_disabled &&
                  ground_y_delta < 0 &&
                  c.pos.y > ground_y + falling_height_threshold;
                if (falling || (landing && !c.grounded)) {
                  ...
                  mov
                    .copy(c.inertia)
                    .multiplyScalar(time_diff * Math.pow(0.95, time_diff * 30));
                ...
                if (falling) {
                  ...
                  if (!d.event_mode) d.character_movement_disabled = true;
              ...
              d._combat_para.forEach(function (para) {
                ...
                if (attacker != PC) {
                  ...
                  if (mesh._bone_to_position_last) {
                    mesh.position.sub(
                      mesh._bone_to_position_last.pos_delta_rotated
                    );
                    _pos_restored[attacker.obj._index] = true;
                  }
                } else combat_para_default.object_list = null;

                // ,_bb_expand: {x:0.5, y:0, z:0.5}
                // ,_bb_translate: {x:0, y:0, z:0.5}
                var para_hit = para.para[para.index];
                combat_para_default.bb_expand =
                  para_hit.bb_expand || combat_para_default._bb_expand; //{x:0,y:0,z:99}//
                combat_para_default.bb_translate =
                  para_hit.bb_translate || combat_para_default._bb_translate;

                d.check_collision(
                  attacker,
                  attacker != PC
                    ? d._mov[attacker.obj._index] || new THREE.Vector3()
                    : _v3a,
                  true,
                  combat_para_default
                );
              });
              var motion_id_enforced =
                combat_para_default._attacker_list.length;

              if (motion_id_enforced) {
                var hit_para = combat_para_default._attacker_list
                  .sort(function (a, b) {
                    return a.hit_level - b.hit_level;
                  })
                  .pop();
                motion_id = hit_para.motion_id;
                //DEBUG_show(motion_id,0,1)
                rot_absolute = new THREE.Vector3().set(
                  0,
                  Math.PI / 2 -
                    Math.atan2(
                      hit_para.attacker.obj._obj.position.z - c.pos.z,
                      hit_para.attacker.obj._obj.position.x - c.pos.x
                    ),
                  0
                );
              }
              ...
              if (
                ...
                rot_absolute = new THREE.Vector3().set(
                  ...
                  Math.PI / 2 -
                    Math.atan2(
                      TPS_camera_lookAt_.z - c.pos.z,
                      TPS_camera_lookAt_.x - c.pos.x
                    ),
                  0
                );
              }
              ...
              d.object_list.forEach(function (obj, idx) {
                ...
                if (!_object_list)
                  _object_list = [c].concat(
                    (combat && combat.enemy_list) || []
                  ); //.concat(d.object_list)//
                ...
                var result = d.check_collision(
                  { obj: obj, mass: obj.mass },
                  _mov,
                  true,
                  {
                    collision_by_mesh_disabled: true,
                    check_grid_blocks: true,
              ...
              if (!motion_id && pos_delta) {
                mov = _v3a.add(
                  model.mesh._bone_to_position_last.pos_delta_rotated
                );
                null_mov = false;
              }
              ...
              for (var i = 0, i_max = dir_block.length; i < i_max; i++) {
                ...
                if (b_list) {
                  ...
                  if (!null_mov && !_v3b.x && !_v3b.y && !_v3b.z) continue;
                }
                if (true) {
                  //!d.check_grid_blocking(_v3c.copy(_v3b).add(c.pos), d.grid_blocking_character_offset, (null_mov)?null:_v3b)) {//
                  //let _t = performance.now()
                  ...
                  if (result.collision_by_mesh_checked) break;
                }
                if (moved) break;
              }
              ...
              // check ground movement START
              ground_y = collision_by_mesh_failed
                ? c.pos.y
                : d.get_ground_y(c.pos);
              if (c.ground_obj) {
                const g = c.ground_obj.obj;
                const g_obj = g._obj;
                for (var index in c.ground_obj.bb_y_scale) {
...
*/
              ...
              if (change_motion) {
                MMD_SA_options.motion_shuffle_list_default = [
                  MMD_SA_options.motion_index_by_name[motion_id],
                ];
                MMD_SA._force_motion_shuffle = true;
              }
              ...
              if (moved) {
                ...
                if (c.pos.y + mov.y < ground_y) mov.y = ground_y - c.pos.y;

                var _y = c.pos.y;
                c.pos.add(mov);
                if (d.camera_y_default_non_negative && c.pos.y < 0)
                  camera_y_offset = mov.y < 0 ? Math.min(_y, 0) - c.pos.y : 0;
                else camera_y_offset = -MMD_SA._camera_y_offset_;
              ...
              if (mov || rot) {
                ...
                let _rot_camera =
                  TPS_mode_in_action && !TPS_camera_lookAt ? null : rot; //(rot_camera.enabled && rot && rot_camera.v3.negate().add(rot))||rot;
...
*/
              ...
              if (about_turn) {
                model.mesh.quaternion.multiply(
                  MMD_SA.TEMP_q.setFromEuler(MMD_SA.TEMP_v3.set(0, Math.PI, 0))
                );
              }

              if (rot_absolute || rot_absolute_with_camera) {
                if (!rot_absolute) rot_absolute = _v3b.set(0, 0, 0);
                if (rot_absolute_with_camera) {
                  rot_absolute.add(rot_absolute_with_camera);
                  rot_absolute_with_camera = _v3a
                    .copy(rot_absolute_with_camera)
                    .sub(c.rot);
                }
                c.about_turn = false;
                ...
                if (rot_absolute_with_camera)
                  MMD_SA._trackball_camera.SA_adjust(
                    null,
                    rot_absolute_with_camera
                  );
              }

              if (update_dungeon_blocks) d.update_dungeon_blocks();
              ...
              d.check_points.forEach(function (cp) {
                var pos;
                if (cp.position) pos = _v3a.copy(cp.position);
                ...
                cp.range.forEach(function (r) {
                  ...
                  if (is_inside) {
                    if (r.onenter && !r._entered) cp_events.push(r.onenter);
                    if (r.onstay) cp_events.push(r.onstay);
                    r._entered = true;
                    if (r.onexit) r.onexit._pos_last = c.pos.clone();
                  } else {
                    if (
                      r.onexit &&
                      r.onexit.condition &&
                    ...
                    if (r.onexit && r._entered) cp_events.push(r.onexit);
                    r._entered = false;
                  }
                });
              });
        ...
        (function () {
          var _reset = function () {
            ...
            for (
              var lvl = 0, lvl_max = this.geo_by_lvl.length;
              lvl < lvl_max;
              lvl++
            ) {
              ...
              lvl_obj.list
                .concat(lvl_obj.list_material_cloned)
              ...
              for (
                i = lvl_obj.index + 1, i_max = lvl_obj.list.length;
                i < i_max;
                i++
              )
              ...
              for (
                i = lvl_obj.index_material_cloned + 1,
                  i_max = lvl_obj.list_material_cloned.length;
                i < i_max;
                i++
              )
                lvl_obj.reusable_list_material_cloned.push(i);
              lvl_obj.index_material_cloned = Math.max(
                i_max - 1,
                lvl_obj.index_material_cloned
              );
            }
          };
              ...
              if (!plane) {
                plane = list[index_used] =
                  list == this.list
                    ? plane0.clone()
                    : plane0.clone(
                        new THREE.Mesh(plane0.geometry, plane0.material.clone())
                      );
          ...
          var _init = function () {
            if (this.disabled || this._initialized) return this;
          ...
          MMD_SA_options.Dungeon.grid_material_list.forEach(function (
            p_obj,
            idx
          ) {
            p_obj.reset = _reset;
        ...
        (function () {
          var _init = function () {
            ...
            this.list.forEach(function (cache) {
              // NOTE: scene.add must come first, or .visible will always be reset to true.
              if (!cache.parent) MMD_SA.THREEX.scene.add(cache);
              cache.visible = false;
              if (!MMD_SA.THREEX.enabled)
                cache.children.forEach(function (c) {
                  c.visible = false;
                });
            });
          };
          ...
          var _reset = function () {
            ...
            var stay_on_scene =
              this.obj_base.stay_on_scene &&
              MMD_SA_options.Dungeon.object_list.some(function (obj) {
                return obj.object_index == that.obj_base.index;
              });
            ...
            this.list.forEach(function (cache, idx) {
              ...
              if (!stay_on_scene) MMD_SA.THREEX.scene.remove(cache);
            });
          };
          ...
          d.object_base_list.forEach(function (obj, idx) {
            ...
            if (obj.construction && !obj.path) {
              obj._mesh_id =
                obj.character_index != null
                  ? "mikuPmx" + obj.character_index
                  : obj.construction.mesh_obj.id;
              ...
              if (mesh.geometry) {
                ...
                mesh.children.forEach(function (mesh_child) {
                  if (mesh_child.geometry) geo_list.push(mesh_child.geometry);
                });
              }
            ...
            mesh.children.forEach(function (mesh_child) {
              if (!mesh_child.useQuaternion)
                mesh_child.quaternion.setFromEuler(mesh_child.rotation);
              mesh_child.useQuaternion = true;
            });

            var geo = mesh.geometry || mesh.children[0].geometry;
            if (
              obj.collision_by_mesh &&
              !obj.collision_by_mesh_sort_range &&
              MMD_SA_options.Dungeon.use_octree
            ) {
            ...
            if (obj.collision_by_mesh_sort_range) {
              ...
              let collision_by_mesh_material_index_max =
                obj.collision_by_mesh_material_index_max || 999;
              ...
              if (d.use_local_mesh_sorting) {
                ...
                if (!d.use_octree) {
                  ...
                  for (let f = 0, fl = geo.faces.length; f < fl; f++) {
                    ...
                    if (
                      face.materialIndex >= collision_by_mesh_material_index_max
                    )
                      break;
            ...
            if (obj.character_index != null) {
              THREE.MMD.getModels()[obj.character_index]._clone_cache =
                obj.cache;
            }
            ...
            if (obj.LOD_far) {
              ...
              let mesh_far = new THREE.Mesh(
                ...
                new THREE.MeshBasicMaterial({
                  color: "#" + MMD_SA.THREEX.scene.fog.color.getHexString(),
                })
              );
        ...
        MMD_SA.SpeechBubble.list.forEach((m) => {
          m.renderDepth = 9999999 * (!MMD_SA.THREEX.enabled ? -1 : 1);
        });
        MMD_SA.SpeechBubble.list.forEach((b) =>
          b.bubbles.forEach((b) => {
            b.pos_mod = [0, -2.5, 0];
          })
        );
        ...
        d.grid_blocks = {
          ...
          update: (function () {
            ...
            return function (pos) {
              ...
              d.grid_blocks.objs[d.grid_blocks.objs.length - 1]._obj.visible =
                d.ceil_material_index_default != -1;
...
*/
              xy_list.forEach(function (_xy, idx) {
                ...
                if (update) {
                  obj.position.copy(c);
                  geo.boundingBox.set(
                    v3b.set(-gs_half, -999, -gs_half),
                    v3c.set(gs_half, ground_y, gs_half)
                  );
                  // it will be updated in .check_collision() as a boundingSphere-hit object
                  //      obj.updateMatrixWorld()
                  obj.visible = true;
      ...
      window.addEventListener("SA_MMD_toggle_shadowMap", function (e) {
        ...
        for (var i = 0, i_max = p.length; i < i_max; i++) {
          ...
          for (
            ...
            p_obj.lvl[lvl].list
              .concat(p_obj.lvl[lvl].list_material_cloned)
              .forEach(function (mesh_obj) {
        ...
        MMD_SA_options.Dungeon.object_base_list.forEach(function (obj) {
          ...
          if (obj.character_index != null) {
            ...
            const model_para =
              MMD_SA_options.model_para_obj_all[obj.character_index];
            const material_para =
              (model_para.material_para &&
                model_para.material_para._default_) ||
              {};

            castShadow =
              enabled &&
              (material_para.castShadow != null
                ? !!material_para.castShadow
                : construction && construction.castShadow != null
                ? construction.castShadow
                : true);
            receiveShadow =
              enabled &&
              (material_para.receiveShadow != null
                ? !!material_para.receiveShadow
                : model_para.is_object || !MMD_SA_options.ground_shadow_only);
          } else {
            updated0 = !!obj.path;

            const x_obj = obj.path
              ? MMD_SA_options.x_object_by_name[
                  obj.path.replace(/^.+[\/\\]/, "").replace(/\.x$/i, "")
                ]
              : MMD_SA_options.mesh_obj_by_id[construction.mesh_obj.id];
            castShadow = enabled && !!x_obj.castShadow;
            receiveShadow = enabled && !!x_obj.receiveShadow;
          }

          for (var i = 0, i_max = c.length; i < i_max; i++) {
            if (i == 0 && updated0) continue;
      ...
      if (MMD_SA_options.model_path_extra) {
        ...
        if (_model_path) {
          ...
          if (index != -1)
            MMD_SA_options.model_path_extra[index] =
              System.Gadget.path +
              "\\jThree\\model\\Appearance Miku\\Appearance Miku_BDEF_mod-v04.pmx";
        }
      }

      if (options.multiplayer) {
        if (!MMD_SA_options.model_para) MMD_SA_options.model_para = {};
        if (!MMD_SA_options.model_path_extra)
          MMD_SA_options.model_path_extra = [];

        let that = this;
        let OPC_index = (options.multiplayer.OPC_index0 =
          1 + MMD_SA_options.model_path_extra.length);
        options.multiplayer.OPC_list.forEach(function (opc, idx) {
          ...
          var para = (MMD_SA_options.model_para[model_filename_raw] =
            opc.para || MMD_SA_options.model_para[model_filename_raw] || {});
      ...
      this.motion["PC default"] = Object.assign(
        {
          path:
            System.Gadget.path +
            "/MMD.js/motion/motion_rpg_pack01.zip#/tsuna/tsuna_standby.vmd",
          para: {
            adjust_center_view_disabled: true,
            loop_on_blending: true,
            onended: function () {
      ...
      Object.assign(
        this.motion["PC default"].para,
        this.motion["PC default"].para_SA || {}
      );
      ...
      this.motion["PC movement forward"] = Object.assign(
        {
          path:
            System.Gadget.path +
            "/MMD.js/motion/motion_rpg_pack01.zip#/walk_n_run/run_H57_f0-20.vmd",
          para: {
            adjust_center_view_disabled: true,
            loop_on_blending: true,
            onended: function () {
      ...
      Object.assign(
        this.motion["PC movement forward"].para,
        this.motion["PC movement forward"].para_SA || {}
      );
...
*/
      ...
      if (!this.motion["PC forward jump"]) {
        this.motion["PC forward jump"] = {
          path:
            System.Gadget.path +
            "/MMD.js/motion/motion_rpg_pack01.zip#/tsuna/tsuna_small_jump.vmd",
          para: {
            adjust_center_view_disabled: true,
            motion_duration: (46 - 12) / 30,
            onended: function () {
              MMD_SA._no_fading = true;
            },
            adjustment_per_model: {
            ...
            mov_speed: (function () {
              ...
              return [
                ...
                {
                  frame: 12,
                  speed: { x: 0, y: va.v, z: (41 / 22) * 30 },
                  acceleration: { x: 0, y: va.a, z: 0 },
                },
            ...
            SFX: [
              { frame: 6 + 1, sound: {} },
              {
                frame: 34,
                condition: (model) =>
                  model.mesh._model_index != 0 ||
                  model.mesh.position.y <
                    MMD_SA_options.Dungeon.character.ground_y + 5,
                sound: {},
              },
            ],
            range: [{ time: [6, 0] }],
      ...
      if (!this.motion["PC high jump"]) {
        this.motion["PC high jump"] = {
          path:
            System.Gadget.path +
            "/MMD.js/motion/motion_rpg_pack01.zip#/landing/05_magical_jump_v01.vmd",
          para: {
            adjust_center_view_disabled: true,
            motion_duration: 162 / 30,
            onended: function () {
              MMD_SA._no_fading = true;
            },
            ...
            mov_speed: (function () {
              ...
              return [
                ...
                {
                  frame: 51,
                  speed: { x: 0, y: 0.001, z: va_z2.v },
                  acceleration: { x: 0, y: va_y2.a, z: va_z2.a },
                },
                {
                  frame: 23,
                  speed: { x: 0, y: va_y1.v, z: 0 },
                  acceleration: { x: 0, y: va_y1.a, z: 0 },
                },
      ...
      if (!this.motion["PC landing"]) {
        this.motion["PC landing"] = {
          path:
            System.Gadget.path +
            "/MMD.js/motion/motion_rpg_pack01.zip#/landing/04_deep_landing.vmd",
          para: {
            adjust_center_view_disabled: true,
            duration: 57 / 30,
      ...
      if (!this.motion["NPC talk A"]) {
        this.motion["NPC talk A"] = {
          path:
            System.Gadget.path +
            "/MMD.js/motion/motion_rpg_pack01.zip#/talk/xs-talk8-west-negotiate.vmd",
          para: {
            onended: function () {
              MMD_SA._no_fading = true;
            },
      ...
      if (!this.motion["NPC walk A"]) {
        this.motion["NPC walk A"] = {
          path:
            System.Gadget.path +
            "/MMD.js/motion/motion_rpg_pack01.zip#/walk_n_run/walk_A01_f0-40_s9.85.vmd",
        };
      }
      if (!this.motion["PC fall on ass"]) {
        this.motion["PC fall on ass"] = {
          path: "MMD.js/motion/motion_rpg_pack01.zip#/hit/w01_すべって尻もち.vmd",
          para: {
            adjust_center_view_disabled: true,
            get look_at_screen_ratio() {
              var t = THREE.MMD.getModels()[0].skin.time;
              return t > 1 ? 0 : 1 - t;
            },
            ...
            onplaying: function (model_index) {
              ...
              if (model.skin.time > mm._timeMax) {
                if (MMD_SA_options.WebXR) {
                  let dis =
                    MMD_SA._v3a
                      .copy(MMD_SA.camera_position)
                      .setY(0)
                      .distanceTo(
                        MMD_SA._v3b
                          .copy(THREE.MMD.getModels()[0].mesh.position)
                          .setY(0)
                      ) /
                    10 /
                    MMD_SA.WebXR.zoom_scale;
                  if (dis < 1) return;
                }
              } else {
                return;
              ...
              if (!this._freeze_onended) model.skin.time = mm.lastFrame_ / 30;
            },
      ...
      if (!this.motion["PC stand up from ass"]) {
        this.motion["PC stand up from ass"] = {
          path: "MMD.js/motion/motion_rpg_pack01.zip#/casual/女の子座り→立ち上がる_gumi_v01.vmd",
          para: {
            adjust_center_view_disabled: true,
            get look_at_screen_ratio() {
              var t = THREE.MMD.getModels()[0].skin.time;
            ...
            adjustment_per_model: {
              _default_: {
                skin_default: {
                  右足ＩＫ: {
                    keys_mod: [
                      { frame: 0, pos: { x: -2.33 - 0.5, y: -0.4, z: 1.3 } },
                      { frame: 7, pos: { x: -2.33 - 0.5, y: -0.4, z: 1.3 } },
                    ],
                  },
                  左足ＩＫ: {
                    keys_mod: [
                      { frame: 0, pos: { x: 2.33 + 0.5, y: -0.4, z: 1.3 } },
                      { frame: 47, pos: { x: 2.33 + 0.5, y: -0.4, z: 1.3 } },
                    ],
                  },
                  //がくっぽいどver.2.11.pmd
                  センター: {
                    pos_scale: { auto_adjust: { ref_length: 11.69667 } },
                  },
                },
              },
              "TdaMeiko_Bikini_TypeA.pmx": {
                skin_default: {
                  右足ＩＫ: {
                    keys_mod: [
                      { frame: 0, pos: { x: -2.33 - 1, y: -0.4, z: 1.3 } },
                      { frame: 7, pos: { x: -2.33 - 1, y: -0.4, z: 1.3 } },
                    ],
                  },
                  左足ＩＫ: {
                    keys_mod: [
                      { frame: 0, pos: { x: 2.33 + 1, y: -0.4, z: 1.3 } },
                      { frame: 47, pos: { x: 2.33 + 1, y: -0.4, z: 1.3 } },
                    ],
                  },
                  //がくっぽいどver.2.11.pmd
                  センター: {
      ...
      if (!this.motion["PC fall from trip"]) {
        this.motion["PC fall from trip"] = {
          path: "MMD.js/motion/motion_rpg_pack01.zip#/hit/r01_普通に転ぶ.vmd",
          para: {
            adjust_center_view_disabled: true,
            get look_at_screen_ratio() {
              var f = THREE.MMD.getModels()[0].skin.time * 30;
              return f > 10 ? 0 : (10 - f) / 10;
            },
            super_armor: { level: 99 },
            SFX: [{ frame: 19, sound: {} }],
            onended: function () {
            ...
            adjustment_per_model: {
              ...
              "Amelia Watson_MMD_ver 1.0.pmx": {
                skin_default: {
                  ...
                  センター: {
                    keys_mod: [
                      { frame: 23, pos: { x: -0.65, y: -6.06 - 1.5, z: 0.67 } },
                      {
                        frame: 30,
                        pos: { x: -1.19, y: -5.02 - 1.5, z: -0.13 - 1 },
                      },
                      {
                        frame: 39,
                        pos: { x: -1.19, y: -5.17 - 1.5, z: 0.28 - 2 },
                      },
                      {
                        frame: 68,
                        pos: { x: -1.19, y: -5.17 - 1.5, z: 0.28 - 2 },
      ...
      if (!this.motion["PC stand up from face down"]) {
        this.motion["PC stand up from face down"] = {
          path: "MMD.js/motion/motion_rpg_pack01.zip#/casual/OTL→立ち上がり.vmd",
          para: {
            adjust_center_view_disabled: true,
            get look_at_screen_ratio() {
              var f = THREE.MMD.getModels()[0].skin.time * 30;
              return f < 29 ? 0 : f > 47 ? 1 : (f - 29) / 18;
            },
            super_armor: { level: 99 },
            onended: function () {
              MMD_SA._hit_legs_ = false;
      ...
      if (!this.motion["PC down"]) {
        this.motion["PC down"] = {
          path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\粗製のモーション３\\ナイトミク\\k.miku-down_modified.vmd",
          para: {
            adjust_center_view_disabled: true,
            duration: 2,
            auto_blink: false,
            ...
            look_at_screen: false,
            model_index_list: [0],
            NPC_turns_to_you: true,
            onended_NPC: function (model_index) {
              var d = MMD_SA_options.Dungeon;
              if (
                d._states.combat &&
                d._states.combat.ondefeated(model_index)
              ) {
                return;
              }
      ...
      if (options.combat_mode_enabled) {
        ...
        (function () {
          function basic_check(model_index, x_object) {
            if (!x_object._obj.visible) return false;
            var p_bone = x_object.parent_bone;
            if (!p_bone) return false;
            if (model_index != (p_bone.model_index || 0)) return false;
            return true;
          }

          function two_handed_weapon_equipped(model_index) {
            function _two_handed_weapon_equipped(x_object) {
              if (!basic_check(model_index, x_object)) return false;
              return (
                x_object.user_data.weapon &&
                x_object.user_data.weapon.type == "2-handed"
              );
            }

            return MMD_SA_options.Dungeon.accessory_list.some(
              _two_handed_weapon_equipped
            );
          }
          ...
          function one_handed_weapon_equipped(model_index) {
            function _one_handed_weapon_equipped(x_object) {
              ...
              return (
                x_object.user_data.weapon &&
                x_object.user_data.weapon.type == "1-handed"
              );
            }

            return MMD_SA_options.Dungeon.accessory_list.some(
              _one_handed_weapon_equipped
            );
          }
          ...
          function twin_weapon_equipped(model_index) {
            function _twin_weapon_equipped(x_object) {
              ...
              return (
                x_object.user_data.weapon &&
                x_object.user_data.weapon.type == "twin"
              );
            }

            return MMD_SA_options.Dungeon.accessory_list.some(
              _twin_weapon_equipped
            );
          }

          // /^(\u5DE6|\u53F3)(\u80A9|\u8155|\u3072\u3058|\u624B\u9996|\u624B\u6369|.\u6307.)/
          var RE_skin_jThree = new RegExp(
            "^(" +
              toRegExp(["左", "右"], "|") +
              ")(" +
              toRegExp(["肩", "腕", "ひじ", "手首", "手捩", "ダミー"], "|") +
              "|." +
              toRegExp("指") +
              ".)"
          );
          var RE_skin_jThree_one_handed_guard = new RegExp(
            toRegExp(["上半身", "両目"], "|")
          );
          var RE_skin_jThree_hand_R = new RegExp(
            "^" +
              toRegExp("右") +
              "(" +
              toRegExp("ダミー") +
          ...
          var RE_arms =
            /^(PC movement forward|PC.+jump|PC combat default|PC combat movement|PC combat hit small|PC combat hit medium)/;
          var RE_parry = /^PC combat (parry|parrying)$/;

          if (!MMD_SA_options.custom_action) MMD_SA_options.custom_action = [];
          ...
          MMD_SA_options.custom_action.push(
            {
              action: {
                condition: function (is_bone_action, objs) {
                  ...
                  var motion_id =
                    MMD_SA_options.Dungeon.motion_id_by_filename[
                      MMD_SA.motion[skin._motion_index].filename
                    ] || "";
                  return (
                    RE_arms.test(motion_id) &&
                    two_handed_weapon_equipped(objs._model_index)
                  );
                },
              ...
              //bone_group:["腕","指"]
              motion: {
                path: "MMD.js/motion/motion_rpg_pack01.zip#/粗製のモーション３/2-handed_weapon_arms.vmd",
                match: { skin_jThree: RE_skin_jThree, morph_jThree: false },
                para_SA: {
                  model_index_list:
                    MMD_SA_options.Dungeon.battle_model_index_list,
                },
              },
            },
            {
              ...
              motion: {
                path: "MMD.js/motion/motion_rpg_pack01.zip#/粗製のモーション３/2-handed_weapon_guard.vmd",
                match: {
                  skin_jThree: {
                    test: function (name) {
                      return name == "上半身" || RE_skin_jThree.test(name);
                    },
                  },
                  morph_jThree: false,
                },
...

            {
              action: {
                condition: function (is_bone_action, objs) {
                  ...
                  return (
                    RE_arms.test(motion_id) &&
                    one_handed_weapon_equipped(objs._model_index)
                  );
                },
              ...
              //bone_group:["腕","指"]
              motion: {
                path: "MMD.js/motion/motion_rpg_pack01.zip#/粗製のモーション５/1-handed_weapon_arms.vmd",
                match: { skin_jThree: RE_skin_jThree, morph_jThree: false },
                para_SA: {
                  model_index_list:
                    MMD_SA_options.Dungeon.battle_model_index_list,
            ...
            },
            {
              ...
              motion: {
                path: "MMD.js/motion/motion_rpg_pack01.zip#/粗製のモーション５/1-handed_weapon_guard.vmd",
                match: {
                  skin_jThree: {
                    test: function (name) {
                      return (
                        RE_skin_jThree_one_handed_guard.test(name) ||
                        RE_skin_jThree.test(name)
                      );
                    },
                  },
                  morph_jThree: false,
...

            {
              action: {
                condition: function (is_bone_action, objs) {
                  ...
                  return (
                    MMD_SA_options.Dungeon.character.combat_mode &&
                    RE_arms.test(motion_id) &&
                    twin_weapon_equipped(objs._model_index)
                  );
                },
                onFinish: function () {},
              },
              motion: {
                path: "MMD.js/motion/motion_rpg_pack01.zip#/粗製のモーション４/twin_weapon_arms.vmd",
                match: { skin_jThree: RE_skin_jThree, morph_jThree: false },
                para_SA: {
                  model_index_list:
                    MMD_SA_options.Dungeon.battle_model_index_list,
            ...
            },
            {
              ...
              motion: {
                path: "MMD.js/motion/motion_rpg_pack01.zip#/粗製のモーション４/twin_weapon_guard.vmd",
                match: {
                  skin_jThree: {
                    test: function (name) {
                      return name == "上半身" || RE_skin_jThree.test(name);
...

            {
              ...
              motion: {
                path: "MMD.js/motion/motion_rpg_pack01.zip#/粗製のモーション５/hand_R_weapon_hold.vmd",
                match: {
                  skin_jThree: RE_skin_jThree_hand_R,
                  morph_jThree: false,
                },
                para_SA: {
                  model_index_list:
        ...
        if (!this.motion["PC combat default"]) {
          this.motion["PC combat default"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\咲夜　待機.vmd",
            para: {
              adjust_center_view_disabled: true,
              loop_on_blending: true,
              onended: function () {
        ...
        if (!this.motion["PC combat movement forward"]) {
          this.motion["PC combat movement forward"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\咲夜　構え歩き前進.vmd",
            para: {
              adjust_center_view_disabled: true,
              loop_on_blending: true,
              onended: function () {
        ...
        if (!this.motion["PC combat movement backward"]) {
          this.motion["PC combat movement backward"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\咲夜　構え歩きバック.vmd",
            para: {
              adjust_center_view_disabled: true,
              loop_on_blending: true,
              onended: function () {
        ...
        if (!this.motion["PC combat movement left"]) {
          this.motion["PC combat movement left"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\咲夜　構え歩き左.vmd",
            para: {
              adjust_center_view_disabled: true,
              loop_on_blending: true,
              onended: function () {
        ...
        if (!this.motion["PC combat movement right"]) {
          this.motion["PC combat movement right"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\咲夜　構え歩き右.vmd",
            para: {
              adjust_center_view_disabled: true,
              loop_on_blending: true,
              onended: function () {
        ...
        if (!this.motion["PC combat parry"]) {
          this.motion["PC combat parry"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\咲夜　ガード.vmd",
            para: {
              adjust_center_view_disabled: true,
              onended: function () {
                MMD_SA._no_fading = true;
        ...
        if (!this.motion["PC combat parrying"]) {
          this.motion["PC combat parrying"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\咲夜　ガードヒット.vmd",
            para: {
              adjust_center_view_disabled: true,
              PC_parry_level: 3,
              motion_command_disabled: true,
        ...
        if (!this.motion["PC combat parry broken"]) {
          this.motion["PC combat parry broken"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\格闘簡易作成用モーション\\格闘シーン簡易作成用モーション２．１\\ktntk-albert-2.1\\Albert-guard-broken.vmd",
            para: {
              adjust_center_view_disabled: true,
              motion_command_disabled: true,
              model_index_list: [0],
        ...
        if (!this.motion["PC combat hit small"]) {
          this.motion["PC combat hit small"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\怯み弱.vmd",
            para: {
              adjust_center_view_disabled: true,
              adjustment_per_model: {
                _default_: {
        ...
        if (!this.motion["PC combat hit medium"]) {
          this.motion["PC combat hit medium"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\怯み中.vmd",
            para: {
              adjust_center_view_disabled: true,
        ...
        if (!this.motion["PC combat hit down"]) {
          this.motion["PC combat hit down"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\ふっとび　全親無し.vmd",
            para: {
              adjust_center_view_disabled: true,
              duration: 2,
              auto_blink: false,
                  ...
                  skin_default: {
                    ...
                    グルーブ: {
                      keys_mod: [
                        ...
                        { frame: 23, pos: { x: 0, y: -12.0 + 1.5, z: 0 } },
                        { frame: 27, pos: { x: 0, y: -12.65 + 1.5, z: 0 } },
                        { frame: 30, pos: { x: 0, y: -12.3 + 1.5, z: 0 } },
              ...
              onended_NPC: function (model_index) {
                ...
                var model_para = MMD_SA_options.model_para_obj_all[model_index];
                model_para._motion_name_next =
                  that.motion["NPC-" + model_index + " combat get up"].name;
              },
              onended: function (natural_end) {
                MMD_SA._no_fading = true;
                if (!natural_end) return;
                ...
                MMD_SA_options.motion_shuffle_list_default = [
                  MMD_SA_options.motion_index_by_name[
                    that.motion["PC combat get up"].name
                  ],
                ];
                MMD_SA._force_motion_shuffle = true;
              },
        ...
        if (!this.motion["PC combat get up"]) {
          this.motion["PC combat get up"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\攻撃配布用\\起き上がり.vmd",
            para: {
              adjust_center_view_disabled: true,
              motion_command_disabled: true,
              super_armor: { level: 99 },
        ...
        if (!this.motion["PC combat victory"]) {
          this.motion["PC combat victory"] = {
            path: "MMD.js\\motion\\motion_rpg_pack01.zip#\\emote\\happy1.vmd",
            para: {
              adjust_center_view_disabled: true,
              motion_command_disabled: true,
              onended: function (natural_end) {
              ...
              adjustment_per_model: {
                _default_: {
                  skin_default: {
                    ...
                    センター: {
                      pos_scale: { auto_adjust: { ref_length: 11.65455 } },
                    },
                    左足ＩＫ: {
                      pos_scale: {
                        auto_adjust: {
                          ref_length: 11.65455 - 1.78831,
                          scale: 0.5,
                        },
                      },
                    },
              ...
              adjustment_per_model: {
                _default_: {
                  skin_default: {
                    ...
                    センター: {
                      pos_scale: { auto_adjust: { ref_length: 11.03743 } },
                    },
                    左足ＩＫ: {
                      pos_scale: {
                        auto_adjust: {
                          ref_length: 11.03743 - 1.3733,
                          scale: 0.5,
                        },
                      },
                    },
            ...
            para: {
              ...
              adjustment_per_model: {
                _default_: {
                  skin_default: {
                    全ての親: {
                      pos_add: { x: 15.6, y: 0, z: 0 },
                      rot_add: { x: 0, y: 90, z: 0 },
                    },
                    //如月千早カジュアル Ver.2.0a_簡易版.pmx
                    センター: {
                      pos_scale: { auto_adjust: { ref_length: 11.57427 } },
                    },
                    左足ＩＫ: {
                      pos_scale: {
                        auto_adjust: {
                          ref_length: 11.57427 - 1.5702,
                          scale: 0.5,
                        },
                      },
                    },
              ...
              adjustment_per_model: {
                _default_: {
                  skin_default: {
                    ...
                    //日高舞.pmx
                    センター: {
                      pos_scale: { auto_adjust: { ref_length: 11.67395 } },
                    },
                    左足ＩＫ: {
                      pos_scale: {
                        auto_adjust: {
                          ref_length: 11.67395 - 1.750509,
                          scale: 0.5,
                        },
                      },
                    },
              ...
              combat_para: [
                {
                  ...
                  SFX: {
                    ...
                    visual: {
                      hit: { sprite: [{ name: "explosion_red_01", scale: 2 }] },
                    },
                  },
                  bb_expand: { x: 0.5 * 3, y: 0, z: 0.5 * 3 },
                },
              ],
              ...
              SFX: [
                {
                  ...
                  sprite: [
                    {
                      bone_ref: "左足首",
                      name: "explosion_sinestesia-01_03",
                      scale: 3,
                      depth: 5,
                    },
                  ] /*, camera_shake:{magnitude:0.2,duration:500}*/,
                },
              ],
            },
          };
              ...
              combat_para: [
                {
                  ...
                  hit_level: 2,
                  SFX: { bone_to_pos: "右手首" },
                  bb_expand: { x: 0.5 * 2, y: 0, z: 0.5 * 2 },
                  bb_translate: { x: 0, y: 0, z: 0.5 * 2 },
                },
                {
                  frame_range: [41, 43],
                  hit_level: 3,
              ...
              combat_para: [
                {
                  ...
                  bb_expand: { x: 0.5 * 1, y: 0, z: 0.5 * 1 },
                  bb_translate: { x: 0, y: 0, z: 0.5 * 3 },
                  sound_name: "swing",
                },
                {
                  frame_range: [36, 39],
...
*/
      ...
      window.addEventListener("SA_MMD_init", function (e) {
        for (var name in that.motion) {
          ...
          motion.name = decodeURIComponent(
            (motion.path ? motion : MMD_SA_options.motion[motion.index]).path
              .replace(/^.+[\/\\]/, "")
              .replace(/\.vmd$/i, "")
          );
          ...
          if (motion.index != null) {
            if (motion.path) {
              MMD_SA_options.motion[motion.index] = {
                must_load: true,
                no_shuffle: true,
                path: motion.path,
              };
            ...
            let index = MMD_SA_options.motion.findIndex(function (obj) {
              return !obj || obj.path == motion.path;
            });
            ...
            } else motion.index = index;
          }

          let para = motion.para;
          if (para) {
            if (para.combat_para) {
              if (para.look_at_screen == null) para.look_at_screen = false;

              // define combat motion.duration to prevent looping
              if (para.motion_duration && !para.duration) {
                para.duration =
                  Math.max(10, para.motion_duration) + (para.duration_NPC || 0);
              }
              ...
              para.combat_para.forEach(function (p) {
                if (p.bb_expand) {
                  xz.forEach(function (_xz) {
                    p.bb_expand[_xz] =
                      (1 + p.bb_expand[_xz]) / _bb_xz_factor_ - 1;
                  });
                }
                if (p.bb_translate) {
                  xz.forEach(function (_xz) {
                    if (p.bb_translate[_xz])
                      p.bb_translate[_xz] =
                        p.bb_translate[_xz] / _bb_xz_factor_;
                  });
                }
              });
            }
        ...
        if (MMD_SA_options.model_path_extra) {
          ...
          MMD_SA_options.model_path_extra.forEach(function (path, idx) {
            ...
            var model_filename_cleaned = model_filename
              .replace(/[\-\_]copy\d+\.pmx$/, ".pmx")
              .replace(/[\-\_]v\d+\.pmx$/, ".pmx");
            var model_para =
              MMD_SA_options.model_para[model_filename] ||
              MMD_SA_options.model_para[model_filename_cleaned];
            if (!model_para)
            ...
            model_para.motion_included =
              model_para.motion_included || model_para.motion_includes;
            ...
            if (model_para.motion_included) {
              ...
              for (var name in that.motion) {
                ...
                if (
                  model_para.motion_included.indexOf(name) != -1 ||
                  model_para.motion_included.indexOf(motion.name) != -1
                ) {
            ...
            model_para.look_at_character =
              model_para.look_at_character == -1
                ? null
                : model_para.look_at_character || 0;
        ...
        NPC_motion_list.forEach(function (motion_name) {
          that.object_base_list.forEach(function (obj_base) {
            ...
            var path =
              MMD_SA_options.model_path_extra[obj_base.character_index - 1];
            var model_filename = path.replace(/^.+[\/\\]/, "");
            var model_filename_cleaned = model_filename
              .replace(/[\-\_]copy\d+\.pmx$/, ".pmx")
              .replace(/[\-\_]v\d+\.pmx$/, ".pmx");
            ...
            var NPC_motion_name = motion_name.replace(
              /^PC/,
              "NPC-" + obj_base.character_index
            );
            var NPC_motion_name_translated =
              (that.motion[NPC_motion_name] && NPC_motion_name) ||
              (model_para.motion_map &&
                model_para.motion_map[NPC_motion_name]) ||
              motion_name;
            that.motion[NPC_motion_name] =
              that.motion[NPC_motion_name_translated];

            if (!that.motion[NPC_motion_name].para.model_index_list)
              that.motion[NPC_motion_name].para.model_index_list = [];
            that.motion[NPC_motion_name].para.model_index_list.push(
              obj_base.character_index
            );
            ...
            if (/combat default/.test(NPC_motion_name)) {
              model_para.motion_name_default_combat =
                model_para.motion_name_default_combat ||
                that.motion[NPC_motion_name].path
                  .replace(/^.+[\/\\]/, "")
                  .replace(/\.vmd$/i, "");
              model_para.motion_name_default =
                model_para.motion_name_default ||
                model_para.motion_name_default_combat;
            }
          });
        ...
        window.addEventListener("MMDStarted", function (e) {
          var d = MMD_SA_options.Dungeon;
          d._motion_shuffle_list_default =
            MMD_SA_options.motion_shuffle_list_default.slice();
          d._motion_shuffle_list_default_combat = d.motion["PC combat default"]
            ? [
                MMD_SA_options.motion_index_by_name[
                  d.motion["PC combat default"].name || "PC combat default"
                ],
              ]
            : null;
          d._motion_shuffle_list_default_parry = d.motion["PC combat parry"]
            ? [
                MMD_SA_options.motion_index_by_name[
                  d.motion["PC combat parry"].name || "PC combat parry"
                ],
              ]
            : null;
        });
      ...
      for (var key in this.key_map) {
        ...
        if (index >= 0) ULDR_indexd[index] = true;
      }

      ULDR_id.forEach((id, idx) => {
        if (ULDR_indexd[idx]) return;

        var key_map = (that.key_map[ULDR_keyCode[idx]] = {
          order: 1000 + idx,
          id: id,
          type_movement: true,
        });
        ...
        switch (id) {
          case "up":
            ...
            Object.defineProperty(key_map, "mov_speed", {
              get: () => this.motion["PC movement forward"].para._speed,
            });
              ...
              mov_speed: [
                {
                  ...
                  speed: {
                    ...
                    y: 0,
                    get z() {
                      return MMD_SA_options.Dungeon.motion[
                        "PC movement forward"
                      ].para._speed;
                    },
                  },
        ...
        this.key_map[38] = {
          ...
          onfirstpress: (function () {
                ...
                case 1:
                  return v3a
                    .set(
                      ...
                      dc[2] *
                        2 *
                        MMD_SA_options.Dungeon_options.camera_position_z_sign
                    )
                    .toArray();
                case 2:
                  return v3a
            ...
            window.addEventListener("SA_Dungeon_onrestart", function () {
              ...
              c.camera_position_base_default = camera_position_preset(
                d.key_map[38].camera_position_preset_index
              );
              c.camera_position_base = c.camera_position_base_default.slice();
            });
            ...
            return function () {
              ...
              if (
                ++this.camera_position_preset_index >=
                camera_position_preset_length
              )
              ...
              c.camera_rotation_from_preset.y =
                Math.PI / 2 -
                Math.atan2(
                  tc.target.z - tc.object.position.z,
                  tc.target.x - tc.object.position.x
                );

              DEBUG_show(
                "Camera preset:" +
                  (this.camera_position_preset_index + 1) +
                  "/" +
                  camera_position_preset_length,
                2
              );
...
*/
            ...
            if (combat && combat._target_enemy_index >= 0) {
              combat._target_enemy_index = -1;
              if (!c.TPS_mode) MMD_SA.reset_camera(true);
            } else {
      ...
      (function () {
        function select_target(counter) {
          var c = that.character;
          if (!c.combat_mode) return;

          var combat = that._states.combat;
          if (
            !combat.enemy_list.some(function (enemy) {
              return enemy.hp;
            })
          )
            return;

          var target_enemy_index =
            combat._target_enemy_index == null
              ? -1
              : combat._target_enemy_index;
          var enemy;
          while (true) {
      ...
      this.key_map_reset = function () {
        ...
        for (var key in this.key_map) {
          ...
          this.key_map_by_id[k.id_default || key] = this.key_map_by_id[
            k.id || key
          ] = k;
          this.key_map_list.push(k);
        }
        this.key_map_list.sort(function (a, b) {
      ...
      if (!this.key_map[32] && !this.key_map_by_id["jump"]) {
        let _onfirstpress = function (e) {
          var keyCode =
            that.key_map_by_id["up"].down ||
            that.key_map_by_id["down"].down ||
            that.key_map_by_id["left"].down ||
            that.key_map_by_id["right"].down ||
            !e.detail.e.shiftKey
              ? 1320
              : 1321;
          ...
          var key_map =
            (that.key_map[32] =
            that.key_map_by_id["jump"] =
              that.key_map[keyCode]);
          key_map.keyCode = 32;
          key_map.id = "jump";
        ...
        let _onupdate = (function () {
          ...
          return function (para) {
            ...
            var model = THREE.MMD.getModels()[0];
            var motion_para =
              MMD_SA_options.Dungeon.motion[this.motion_id].para;
            ...
            if (frame >= 12 && frame < 12 + 22) {
              ...
              let playbackRate = (motion_para.playbackRate_by_model_index[0] =
                ((t_diff - _t_diff) / t_diff) * playbackRate_last +
                ((_t_diff / t_diff) * 1) / scale);
              if (playbackRate_last != playbackRate) {
                result.t2_extended =
                  (playbackRate - playbackRate_last) * t_diff;
              }
            ...
            motion_para.mov_speed[1] = {
              frame: 12,
              speed: { x: 0, y: va.v, z: (41 / 22) * 30 * scale },
              acceleration: { x: 0, y: va.a, z: 0 },
            };
...
*/
        ...
        let _ondown = function (e) {
          var c = MMD_SA_options.Dungeon.character;
          if (c.mount_para && !c.mount_para.can_jump) return true;
        };
        this.key_map[1320] = {
          order: 999,
          id: "jump_forward",
        ...
        (function () {
            ...
            MMD_SA_options.Dungeon_options.attack_combo_list = [
              {
                keyCode: 20085,
                combo_RE: "8,5",
                motion_id: "PC combat attack 01",
                plus_down: true,
              },
              {
                keyCode: 10044,
                combo_RE: "4,4",
                motion_id: "PC combat attack 02",
                combo_type: "kick",
              },
              {
                keyCode: 10077,
                combo_RE: "7,7",
                motion_id: "PC combat attack 03",
              },
              {
                keyCode: 14545,
                combo_RE: "45,45",
                motion_id: "PC combat attack 04",
                combo_type: "kick",
              },
              {
                keyCode: 10055,
                combo_RE: "5,5",
                motion_id: "PC combat attack 05",
                combo_type: "kick",
              },
              {
                keyCode: 10054,
                combo_RE: "5,4",
                motion_id: "PC combat attack 06",
                combo_type: "kick",
              },
              {
                keyCode: 20087,
                combo_RE: "8,7",
                motion_id: "PC combat attack 07",
                plus_down: true,
              },
              {
                keyCode: 14556,
                combo_RE: "45,56",
                motion_id: "PC combat attack 08",
                combo_type: "kick",
              },
              {
                keyCode: 20048,
                combo_RE: "4,8",
                motion_id: "PC combat attack 09",
                plus_down: true,
              },
              {
                keyCode: 10087,
                combo_RE: "8,7",
                motion_id: "PC combat attack 10",
              },
              {
                keyCode: 10456,
                combo_RE: "456",
                motion_id: "PC combat attack 11",
                combo_type: "kick",
              },
              {
                keyCode: 10078,
                combo_RE: "7,8",
                motion_id: "PC combat attack 12",
              },
              {
                keyCode: 10047,
                combo_RE: "4,7",
                motion_id: "PC combat attack 13",
              },
              {
                keyCode: 20075,
                combo_RE: "7,5",
                motion_id: "PC combat attack 14",
                plus_down: true,
              },
              {
                keyCode: 17878,
                combo_RE: "78,78",
                motion_id: "PC combat attack 15",
              },
              {
                keyCode: 20077,
                combo_RE: "7,7",
                motion_id: "PC combat attack 16",
                plus_down: true,
              },
              {
                keyCode: 10088,
                combo_RE: "8,8",
                motion_id: "PC combat attack 17",
              },
              {
                keyCode: 20045,
                combo_RE: "4,5",
                motion_id: "PC combat attack 18",
                plus_down: true,
              },
              {
                keyCode: 15656,
                combo_RE: "56,56",
                motion_id: "PC combat attack 19",
                combo_type: "kick",
              },

              {
                keyCode: 50000,
                combo_RE: "5,5",
                motion_id: "PC combat attack 2-handed weapon 01",
                combo_type: "2-handed",
              },

              {
                keyCode: 10022,
                combo_RE: "2,2",
                motion_id: "PC combat attack twin weapon 01",
                combo_type: "twin",
              },
              {
                keyCode: 10021,
                combo_RE: "2,1",
                motion_id: "PC combat attack twin weapon 02",
                combo_type: "twin",
              },
              {
                keyCode: 11212,
                combo_RE: "12,12",
                motion_id: "PC combat attack twin weapon 03",
                combo_type: "twin",
              },
              {
                keyCode: 12323,
                combo_RE: "23,23",
                motion_id: "PC combat attack twin weapon 04",
                combo_type: "twin",
              },
              {
                keyCode: 20022,
                combo_RE: "2,2",
                motion_id: "PC combat attack twin weapon 05",
                plus_down: true,
                combo_type: "twin",
              },
              {
                keyCode: 22323,
                combo_RE: "23,23",
                motion_id: "PC combat attack twin weapon 06",
                plus_down: true,
                combo_type: "twin",
              },
              {
                keyCode: 10012,
                combo_RE: "1,2",
                motion_id: "PC combat attack twin weapon 07",
                combo_type: "twin",
              },
              {
                keyCode: 10123,
                combo_RE: "123",
                motion_id: "PC combat attack twin weapon 08",
                combo_type: "twin",
              },

              {
                keyCode: 30012,
                combo_RE: "1,2",
                motion_id: "PC combat attack 1-handed weapon 01",
                combo_type: "1-handed slash",
              },
              {
                keyCode: 31212,
                combo_RE: "12,12",
                motion_id: "PC combat attack 1-handed weapon 02",
                combo_type: "1-handed slash",
              },
          ...
          var combo_onfirstpress = function (e) {
            ...
            if (combo_str.length) {
              ...
              if (c_last_array.indexOf(c_str) != -1) combo_str += "," + c_str;
              else if (
                c_last_array.some(function (digit) {
                  return !d.key_map[digit + 96].is_down;
                })
              )
                combo_str += "," + c_str;
              else {
                ...
                combo_str = combo_str.replace(
                  /\d+$/,
                  c_last_array.sort().join("")
                );
              }
            } else combo_str += c_str;
            ...
            if (!para_SA.motion_duration_by_combo) {
              let combos =
                MMD_SA_options.Dungeon_options.attack_combo_list.filter(
                  function (combo) {
                    if (
                      combo.combo_type != "free" &&
                      !c.combat_stats.weapon.combo_type_RE.test(
                        combo.combo_type
                      )
                    )
                      return false;

                    let _RE = MMD_SA_options.Dungeon_options
                      .simple_combat_input_mode_enabled
                      ? combo._RE_simple
                      : combo._RE;
              ...
              if (combos.length) {
                ...
                if (motion_para.motion_duration_by_combo) {
                  key_map.motion_duration = motion_para.motion_duration =
                    motion_para.motion_duration_by_combo[
                      motion_para.motion_duration_by_combo.length - 1
                    ].motion_duration;
            ...
            if (!para_SA.motion_duration_by_combo || !combo_str.length) return;
            ...
            para_SA.motion_duration_by_combo.some(function (combo) {
              ...
              if (!_RE || combo_str.search(_RE) == 0) {
                if (combo.motion_duration > motion_duration) {
                  key_map.motion_duration = para_SA.motion_duration =
                    combo.motion_duration;
...
*/
          var combo_keys = [107];
          for (var k = 97; k < 96 + 10; k++) combo_keys.push(k);
        ...
        if (!this.key_map[96] && !this.key_map_by_id["combat_parry"]) {
          this.key_map[96] = this.key_map_combat[this.key_map_combat.length] = {
            order: 950,
            id: "combat_parry",
            type_movement: true,
            keyCode: 96,
            TPS_mode: { no_rotation: true },
            key_id_cancel_list: ["jump"],
            onupdate: function () {
              var d = MMD_SA_options.Dungeon;
              var parry_mode =
                d.key_map[87].motion_id == "PC combat movement forward";

              var TPS_mode;

              if (this.is_down) {
                if (!d.character.combat_mode) MMD_SA.reset_camera();
                if (!parry_mode && d.can_parry) {
                  MMD_SA_options.motion_shuffle_list_default =
                    MMD_SA_options.Dungeon._motion_shuffle_list_default_parry.slice();
                  MMD_SA_options._motion_shuffle_list_default =
                    MMD_SA_options.motion_shuffle_list_default.slice();
        ...
        ULDR_id.forEach(function (id, idx) {
          ...
          if (
            that.key_map_parry.some(function (kmc) {
              return kmc.keyCode == keyCode;
            })
          )
            return;

          that.key_map_combat[that.key_map_combat.length] =
            that.key_map_by_id[id];

          var key_map = (that.key_map_parry[that.key_map_parry.length] = {
            order: 1000 + idx,
            id: id,
            keyCode: keyCode,
            about_turn: false,
          ...
          switch (id) {
            case "up":
              key_map.motion_id = key_map.TPS_mode.motion_id =
                "PC combat movement forward";
              that.motion[key_map.motion_id].para.mov_speed = [
                { frame: 0, speed: { x: 0, y: 0, z: 0.5 * 30 } },
              ];
              break;
            case "down":
              key_map.motion_id = key_map.TPS_mode.motion_id =
                "PC combat movement backward";
              that.motion[key_map.motion_id].para.mov_speed = [
                { frame: 0, speed: { x: 0, y: 0, z: -0.5 * 30 } },
              ];
              break;
            case "left":
              key_map.motion_id = key_map.TPS_mode.motion_id =
                "PC combat movement left";
              that.motion[key_map.motion_id].para.mov_speed = [
                { frame: 0, speed: { x: 0.5 * 30, y: 0, z: 0 } },
              ];
              break;
            case "right":
              key_map.motion_id = key_map.TPS_mode.motion_id =
                "PC combat movement right";
              that.motion[key_map.motion_id].para.mov_speed = [
                { frame: 0, speed: { x: -0.5 * 30, y: 0, z: 0 } },
              ];
              break;
      ...
      this.key_map_swap = function (key_map) {
        if (!Array.isArray(key_map)) {
          key_map = Object.keys(key_map).map(function (name) {
            return key_map[name];
          });
        }
      ...
      if (!MMD_SA_options.x_object) MMD_SA_options.x_object = [];
      ...
      this.object_base_list.forEach(function (obj, idx) {
        ...
        if (obj.character_index) {
          ...
          if (obj.path) {
            if (c && c.model_para) {
              const model_filename_cleaned = obj.path
                .replace(/^.+[\/\\]/, "")
        ...
        if (c && !obj.path) {
          if (c.GOML_head) MMD_SA_options.GOML_head += c.GOML_head;
          if (c.GOML_scene) MMD_SA_options.GOML_scene += c.GOML_scene;
          if (c.mesh_obj) MMD_SA_options.mesh_obj.push(c.mesh_obj);
          //console.log(MMD_SA_options.GOML_head)
          //console.log(MMD_SA_options.GOML_scene)
          c.build && c.build();
          return;
        ...
        if (c && c.model_para && obj.path) {
          let model_filename_cleaned = obj.path
            .replace(/^.+[\/\\]/, "")
            .replace(/[\-\_]copy\d+\.x$/, ".x")
            .replace(/[\-\_]v\d+\.x$/, ".x");
          MMD_SA_options.model_para[model_filename_cleaned] = c.model_para;
        ...
        MMD_SA_options.x_object.push({
          ...
          bb_adjust: obj.collision_by_mesh_enforced
            ? { min: { x: 0, y: -10, z: 0 } }
            : null,
        });
      });
    ...
    get event_mode() {
      return (
        this._states.event_mode_locked ||
        this._states.event_mode ||
        this._states.dialogue_mode
      );
    },
    set event_mode(v) {
      this._states.event_mode = v;
    },

    get dialogue_branch_mode() {
      return this._states.dialogue_branch_mode;
    },
    ...
    get character_movement_disabled() {
      return (
        this._states.character_movement_disabled ||
        (this.event_mode && !this._states.action_allowed_in_event_mode) ||
        this.character_combat_locked ||
        MMD_SA_options.Dungeon_options.character_movement_disabled
      );
    },
    set character_movement_disabled(v) {
      this._states.character_movement_disabled = v;
    },

    get can_parry() {
      if (!this.character.combat_mode) return false;
      if (this.character_combat_locked) return false;
    ...
    set character_combat_locked(v) {
      this._states.character_combat_locked = v;
    },

    get object_click_disabled() {
      return (
        this._states.object_click_disabled ||
        this.event_mode ||
        (MMD_SA.WebXR.reticle && MMD_SA.WebXR.reticle.visible)
      );
    },
    set object_click_disabled(v) {
      this._states.object_click_disabled = v;
    },
    ...
    check_states: (function () {
      function check_hp(c) {
        if (c.hp == 0) {
          ...
          let motion_id =
            d.motion_id_by_filename[MMD_SA.MMD.motionManager.filename] || "";
          if (!/^(PC combat hit down|PC down)$/.test(motion_id)) {
            MMD_SA_options.motion_shuffle_list_default = [
              MMD_SA_options.motion_index_by_name[d.motion["PC down"].name],
            ];
            MMD_SA._force_motion_shuffle = true;
            return true;
          }
      ...
      return function () {
        ...
        if (s.auto_damage && !s.dialogue_mode) {
          var damage =
            (Math.min((s.auto_damage.t || t) - t, 100) / 1000) *
            s.auto_damage.damage;
          s.auto_damage.t = t;
    ...
    events_default: {
      _PLAYER_MANUAL_: [
        ...
        [
          ...
          },
          {
            message: {
              content:
                '(*) - In default control mode, move the player character (PC) with WS keys, and rotate (PC and camera) with AD keys. In "TPS Control Mode", all WASD keys move PC with camera angle fixed. To change movement direction and camera angle, drag the mouse pointer.',
              bubble_index: 3,
              para: { scale: 1.5, font_scale: 1 },
            },
          },
        ...
        [
          ...
          },
          {
            message: {
              content:
                "(*) - Attacks are combinations of different Numpad keys.\nExample: 5,5,4,6\nSome attack combos required the + key pressed.\nExample: +8,5,9",
              bubble_index: 3,
              para: { scale: 1.5 },
            },
          },
      ...
      _SETTINGS_: [
        ...
        [
          {
            func: function () {
              MMD_SA_options.MME.self_overlay = {
                enabled: true,
                opacity: 0.2,
                brightness: 1,
              };
              MMD_SA_options.MME.HDR = { enabled: true, opacity: 0.2 };
              MMD_SA_options.MME.serious_shader = {
                enabled: true,
                type: "AdultShaderS2",
                OverBright: 1.3,
                shadow_opacity: 0.4,
        ...
        [
          {
            func: function () {
              ...
              DEBUG_show(
                "Shadow:" + ((MMD_SA_options.use_shadowMap && "ON") || "OFF"),
                3
              );
            },
            ended: true,
            ...
            func: (function () {
              ...
              return function () {
                ...
                if (edgeScale) {
                  ...
                  edgeScale = 0;
                } else edgeScale = edgeScale_default;
                jThree.MMD.edgeScale = edgeScale;
                DEBUG_show(
                  "Model Outline:" + ((edgeScale && "ON") || "OFF"),
                  3
                );
              };
            })(),
        ...
        [
          {
            func: function () {
              ...
              MMD_SA._renderer.devicePixelRatio = is_default_res
              ...
              DEBUG_show(
                "3D Resolution:" +
                  ((is_default_res &&
                    Math.round(
                      (MMD_SA._renderer.devicePixelRatio /
                        window.devicePixelRatio) *
                        100
                    ) + "%") ||
                    "100%"),
        ...
        [
          {
            message: {
              get content() {
                if (System._browser.overlay_mode)
                  return (
                    System._browser.translation.get(
                      "Dungeon.UI.tome.settings.UI_and_overlays.user_interface.UI_off"
                    ) +
                    "\n1. " +
                    (System._browser.overlay_mode == 2
                      ? "UI: OFF + " +
                        System._browser.translation.get(
                          "Dungeon.UI.tome.settings.UI_and_overlays.user_interface.UI_off.green_screen"
                        )
                      : System._browser.translation.get(
                          "Dungeon.UI.tome.settings.UI_and_overlays.user_interface"
                        ) + ": OFF") +
                    "\n2. " +
                    System._browser.translation.get("Misc.done")
                  );
                return (
                  "1. " +
                  System._browser.translation.get(
                  ...
                  ": ON" +
                  (MMD_SA_options.user_camera.ML_models.enabled &&
                  System._browser.overlay_mode == 0
                    ? "\n2. " +
                      System._browser.translation.get(
                        "Dungeon.UI.tome.settings.UI_and_overlays.camera_display"
                      ) +
                      ": " +
                      (MMD_SA_options.user_camera.display.video.hidden
                        ? "OFF"
                        : MMD_SA_options.user_camera.display.video.hidden ==
                          null
                        ? System._browser.translation.get(
                            "Dungeon.UI.tome.settings.UI_and_overlays.camera_display.non_webcam"
                          )
                        : "ON") +
                      "\n3. " +
                      System._browser.translation.get(
                        "Dungeon.UI.tome.settings.UI_and_overlays.wireframe_display"
                      ) +
                      ": " +
                      (MMD_SA_options.user_camera.display.wireframe.hidden
                        ? "OFF"
                        : "ON") +
                      "\n4. " +
                      System._browser.translation.get(
                        "Dungeon.UI.tome.settings.UI_and_overlays.mocap_debug_display"
                      ) +
                      ": " +
                      ...
                      "\n5. " +
                      System._browser.translation.get(
                        "Dungeon.UI.tome.settings.UI_and_overlays.UI_sound_effects"
                      ) +
                      ": " +
                      (MMD_SA_options.Dungeon.inventory.UI.muted
                        ? "OFF"
                      ...
                      "\n6. " +
                      System._browser.translation.get(
                        "Dungeon.UI.tome.settings.UI_and_overlays.UI_language"
                      ) +
                      ": " +
                      System._browser.translation.language_info +
                      "\n7. " +
                      System._browser.translation.get("Misc.done")
              ...
              bubble_index: 3,
              get branch_list() {
                return [{ key: 1, branch_index: 12 }].concat(
                  MMD_SA_options.user_camera.ML_models.enabled &&
                    System._browser.overlay_mode == 0
                    ? [
        ...
        [
          {
            func: function () {
              MMD_SA_options.user_camera.display.video.hidden =
                MMD_SA_options.user_camera.display.video.hidden == null
                  ? false
                  : MMD_SA_options.user_camera.display.video.hidden
                  ? null
                  : true;
        ...
        [
          {
            func: function () {
              ...
              let index = language.indexOf(
                System._browser.translation.language_full
              );
              if (index == -1) {
                index = 1;
        ...
        [
          {
            func: function () {
              ...
              if (xr.session) {
                DEBUG_show(
                  "(This option cannot be changed during WebXR mode.)",
                  3
                );
                return;
              }

              AR_options.dom_overlay.enabled =
                AR_options.dom_overlay.enabled !== false ? false : true;
              DEBUG_show(
                "DOM Overlay:" +
                  ((AR_options.dom_overlay.enabled && "ON") || "OFF"),
                3
              );
            },
            ended: true,
        ...
        [
          {
            func: function () {
              ...
              AR_options.light_estimation_enabled =
                AR_options.light_estimation_enabled !== false ? false : true;
              DEBUG_show(
                "Light Estimation:" +
                  ((AR_options.light_estimation_enabled && "ON") || "OFF"),
                3
              );
            },
            ended: true,
        ...
        [
          {
            func: function () {
              ...
              AR_options.anchors_enabled =
                AR_options.anchors_enabled !== false ? false : true;
              DEBUG_show(
                "Anchors:" + ((AR_options.anchors_enabled && "ON") || "OFF"),
                3
              );
            },
            ended: true,
        ...
        [
          {
            func: function () {
              ...
              DEBUG_show(
                "Framebuffer Scale:x" +
                  (System._browser.url_search_params.xr_fb_scale || 1),
                3
              );
            },
            ended: true,
        ...
        [
          {
            func: (function () {
              ...
              return function () {
                show_fps = !show_fps;
                if (show_fps) {
                  if (!timerID) timerID = requestAnimationFrame(fps);
                } else {
        ...
        [
          {
            message: {
              ...
              bubble_index: 3,
              branch_list: [{ key: 1, branch_index: 11 }, { key: 2 }],
            },
          },
        ],
        // 11
        [
          {
            func: function () {
              ...
              AR_options.dom_overlay.use_dummy_webgl =
                !AR_options.dom_overlay.use_dummy_webgl;
              DEBUG_show(
                "Dummy WebGL Layer:" +
                  ((AR_options.dom_overlay.use_dummy_webgl && "ON") || "OFF"),
                3
              );
            },
            ended: true,
                ...
                if (mf) {
                  ...
                  for (const morph_name in mf) {
                    ...
                    if (_m_idx != null) {
                      ...
                      MMD_SA._custom_morph.push({
                        key: {
                          weight: mf[morph_name],
                          morph_type: _m.type,
                          morph_index: _m_idx,
                        },
                        idx: model.morph.target_index_by_name[morph_name],
                      });
                    }
                  }
                }
        ...
        [
          {
            func: function () {
              ...
              DEBUG_show(
                "WASM-SIMD:" + (System._browser.use_WASM_SIMD ? "ON" : "OFF"),
                3
              );
            },
            ended: true,
        ...
        [
          {
            message: {
              ...
              para: { pos_mod: [0, 15, 0] },
              bubble_index: 3,
              branch_list: [{ key: 1, branch_index: 1 }, { key: 2 }],
            },
          },
        ],
        //1
    ...
    run_event: (function () {
      function statement(statement_obj) {
        if (statement_obj._if) {
          if (condition.call(this, statement_obj._if)) {
            if (statement_obj._then) statement.call(this, statement_obj._then);
          } else {
            if (statement_obj._else) statement.call(this, statement_obj._else);
          }
        }
      ...
      function condition(condition_obj) {
        ...
        if (c) {
          ...
          if (typeof c[0] == "string" || c[0][1] == this.area_id) {
            ...
            var event_flag =
              MMD_SA_options.Dungeon_options.options_by_area_id[c[0][0]]._saved
                .event_flag || {};
            e = event_flag[c[0][1]];
          }
          switch (c[1]) {
      ...
      Combat.prototype.ondefeated = (function () {
        ...
        function onmotionended(e) {
          MMD_SA_options.motion_shuffle_list_default = [
            MMD_SA_options.motion_index_by_name[
              MMD_SA_options.Dungeon.motion["PC combat victory"].name
            ],
          ];
          MMD_SA._no_fading = true;
          e.detail.result.return_value = true;
          window.removeEventListener(
            "SA_MMD_model0_onmotionended",
            onmotionended
          );
        }
        ...
        return function (character_index) {
          ...
          if (!this.enemy_list.length) return false;

          var d = MMD_SA_options.Dungeon;

          if (character_index == 0) {
            if (d.character.hp == 0) {
              if (this.onplayerdefeated && this.onplayerdefeated()) return true;
          ...
          var c = d.object_base_list.find(function (obj_base) {
            return obj_base.character_index == character_index;
          }).object_list[0];

          var defeated;
          if (c && c.hp == 0) {
            if (c.ondefeated) {
              if (c.ondefeated()) return true;
            } else if (this.onenemydefeated) {
              if (this.onenemydefeated(c)) return true;
            }
            defeated = true;
          ...
          if (
            this.enemy_list.every(function (enemy) {
              return enemy.hp == 0;
            })
          ) {
            defeated = true;
            if (
              this.onenemyalldefeated &&
              this.onenemyalldefeated(onenemyalldefeated_default)
            ) {
              return true;
            }
      ...
      function event_main(e) {
        ...
        if (sb.visible && sb.msg_timerID && this._states.dialogue_mode) {
          sb.hide();
          if (sb_index == 0) that._states.dialogue_mode = false;
        }
            ...
            const func = function () {
              if (msg.content) {
                ...
                if (c.mount_para && c.mount_para.msg_para) {
                  ...
                  para.scale =
                    (c.mount_para.msg_para.scale || 1) * (para.scale || 1);
                  const _pos_mod = c.mount_para.msg_para.pos_mod || [0, 0, 0];
                  para.pos_mod = para.pos_mod
                    ? para.pos_mod.map(function (v, idx) {
                        return v + _pos_mod[idx];
                      })
                    : _pos_mod;
                }
                ...
                if (msg.NPC) {
                  para.head_pos =
                    (_obj && _obj._obj.position.clone()) ||
                    THREE.MMD.getModels()[msg.NPC].mesh.position.clone();
                  para.head_pos.y += 20;
                }

                if (msg.branch_list) {
                  msg.branch_list.forEach((b) => {
                    if (b.sb_index == null) b.sb_index = index;
                  });
                  that.dialogue_branch_mode = msg.branch_list;
                }

                MMD_SA.SpeechBubble.list[index].message(
                  bubble_index,
                  msg.content,
                  duration,
                  para
                ...
                if (index == 0) that._states.dialogue_mode = false;
              }
            };
        ...
        if (e.turn_to_character) {
          var target_pos =
            (obj &&
              obj._obj.position &&
              typeof e.turn_to_character != "number") ||
            THREE.MMD.getModels()[e.turn_to_character].mesh.position;
          c.rot.y =
            Math.PI / 2 -
            Math.atan2(target_pos.z - c.pos.z, target_pos.x - c.pos.x);
          c.about_turn = false;
          THREE.MMD.getModels()[0].mesh.quaternion.setFromEuler(c.rot);
          c.pos_update();
        }
        ...
        if (e.look_at_character) {
          ...
          if (
            obj &&
            obj.cache_index &&
            typeof e.look_at_character != "number"
          ) {
            ...
            MMD_SA_options.model_para_obj_all[0].look_at_character =
              e.look_at_character;
        }

        if (e.NPC_turns_to_you) {
          var npc =
            (obj && obj._obj && typeof e.NPC_turns_to_you != "number") ||
            THREE.MMD.getModels()[e.NPC_turns_to_you].mesh;
          this._event_active._NPC_turns_back = {
            npc: npc,
            quat: npc.quaternion.clone(),
          };
          MMD_SA.TEMP_v3.set(
            0,
            Math.PI / 2 -
              Math.atan2(c.pos.z - npc.position.z, c.pos.x - npc.position.x),
            0
          );
          npc.quaternion.setFromEuler(MMD_SA.TEMP_v3);
        }
        ...
        if (e.swap_PC) {
          if (typeof swap_PC == "number") {
            ...
            if (/^object(\d+)_(\d+)$/.test(id)) {
              c.swap_character(
                this.object_base_list[parseInt(RegExp.$1)].object_list[
                  parseInt(RegExp.$2)
                ]
              );
            } else {
        ...
        if (e.set_starting_position) {
          ...
          var area_saved =
            MMD_SA_options.Dungeon_options.options_by_area_id[area_id]._saved;
          var starting_position = e.set_starting_position.position;
          if (!starting_position) area_saved.starting_position = null;
          else if (typeof starting_position == "string")
            area_saved.starting_position = c.pos.clone();
          else area_saved.starting_position = starting_position;
        }
          ...
          for (var id in e.follow_PC) {
            ...
            this.PC_follower_list.push({
              obj: {
                ...
                follow_PC_ground_normal:
                  obj_base && obj_base.use_PC_ground_normal_when_following,
              },
            });
          }
        }
        ...
        if (objects) {
          ...
          for (var id in objects) {
            ...
            if (p) {
              if (p.position) {
                ...
                if (pos.grid) {
                  ...
                  pos.add(
                    new THREE.Vector3(
                      (pos.grid.x + 0.5) * this.grid_size,
                      0,
                      (pos.grid.y + 0.5) * this.grid_size
                    ).add(pos)
                  );
                }
                if (pos.center) {
                  let center_id =
                    this.object_id_translated[pos.center.id] || pos.center.id;
                  if (center_id == "PC")
                  ...
                  if (pos.center.offset) {
                    ...
                    if (pos.center.offset_rotation) {
                      ...
                      else offset.applyEuler(center_mesh.rotation);
                      offset.applyEuler(
                        MMD_SA._v3b
                          .copy(pos.center.offset_rotation)
                          .multiplyScalar(Math.PI / 180)
                      );
                    }
                ...
                if (pos_center) pos.add(pos_center);
                if (p.behind_camera) {
                  var camera = MMD_SA._trackball_camera;
                  MMD_SA.TEMP_v3.copy(camera.object.position)
                    .sub(camera.target)
                    .setY(0);
                  var a =
                    -Math.atan2(MMD_SA.TEMP_v3.z, MMD_SA.TEMP_v3.x) +
                    Math.PI / 2;
                  //console.log(MMD_SA.TEMP_v3.toArray().join(",")+"/"+a)
                  rot_mod = MMD_SA._v3a.set(0, a, 0);
                  Object.assign(
                    pos,
                    MMD_SA.TEMP_v3.set(pos.x, 0, pos.z)
                      .applyEuler(rot_mod)
                      .add(camera.object.position)
                  );
                }
                if (pos.grounded) {
                  var x = ~~(pos.x / this.grid_size);
              ...
              if (p.rotation) {
                var rot = MMD_SA.TEMP_v3.copy(p.rotation).multiplyScalar(
                  Math.PI / 180
                );
                if (center_mesh) {
                  if (center_mesh.useQuaternion) {
                    //            rot.add(MMD_SA._v3b.setEulerFromQuaternion(center_mesh.quaternion))
                  } else rot.add(center_mesh.rotation);
                }
                if (rot_mod) rot.add(rot_mod);
                if (character_index == 0) {
                  c.about_turn = false;
                  c.rot.copy(rot);
                  mesh.quaternion.setFromEuler(rot);
                } else {
                  if (mesh.useQuaternion) mesh.quaternion.setFromEuler(rot);
                ...
                if (
                  mesh.useQuaternion &&
                  center_mesh &&
                  center_mesh.useQuaternion
                )
                  mesh.quaternion.multiply(center_mesh.quaternion);
            ...
            if (zom) {
              ...
              _obj.zone_of_movement
                .copy(zom)
                .translate(zom.center_position || pos_center || mesh.position);
            }
            ...
            if (
              ...
              id != "PC" &&
              _obj.hp > 0 &&
              this._states.combat.enemy_list.indexOf(_obj) == -1
            ) {
              this._states.combat.enemy_list.push(_obj);
              if (this._states.combat.show_HP_bar) {
                this.sprite.display(
                  new this.sprite.TextureObject_HP_bar(++enemy_index),
                  {
                    pos_target: {
                      mesh: mesh,
                      offset: new THREE.Vector3(
                        0,
                        mesh.geometry.boundingBox.max.y * mesh.scale.y + 2,
                        0
                      ),
                    },
                    get_value: (function () {
                      ...
                      return function () {
                        ...
                        if (combat)
                          para.border_color =
                            para.index == combat._target_enemy_index
                              ? "white"
                              : "black";
                        return para;
                      };
        ...
        if (e.motion) {
          for (var index in e.motion) {
            ...
            if (index == 0) {
              if (!(motion instanceof Array)) motion = [motion];
              var motion_name0 = motion[0].name;
              var loop = motion[0].loop;
              if (
                motion_name0 &&
                ((this.motion[motion_name0] &&
                  this.motion[motion_name0].name) ||
                  MMD_SA_options.motion_index_by_name[motion_name0])
              ) {
                var motion_list = motion.map(function (m, idx) {
                  var motion_name = motion[idx].name;
                  return MMD_SA_options
                    .motion_index_by_name[(that.motion[motion_name] && that.motion[motion_name].name) || motion_name];
                });
                if (loop) {
              ...
              if (model_para)
                model_para._motion_name_next =
                  (motion.name &&
                    ((this.motion[motion.name] &&
                      this.motion[motion.name].name) ||
                      motion.name)) ||
                  model_para.motion_name_default;
        ...
        if (e.inventory) {
          ...
          if (inv.onsuccess) {
            ...
            MMD_SA.SpeechBubble.message(
              3,
              'You found "' +
                this.item_base[inv.item_id].info_short +
                '" x' +
                inv.stock +
                ".",
              0
            ...
            if (!this.events[this._event_active.id][bi])
              this.events[this._event_active.id][bi] = [
                { message: { bubble_index: 3, content: "You found nothing." } },
              ];
            this._event_active.index = 999;
            this._states.dialogue_mode = true;
          }
        ...
        if (e.mount) {
          if (e.mount.target) {
            let id =
              this.object_id_translated[e.mount.target] || e.mount.target;
            if (/^object(\d+)_(\d+)$/.test(id)) {
              e.mount.target =
                this.object_base_list[parseInt(RegExp.$1)].object_list[
                  parseInt(RegExp.$2)
        ...
        if (e.sound) {
          ...
          switch (sound.action) {
            case "play":
              player_obj = this.sound.audio_object_by_name[sound.name].play(
                obj_parent,
                sound.spawn_id
              );
              break;
            case "pause":
              player_obj = this.sound.audio_object_by_name[
                sound.name
              ].get_player_obj(obj_parent, sound.spawn_id);
              if (player_obj) player_obj.player.pause();
              break;
          }
          if (player_obj) {
            if (sound.volume) player_obj.player.volume = sound.volume;
          }
        }
        ...
        if (e.combat) {
          if (e.combat.enabled != null) {
            ...
            if (
              e.combat.enabled != c.combat_mode &&
              (!e.combat.enabled ||
                !e.combat.cooling_time ||
                !e.combat.ended_timestamp ||
                Date.now() >
                  e.combat.ended_timestamp + e.combat.cooling_time * 1000)
            ) {
              if (e.combat.enabled) {
              ...
              event_main.call(
                this,
                e.combat.onstateunchange || { ended: true }
              );
            }
          }
        }
        ...
        if (e.camera_focus) {
          ...
          c.rot.y =
            Math.PI / 2 -
            Math.atan2(e.camera_focus.z - c.pos.z, e.camera_focus.x - c.pos.x);
          THREE.MMD.getModels()[0].mesh.quaternion.setFromEuler(c.rot);
          c.about_turn = false;
          MMD_SA.reset_camera(true);
        }
        ...
        if (e.set_event_flag) {
          if (
            e.set_event_flag.area_id &&
            e.set_event_flag.area_id != this.area_id
          ) {
            var _saved =
            ...
            _saved.event_flag[e.set_event_flag.id || this._event_active.id] =
              e.set_event_flag.branch_index;
          } else
            this.event_flag[e.set_event_flag.id || this._event_active.id] =
              e.set_event_flag.branch_index;
        }
        ...
        if (e.load_area) {
          this.restart(
            e.load_area.id,
            e.load_area.refresh_state != null ? e.load_area.refresh_state : 2
          );
        }
        ...
        if (e.goto_event) {
          this.run_event(
            e.goto_event.id,
            e.goto_event.branch_index,
            e.goto_event.step || e.goto_event.event_index || 0
          );
        }
        ...
        if (e.next_step) {
          if (e.next_step.delay != null)
            ...
            }, e.next_step.delay * 1000);
          else this.run_event();
        }

        if (e.ended) {
          if (typeof e.ended == "string" && e.ended != this._event_active.id)
            return;
          ...
          if (sb_index == 0) {
            MMD_SA.SpeechBubble.list.forEach((sb) => {
              if (sb.visible && !sb.msg_timerID) sb.hide();
            });

            if (this._event_active._NPC_turns_back)
              this._event_active._NPC_turns_back.npc.quaternion.copy(
                this._event_active._NPC_turns_back.quat
              );

            this._states.event_mode =
              this._states.dialogue_mode =
              this.dialogue_branch_mode =
                false;
            this._event_active = {};
      ...
      return function (event_id, branch_index, event_index) {
        if (!event_id) event_id = this._event_active.id;
        else if (Array.isArray(event_id)) {
          this.events["_ONETIME_"] = event_id;
          event_id = "_ONETIME_";
          if (branch_index == null) branch_index = 0;
          if (event_index == null) event_index = 0;
        ...
        if (branch_index == null) branch_index = this.event_flag[event_id];
        ...
        if (event_index == null) event_index = this._event_active.index || 0;
    ...
    SFX_check: (function () {
      ...
      return function (model, para_SA, animation, dt) {
        ...
        para_SA.SFX.some(function (obj) {
          ...
          if (!(obj.frame > frame0 && obj.frame <= frame1)) return false;
          ...
          if (sound) {
            ...
            if (!name) {
              ...
              let footstep_sound =
                that.get_para(x, y, true).footstep_sound ||
                model_para.footstep_sound ||
                footstep_sound_default;
    ...
    get sound() {
      return MMD_SA.Audio3D;
    },

    get sprite() {
      return MMD_SA.Sprite;
    },
    ...
    multiplayer: (function () {
      ...
      if (self.ChatboxAT && !Chatbox_intro_msg) {
        Chatbox_intro_msg =
          "<p>Anime Theme World Online 3D - Chatbox Mini (" +
          ChatboxAT.Chatbox_version +
          ")</p>\n" +
      ...
      var peer_para_default = {
        events: {
          peer: {
            open: function (peer) {
              ChatboxAT.smallMsg(
                "(P2P network: Peer initialized successfully)"
              );

              if (parent.System._browser.url_search_params.host_peer_id) {
                MMD_SA_options.Dungeon.multiplayer.connect(
                  parent.System._browser.url_search_params.host_peer_id,
                  { onconnect: _init_func, onerror: _init_func }
                );
              } else {
                _init_func();
              }
            ...
            error: function (peer, err) {
              ChatboxAT.smallMsg(
                "(P2P network: Peer error / " + err.type + ")"
              );
            },
          },
          connection: {
            handshake_request: function (peer, connection) {
              console.log(
                "P2P_network: Remote Peer" +
                  "(" +
                  connection.peer +
                  "/" +
                  connection.label +
                  "/host) responding handshake request from Peer-" +
                  peer.index +
                  "(" +
                  peer.id +
                  ")"
              ...
              connection.send({
                handshake: {
                  request: true,
                  para: {
                    game_id: d_options.game_id,
                    game_version: d_options.game_version,
                    chapter_id: d_options.chapter_id,
                  },
                },
              });
            },
            handshake_respond: function (peer, connection, handshake) {
              ...
              if (handshake.request) {
                // accept or reject
                var console_msg_rejected =
                  "P2P_network: Remote Peer" +
                  "(" +
                  connection.peer +
                  "/" +
                  connection.label +
                  "/host) rejected handshake request from Peer-" +
                  peer.index +
                  "(" +
                  peer.id +
                  ")";
                ...
                if (
                  !handshake.para ||
                  d_options.game_id != handshake.para.game_id ||
                  d_options.game_version != handshake.para.game_version ||
                  d_options.chapter_id != handshake.para.chapter_id
                ) {
                  console.log(
                    console_msg_rejected + "/" + "incompatible game",
                    handshake.para
                  );
                  connection.send({
                    handshake: {
                      rejected: true,
                      para: { msg: "incompatible game" },
                    },
                  });
                ...
                if (mp.is_client) {
                  console.log(
                    console_msg_rejected + "/" + "not host",
                    handshake.para
                  );
                  connection.send({
                    handshake: { rejected: true, para: { msg: "not host" } },
                  });
                  connection.close(peer);
                  return;
                }
                ...
                if (connection_count > connection_max) {
                  console.log(
                    console_msg_rejected + "/" + "host full",
                    handshake.para
                  );
                  connection.send({
                    handshake: { rejected: true, para: { msg: "host full" } },
                  });
                  connection.close(peer);
                  return;
                }
                ...
                console.log(
                  "P2P_network: Remote Peer" +
                    ...
                    "/" +
                    connection.label +
                    "/client)'s handshake request accepted from Peer-" +
                    peer.index +
                    "(" +
                    peer.id +
                    ")"
                ...
                online_data_cache.data.msg_out = (
                  online_data_cache.data.msg_out || []
                ).concat(["<p class=Msg_Default>" + msg + "</p>"]);
                ...
                console.log(
                  "P2P_network: Remote Peer" +
                    ...
                    "/" +
                    connection.label +
                    "/host) accepted handshake request from Peer-" +
                    peer.index +
                    "(" +
                    peer.id +
                    ")"
                ...
                if (
                  ...
                  delete peer.para.events.connection
                    .handshake_request_accecpted[connection.label];
                }
              } else {
                console.log(
                  "P2P_network: Remote Peer" +
                ...
                if (
                  peer.para.events.connection.handshake_request_rejected &&
                  peer.para.events.connection.handshake_request_rejected[
                    connection.label
                  ]
                ) {
                  ...
                  delete peer.para.events.connection.handshake_request_rejected[
                    connection.label
                  ];
            ...
            data: (function () {
              ...
              return function (peer, connection, data) {
                ...
                if (data.data.msg) {
                  online_data_cache.data.msg = (
                    online_data_cache.data.msg || []
                  ).concat(data.data.msg);
                }
                ...
                if (OPC_data_all) {
                  ...
                  Object.keys(OPC_data_all).forEach(function (index) {
                    ...
                    if (!OPC_data.game && cache) delete OPC_data_all[index];
                    else {
...
*/
              var mp = MMD_SA_options.Dungeon.multiplayer;
              if (!para.command) {
                if (para.id && para.pass) return null;
                ...
                if (!mp.online || mp.is_host) ChatboxAT.ChatShow([msg]);
                if (mp.online)
                  online_data_cache.data.msg_out = (
                    online_data_cache.data.msg_out || []
                  ).concat([msg]);
                ...
                switch (para.command) {
                  case "host":
                    if (mp.is_client) {
                      ChatboxAT.smallMsg(
                        "You cannot host a game in client mode."
                      );
                      break;
                    }
                    ...
                    if (time < host_command_timestamp + 30 * 1000) {
                      ChatboxAT.smallMsg(
                        "No repeated host command in " +
                          Math.round(
                            30 - (time - host_command_timestamp) / 1000
                          ) +
                          " second(s)"
                      );
                      break;
                    ...
                    if (!para.para1) {
                      ...
                      if (host_command_timerID)
                        clearInterval(host_command_timerID);
                      host_command_timerID = setInterval(function () {
                        ChatboxAT.SendData_ChatSend([
                          System._browser.P2P_network.process_message(
                            "/host auto",
                            true
                          ),
                        ]);
                    ...
                    if (path_local.indexOf(System.Gadget.path) == 0)
                      path_local = path_local
                        .substring(System.Gadget.path.length)
                        .replace(/\\/g, "/")
                        .replace(/^\/+/, "/");
                    return (
                      "/host [" +
                      ...
                      encodeURIComponent(
                        [
                          d_options.game_id + "/v" + d_options.game_version,
                          path_local,
                          peer.connections.length,
                          d_options.multiplayer.OPC_list.length,
                        ].join("|")
                      )
                    );
                  case "connect":
      ...
      return {
        ...
        get online_data_cache() {
          return online_data_cache;
        },
        ...
        init: function (init_func) {
          ...
          for (
            var i = 0, i_max = d_options.multiplayer.OPC_list.length;
            i <= i_max;
            i++
          )
            this.OPC_index[i] = i;
          ...
          this.OPC_index.forEach(function (idx) {
            var model_para =
              MMD_SA_options.model_para_obj_all[
                idx == 0 ? 0 : idx - 1 + d_options.multiplayer.OPC_index0
              ];
        ...
        connect: function (peer_id, para_connect = {}) {
          ...
          net.peer_default
            ...
            .then(function (para) {
              ...
              for (
                var i = 1, i_max = d_options.multiplayer.OPC_list.length;
                i <= i_max;
                i++
              ) {
              ...
              //console.log(list)
              ChatboxAT.smallMsg(
                "(P2P network: Host connected / Player " +
                  (handshake.para.OPC_index + 1) +
                  "/" +
                  (d_options.multiplayer.OPC_list.length + 1) +
                  ")"
              );
            ...
            .catch(function (err) {
              //  if (err && err.type)
              ChatboxAT.smallMsg(
                "(P2P network: Remote connection failed, check console for details)"
              );
        ...
        arrange_OPC: function (list) {
          ...
          var path_list = [MMD_SA_options.model_path].concat(
            MMD_SA_options.model_path_extra
          );
          var swapped_index = [];
          list.forEach(function (OPC_index, idx) {
            var c_index_OLD =
              idx == 0 ? 0 : idx - 1 + d_options.multiplayer.OPC_index0;
            if (swapped_index[c_index_OLD]) return;
            swapped_index[c_index_OLD] = true;

            var c_index_NEW =
              OPC_index == 0
                ? 0
                : OPC_index - 1 + d_options.multiplayer.OPC_index0;
            if (swapped_index[c_index_NEW]) return;
            swapped_index[c_index_NEW] = true;

            var model_para_obj_OLD =
              MMD_SA_options.model_para_obj_all[c_index_OLD];
            var model_para_obj_NEW =
              MMD_SA_options.model_para_obj_all[c_index_NEW];
            ...
            if (c_index_OLD == 0) {
              ...
              if (!MMD_SA_options.MME.PostProcessingEffects)
                MMD_SA_options.MME.PostProcessingEffects =
                  (model_para_obj_OLD.MME &&
                    model_para_obj_OLD.MME.PostProcessingEffects) ||
                  MMD_SA_options._MME.PostProcessingEffects;
          ...
          path_list.forEach(function (path) {
            ...
            var path_obj = (path_list[filename] = path_list[filename] || {
              clone_max: 0,
            });
          ...
          path_list.forEach(function (path, idx) {
            ...
            if (path_obj.clone_max) {
              if (!path_obj._index) path = path.replace(/\#clone(\d+)/, "");
              else
                path =
                  path.replace(/\#clone(\d+)/, "").replace(/\.pmx$/i, "") +
                  "#clone" +
                  path_obj._index +
                  ".pmx";
        ...
        update_online_data: (function () {
          ...
          return function () {
            ...
            if (d.started) {
              ...
              PC_data.motion.changed =
                PC_data.motion.name != mm.filename ||
                (mm.para_SA.BPM &&
                  Math.abs(PC_data.motion.time - model.skin.time) > 1); // || ((model_para._playbackRate_OPC_ || 1) != PC_data.motion.playbackRate)
...
*/
        ...
        process_remote_online_data: function (online_data) {
          ...
          if (!online_data) online_data = online_data_cache;
          ...
          if (online_data.data.msg) {
            ...
            if (this.is_host)
              online_data_cache.data.msg_out = (
                online_data_cache.data.msg_out || []
              ).concat(online_data_cache.data.msg);
          ...
          this.OPC_index.forEach(function (OPC_index, idx) {
            ...
            // no update
            if (!OPC_data.game) return;

            // game not started yet
            if (!_obj || !d.started) return;

            // not in the same chapter/area
            if (
              OPC_data.game.chapter_id != d.chapter_id ||
              OPC_data.game.area_id != d.area_id
            ) {
              _obj._obj_proxy.hidden = true;
              _obj._obj_proxy.visible = false;
              return;
            ...
            if (reset_visible) _obj._obj_proxy.visible = true;
            ...
            if (OPC_data.motion) {
              ...
              var model_para =
                MMD_SA_options.model_para_obj_all[character_index];
              if (
                OPC_data.motion
                  .changed /* || (npc_motion.filename != OPC_data.motion.name)*/
              ) {
                var npc_motion_para =
                  MMD_SA.motion[npc_model.skin._motion_index].para_SA;
            ...
            if (OPC_data._sent) OPC_data_all[idx] = {};
          });
        },
      };
    })(),
      ...
      return {
        set: function (url) {
          System._browser.load_file(url, function (xhr) {
            cache[url.replace(/^.+[\/\\]/, "")] = URL.createObjectURL(
              xhr.response
            );
          });
        },
    ...
    generate_seed: function (str_list) {
      ...
      if (!str_list) {
        str_list = [
          d_options.game_id,
          d_options.game_version,
          d_options.chapter_id,
          this.area_id,
        ];
      }

      var _this =
        str_list.join("|") + "|" + this.seed_base + "|" + Math.random();
    ...
    utils: {
      create_combat_character: function (object_index, para) {
        ...
        var combat = {
          ...
          action_check: (function () {
            ...
            return function (attacker) {
              var action_obj = {};
              var seed_max =
                d._combat_seed_ != null
                  ? d._combat_seed_
                  : Math.max(combat.combat_seed, 2);
              var seed = random(seed_max);

              var dis = attacker._obj.position.distanceTo(d.character.pos);

              if (0 && seed_max > 2 && seed == seed_max - 1 && dis < 48) {
                action_obj.motion_id =
                  combat_action[random(combat_action.length)]; //"PC combat attack 12"//
          ...
          parry_check: function (def, atk_para) {
            ...
            if (!def._parry_mod_) def._parry_mod_ = {};
            ...
            return Math.floor(
              Math.random() *
                ((d._parry_level_ != null
                  ? d._parry_level_
                  : combat.parry_level) || 1) +
                p.lvl
            );
          },
        };
        ...
        if (typeof object_index == "number") obj.object_index = object_index;
        else obj.object_id = object_index;
      ...
      create_combat_event: (function () {
        ...
        var center_rot_y = [0, 45, -45, 90, -90, 135, -135, 180].map(function (
          ry
        ) {
          return (ry / 180) * Math.PI;
        });
        window.addEventListener("MMDStarted", function () {
          ray = new THREE.Ray();
          box3 = new THREE.Box3();
        ...
        return function (x, y, para) {
          if (!para.grid_id_excluded) para.grid_id_excluded = [];
          ...
          function condition(x, y) {
            return para.zone_condition
              ? para.zone_condition(x, y)
              : d.grid_array_free[y][x] &&
                  para.grid_id_excluded.indexOf(d.grid_array[y][x]) == -1;
          }
          ...
          var min_y;
          for (
            var i = y, i_min = Math.max(0, y - (para.dis_max || 3));
            i >= i_min;
            i--
          ) {
            if (condition(x, i)) min_y = i;
            else break;
          }
          ...
          for (
            var i = y,
              i_max = Math.min(d_options.height - 1, y + (para.dis_max || 3));
            i <= i_max;
            i++
          ) {
            if (condition(x, i)) max_y = i;
            else break;
          }

          if (max_y - min_y < dis_min) return null;

          var min_x;
          for (
            var i = x, i_min = Math.max(0, x - (para.dis_max || 3));
            i >= i_min;
            i--
          ) {
            if (condition(i, y)) min_x = i;
            else break;
          }
          ...
          for (
            var i = x,
              i_max = Math.min(d_options.width - 1, x + (para.dis_max || 3));
            i <= i_max;
            i++
          ) {
            if (condition(i, y)) max_x = i;
            else break;
          }

          if (max_x - min_x < dis_min) return null;
          ...
          for (var y0 = y; y0 >= min_y; y0--) {
            ...
            for (var x0 = x; x0 >= min_x; x0--) {
              if (condition(x0, y0)) min_x0 = x0;
              else break;
            }

            var max_x0;
            for (var x0 = x; x0 <= max_x; x0++) {
              if (condition(x0, y0)) max_x0 = x0;
              else break;
            }

            if (max_x0 - min_x0 < dis_min) continue;
            ...
            for (var x1 = min_x0; x1 <= max_x0; x1++) {
              for (var y1 = y0; y1 <= max_y; y1++) {
                if (condition(x1, y1)) max_y1[x1] = y1;
                else break;
              }
            }
            ...
            for (var x1 = min_x0; x1 <= x; x1++) {
              if (max_y1[x1] < y) continue;

              for (var x2 = max_x0; x2 >= x; x2--) {
                var length_x = x2 - x1;
                if (length_x < dis_min) continue;

                var max_y0 = Math.min(max_y1[x1], max_y1[x2]);
                if (max_y0 < y) continue;

                var length_y = max_y0 - y0;
                if (length_y < dis_min) continue;

                var area = (length_x + 1) * (length_y + 1);
                if (area < box.area) continue;

                var center_factor =
                  (x2 - x) * (x - x1) * (max_y0 - y) * (y - y0);
          ...
          if (box.area < (para.area || 3)) return null;

          box3.set(
            v3a.set(box.min.x, -999, box.min.y),
            v3b.set(box.max.x + 1, 999, box.max.y + 1)
          );
          ...
          if (para.rotation !== false) {
            ...
            var is_best_ry = center_rot_y.some(function (ry) {
              ...
              ray.set(
                v3a.copy(origin_v3).add(v3b.copy(dir_v3).multiplyScalar(9999)),
                v3c.copy(dir_v3).negate()
              );

              var intersection = ray.intersectBox(box3, v3a);
              box.encounter_center = origin_v3
                .clone()
                .add(intersection)
                .multiplyScalar(0.5);

              if (box.area == 1) return true;
              ...
              ry_list.push({
                dis: dis,
                rot_y: rot_y,
                encounter_center: box.encounter_center,
              });
            });
          ...
          box.encounter_center =
            (box.encounter_center &&
              box.encounter_center.multiplyScalar(grid_size)) ||
            center_v3.clone();

          var zone_of_movement = box3
            .clone()
            .translate(v3a.copy(center_v3).negate());
            ...
            range: [
              {
                ...
                onexit: {
                  event_id: event_id + "_onexit0",
                  condition: function () {
                    return !d.character.combat_mode;
                  },
                },
              },
            ],
          ...
          para.enemy_list.forEach(function (enemy, idx) {
            ...
            if (pos_offset) pos_offset = v3a.copy(pos_offset);
            else {
              if (idx % 2 == 0) {
            ...
            if (rot_y) pos_offset.applyEuler(v3b.set(0, rot_y, 0));
            ...
            var obj_id =
              enemy.index != null
                ? "object" + enemy.index + "_0"
                : d.object_id_translated[enemy.id];
            objs_enter[obj_id] = obj;

            objs_exit[obj_id] = { placement: { hidden: true } };
          });

          if (!para.events) para.events = {};
          var e = para.events;
          ...
          if (e.onenemyalldefeated) {
            onenemyalldefeated = function (onenemyalldefeated_default) {
              var events = e.onenemyalldefeated
                .slice()
                .concat([{ func: onenemyalldefeated_default, ended: true }]);
              d.run_event([events]);
              return true;
            };
          }
          ...
          d.events[event_id + "_onenter0"] = [
            ...
            (function () {
              var events =
                (e.onbeforecombatstart && e.onbeforecombatstart.slice()) || [];
              events.push({ goto_branch: 2 });
              return events;
            })(),
            ...
            (function () {
              var events =
                (e.onaftercombatstart && e.onaftercombatstart.slice()) || [];
              events.push({ set_event_flag: { branch_index: 0 }, ended: true });
              return events;
            })(),
          ];
      ...
      grid_array_by_object: function (RDG_options, para) {
        ...
        var scale =
          para.scale || (obj_base.placement && obj_base.placement.scale) || 10;
        ...
        if (!area_options.grid_size) {
          ...
          grid_size = area_options.grid_size =
            grid_size <= 64 ? 64 : Math.pow(2, Math.ceil(Math.log2(grid_size)));
        }
      ...
      adjust_boundingBox: function (geo, model_para = {}) {
        if (!model_para.is_object && !model_para.use_default_boundingBox) {
          ...
          const _xz =
            (_v3.x + (_v3.z > _v3.x ? _v3.x : _v3.z)) *
            0.5 *
            0.5 *
            (MMD_SA_options.Dungeon._bb_xz_factor_ || 1);
      ...
      tooltip: (() => {
        ...
        return function (x, y, content) {
          ...
          if (content) SB_tooltip.textContent = content;
          ...
          const w = 280 + 5 + 5;
          SB_tooltip.style.left =
            (x > MMD_SA.THREEX.SL.width / 2
              ? x / window.devicePixelRatio - 40 - w * (1 + (scale - 1) / 2)
              : x / window.devicePixelRatio + 40 + (w * (scale - 1)) / 2) +
            "px";
...
if (
  self._js_min_mode_ ||
  (browser_native_mode && !webkit_window && !localhost_mode)
) {
  console.log("dungeon.core.min.js");
  document.write(
    '<script language="JavaScript" src="js/dungeon.core.min.js"></scr' + "ipt>"
  );
} else {
  document.write(
    '<script language="JavaScript" src="js/mersenne-twister.js"></scr' + "ipt>"
  );
  document.write(
    '<script language="JavaScript" src="js/dungeon-generator.js"></scr' + "ipt>"
  );
  document.write(
    '<script language="JavaScript" src="js/terrain.js"></scr' + "ipt>"
  );

  document.write(
    '<script language="JavaScript" src="js/rbush.min.js"></scr' + "ipt>"
  );
  //document.write('<script language="JavaScript" src="node_modules/box-intersect.js"></scr'+'ipt>');

  document.write(
    '<script language="JavaScript" src="js/nipplejs.js"></scr' + "ipt>"
  );
}
