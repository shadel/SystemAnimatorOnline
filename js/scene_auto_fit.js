...
function auto_fit_core() {
  function transform_contact_point(v, depth_v, af_motion_para) {
    rotation_offset = af.rotation
      ? v5.copy(af.rotation).multiplyScalar(Math.PI / 180)
      : v5.set(0, 0, 0);
    ...
    if (af_motion_para) {
      ...
      if (af_motion_para.rotation) {
        rotation_offset.add(
          v4.copy(af_motion_para.rotation).multiplyScalar(Math.PI / 180)
        );
        depth_v.applyEuler(v4);
      }
      if (af_motion_para.depth_rotation) {
        depth_v.applyEuler(
          v4.copy(af_motion_para.depth_rotation).multiplyScalar(Math.PI / 180)
        );
      }
    ...
    if (!af.transform_avatar) ref_pt_core.applyEuler(rotation_offset);
    ref_pt_shift
      .copy(object_3d._mesh.position)
      .sub(model_position0)
      .add(depth_v);
  ...
  function get_ground_y() {
    return af.ground_y
      ? af.ground_y * object_3d._mesh.scale.y + object_3d._mesh.position.y
      : 0;
  }
  ...
  if (af.range) {
    v1.copy(af.reference_point)
      .multiply(object_3d._mesh.scale)
      .add(object_3d._mesh.position);
    if (v1.distanceTo(model.mesh.position) > af.range) return false;
  }
  ...
  if (type == "chair") {
    ...
    const hip = v1
      ...
      .add(
        model
          .get_bone_position_by_MMD_name("右足", true)
          .sub(legL)
          .multiplyScalar(0.5)
      );
    ...
    if (!af_motion_para) {
      ...
      const leg_angle = Math.min(
        Math.abs(
          q1
            .copy(model.get_bone_by_MMD_name("左足").quaternion)
            .toAxisAngle()[1]
        ),
        Math.abs(
          q1
            .copy(model.get_bone_by_MMD_name("右足").quaternion)
            .toAxisAngle()[1]
        )
      );
    ...
    const upper_leg_length = v3
      .fromArray(legL0)
      .distanceTo(v4.fromArray(model.get_bone_origin_by_MMD_name("左ひざ")));
    const depth_v = v3.set(0, 0, -upper_leg_length / 2);

    const ankle_y = Math.min(
      model.get_bone_position_by_MMD_name("左ひざ", true).y,
      model.get_bone_position_by_MMD_name("右ひざ", true).y
    );
    ...
    if (!af_motion_para) {
      ...
      if (
        !!motion_para.motion_tracking.arm_tracking.elbow_lock.left ^
        !!motion_para.motion_tracking.arm_tracking.elbow_lock.right
      )
        return false;
    }
    ...
    if (ref_bones.length > 1) pos.x = 0;

    const is_arm = ["手", "ひじ"].some((b) => ref_bones[0].indexOf(b) != -1);

    const ref_depth_bones = is_arm ? ["左ひじ", "左手首"] : ["左ひざ", "左足"];
    const ref_depth = v3
      .fromArray(model.get_bone_origin_by_MMD_name(ref_depth_bones[0]))
      .distanceTo(
        v4.fromArray(model.get_bone_origin_by_MMD_name(ref_depth_bones[1]))
      );
  ...
  position_offset
    .add(model_position_offset)
    .sub(v4.copy(ref_pt_core).multiplyScalar(scale_offset))
    .sub(v4.copy(ref_pt_shift));
  if (position_offset_y_ignored) position_offset.setY(0);

  if (af.position) position_offset.add(af.position);
  ...
  const transform_list = af.global_transform
    ? MMD_SA.THREEX._object3d_list_
    : [object_3d];
  transform_list.forEach((obj) => {
    ...
    if (obj.placement.scale_offset)
      obj._mesh.scale.multiplyScalar(obj.placement.scale_offset);

    if (!obj.user_data._auto_fit_) obj.user_data._auto_fit_ = {};
    obj.user_data._auto_fit_.position_offset = (
      obj.user_data._auto_fit_.position_offset || new THREE.Vector3()
    ).add(_position_offset);
    obj.user_data._auto_fit_.rotation_offset = (
      obj.user_data._auto_fit_.rotation_offset || new THREE.Vector3()
    ).add(rotation_offset);
    obj.user_data._auto_fit_.scale_offset =
      (obj.user_data._auto_fit_.scale_offset || 1) * scale_offset;

    if (af.global_transform) obj.user_data._auto_fit_.global_transform = true;
  });

  if (af.height_offset_by_bone) {
    height_offset_by_bone.push({
      bone: af.height_offset_by_bone,
      scale: object_3d._mesh.scale.y * scale_offset,
    });
  }
  ...
  if (af.center_view || af.center_view_lookAt) {
    if (af.center_view) {
      af._center_view_ = [
        ...
        motion_para.center_view,
      ];
      motion_para.center_view = af.center_view;
    }
    if (af.center_view_lookAt) {
      af._center_view_lookAt_ = [
        MMD_SA.MMD.motionManager.filename,
        motion_para.center_view_lookAt,
      ];
...
function auto_fit_loop(obj) {
  ...
  object_3d = MMD_SA.THREEX._object3d_list_.find(
    (obj) => obj.uuid == obj_para._object3d_uuid
  );
  if (!object_3d) return;
  ...
  if (ev?.hide_object) {
    ...
    ev.hide_object.forEach((id) => {
      const _obj = para.json.XR_Animator_scene.object3D_list.find(
        (obj) => obj.id == id
      );
      if (!_obj) return;

      const obj_3d = MMD_SA.THREEX._object3d_list_.find(
        (obj) => obj.uuid == _obj._object3d_uuid
      );
      if (!obj_3d) return;
...
function auto_fit(list) {
  ...
  model_position0 =
    MMD_SA_options.Dungeon_options.options_by_area_id[
      MMD_SA_options.Dungeon.area_id
    ]._startup_position_;
  ...
  if (!list) {
    para.json.XR_Animator_scene.object3D_list.forEach((obj_para) => {
      ...
      if (af._hidden_ != null) {
        MMD_SA.THREEX._object3d_list_.find(
          (obj) => obj.uuid == obj_para._object3d_uuid
        )._obj_proxy.hidden = !af._hidden_;
        delete af._hidden_;
      }
      if (af._center_view_) {
        MMD_SA_options.motion_para[af._center_view_[0]].center_view =
          af._center_view_[1];
        delete af._center_view_;
      }
      if (af._center_view_lookAt_) {
        MMD_SA_options.motion_para[
          af._center_view_lookAt_[0]
        ].center_view_lookAt = af._center_view_lookAt_[1];
      ...
      transform_list.forEach((obj) => {
        ...
        obj._mesh.quaternion.multiply(
          q1.setFromEuler(_af_.rotation_offset).conjugate()
        );
    ...
    if (para.json.XR_Animator_scene.auto_fit_list) {
      para.json.XR_Animator_scene.auto_fit_list.some((af_list) => {
        let _fitted;
        af_list.some((af) => {
          const obj = para.json.XR_Animator_scene.object3D_list.find(
            (obj) => obj.id == af.object_id
          );
    ...
    for (const morph_name in _blendshape) {
      ...
      if (_m_idx != null) {
        ...
        MMD_SA._custom_morph.push({
          key: {
            weight: _blendshape[morph_name],
            morph_type: _m.type,
            morph_index: _m_idx,
          },
          idx: model_MMD.morph.target_index_by_name[morph_name],
        });
      }
    }
  }
...
function process_gesture() {
  function transform_property(para, obj, p) {
    ...
    if (typeof obj[p] == "number") {
      if (v_default == null) v_default = obj["_" + p + "_"] = obj[p];
    }
    ...
    for (const d of ["左", "右"]) {
      ...
      for (const name in gesture) {
        ...
        if (name_d) {
          ...
          for (let i = 0; i <= 3; i++) {
            ...
            if (g_event[name_ext]) gestures.push(name_ext);
          }
          break;
        }
      }
      gestures = [
        ...gestures.filter((name) => name.indexOf("OTHERS") == -1),
        ...gestures.filter((name) => name.indexOf("OTHERS") != -1),
      ];
      ...
      for (const key in key_pressed) {
        ...
        for (const type of ["Ctrl", "Alt", "Shift", "raw"]) {
          ...
          if (
            pressed[type] &&
            EV_sync_update.count_frame == pressed[type] + 1
          ) {
            timestamp_match = true;
            if (type != "raw") commands.push(type);
          }
        }
        ...
        if (timestamp_match) {
          ...
          if (g_event[k_name]) {
            if (
              typeof g_event[k_name] == "string"
                ? g_event[k_name].indexOf(dir) != -1
                : d == "右"
            )
            ...
            for (let i = 0; i <= 3; i++) {
              ...
              if (g_event[name_ext]) {
                if (
                  typeof g_event[name_ext] == "string"
                    ? g_event[name_ext].indexOf(dir) != -1
                    : d == "右"
                )
      ...
      for (let i = 0; i < gestures.length; i++) {
        ...
        const gesture_name_raw = name_raw
          .replace(/^.+\|/, "")
          .replace(/\#\d$/, "");
        ...
        let condition_passed =
          !condition ||
          (!para.json.XR_Animator_scene._gesture_initialized_ && g.auto_start);

        condition_passed =
          condition_passed ||
          condition_list.some((condition) => {
            if (condition.duration) {
              //System._browser.camera.DEBUG_show(Object.keys(gesture).join('\n')+'/'+Date.now())
              if (condition.duration > (ge?.search_para?.duration || 0))
                return false;
              //console.log(ge)
            }
            ...
            if (condition.hand_facing) {
              //console.log(ge)
              if (
                condition.hand_facing !=
                (ge.search_para?.duration
                  ? ge.search_para.hand_facing
                  : ge.hand_facing)
              )
                return false;
            }
            ...
            if (condition.user_data) {
              ...
              for (const name in condition.user_data) {
                ...
                if (
                  v_obj?.timeout &&
                  v_obj._timestamp + v_obj.timeout < RAF_timestamp
                ) {
            ...
            if (condition.hand_hidden) {
              ...
              if (
                System._browser.camera.poseNet.frames.get_blend_default_motion(
                  "skin",
                  d + "腕ＩＫ"
                ) < 1
              )
            ...
            if (condition.object_target) {
              const object3d = para.json.XR_Animator_scene.object3D_list.find(
                (obj) => obj.id == condition.object_target
              );
              if (object3d)
                x_object = MMD_SA.THREEX._object3d_list_.find(
                  (obj) => obj.uuid == object3d._object3d_uuid
                );
            }
            ...
            if (
              ...
              (condition.distance_limit ||
                condition.angle_factor ||
                condition.contact_target)
            ) {
              ...
              const hand_ext = v1
                .set((d == "左" ? 1 : -1) * 0.5, 0, 0)
                .applyQuaternion(
                  MMD_SA_options.model_para_obj.rot_arm_adjust[d + "ひじ"]
                    .axis_rot
                )
                .applyQuaternion(
                  model.get_bone_rotation_by_MMD_name(d + "手首")
                );
            ...
            if (condition.distance_limit) {
              ...
              if (hand_pos.distanceTo(obj_pos) > condition.distance_limit)
                return false;
            }
            if (condition.angle_factor) {
              ...
              if (
                hand_pos
                  .sub(arm_pos)
                  .normalize()
                  .dot(obj_pos.sub(arm_pos).normalize()) <
                condition.angle_factor
              )
                return false;
            }
            ...
            if (condition.contact_target) {
              ...
              if (type == "sphere") {
                ...
                switch (condition.contact_target.name) {
                  case "chest":
                    pt = model
                      .get_bone_position_by_MMD_name("首")
                      .add(model.get_bone_position_by_MMD_name("上半身2"))
                      .multiplyScalar(0.5);
                    br = "上半身2";
                    _pos_offset.y += 0.25;
                    break;
                  case "left_chest":
                    pt = model
                      .get_bone_position_by_MMD_name("左腕")
                      .add(model.get_bone_position_by_MMD_name("上半身2"))
                      .multiplyScalar(0.5);
                    br = "上半身2";
                    break;
                  case "right_chest":
                    pt = model
                      .get_bone_position_by_MMD_name("右腕")
                      .add(model.get_bone_position_by_MMD_name("上半身2"))
                      .multiplyScalar(0.5);
                    br = "上半身2";
                    break;
                ...
                if (condition.contact_target.position)
                  pos_offset.copy(condition.contact_target.position);
                pos_offset.add(_pos_offset);
                if (br)
                  pt.add(
                    pos_offset.applyQuaternion(
                      typeof br == "string"
                        ? model.get_bone_rotation_by_MMD_name(br)
                        : br
                    )
                  );

                let z_weight = System._browser.camera.poseNet
                  ._upper_body_only_mode
                  ? 0.5
                  : 1 / 3;
                ...
                System._browser.camera.DEBUG_show(
                  condition.contact_target.name + ":" + dis
                );
        ...
        if (g.action.attach) {
          const passed = Object.keys(g.action.attach).every((object_id, i) => {
            const object3d = para.json.XR_Animator_scene.object3D_list.find(
              (obj) => obj.id == object_id
            );
            if (!object3d) return i != 0;

            const x_object = MMD_SA.THREEX._object3d_list_.find(
              (obj) => obj.uuid == object3d._object3d_uuid
            );

            let p_bone;
            if (x_object.parent_bone_list) {
              const parent_bone_index =
                g.action.attach[object_id].parent_bone_index || 0;
              p_bone = x_object.parent_bone_list[parent_bone_index];
            } else {
              p_bone = x_object.parent_bone;
            }

            const hand_free = MMD_SA.THREEX._object3d_list_.every(
              (obj) =>
                !obj.parent_bone ||
                obj.parent_bone.disabled ||
                (x_object.parent_bone_list
                  ? x_object.parent_bone_list.indexOf(obj.parent_bone) != -1
                  : obj.parent_bone == p_bone) ||
                obj.parent_bone.name != d + p_bone.name.substring(1)
            );
            ...
            if (
              attached_side &&
              (p_bone.name.indexOf("左") != -1 ||
                p_bone.name.indexOf("右") != -1) &&
              p_bone.name.charAt(0) != attached_side
            ) {
              p_bone.name = attached_side + p_bone.name.substring(1);
              p_bone.position.x *= -1;
              const q = MMD_SA.TEMP_q.setFromEuler(
                MMD_SA.TEMP_v3.copy(p_bone.rotation)
                  .multiplyScalar(Math.PI / 180)
                  .multiply(v1.set(-1, 1, -1)),
                "YXZ"
              );
              q.x *= -1;
              q.w *= -1;
              const rot = MMD_SA.TEMP_v3.setEulerFromQuaternion(q, "YXZ")
          ...
          Object.keys(g.action.detach).forEach((object_id, i) => {
            ...
            if (x_object.placement.hidden) {
              ...
              System._browser.on_animation_update.add(
                () => {
                  if (
                    x_object.placement.hidden &&
                    x_object.parent_bone?.disabled
                  )
        ...
        if (g.action.place) {
          Object.keys(g.action.place).forEach((object_id, i) => {
            ...
            if (!x_object.placement.position)
              x_object.placement.position = { x: 0, y: 0, z: 0 };
            if (!x_object.placement.rotation)
              x_object.placement.rotation = { x: 0, y: 0, z: 0 };
        ...
        if (g.action.transform) {
          if (g.action.transform.object3D) {
            for (const id in g.action.transform.object3D) {
              ...
              if (obj.scale) {
                const scale_p =
                  x_object.placement.scale_offset == null
                    ? "scale"
                    : "scale_offset";
                transform_property(obj.scale, x_object.placement, scale_p);
                const s =
                  x_object.placement.scale *
                  (x_object.placement.scale_offset || 1);
                x_object._mesh.scale.set(s, s, s);
              }
              if (obj.position) {
                const pos_host = x_object.parent_bone?.attached
                  ? x_object.parent_bone
                  : x_object.placement;
          ...
          if (g.action.transform.motion_tracking) {
            for (const p in g.action.transform.motion_tracking) {
              transform_property(
                g.action.transform.motion_tracking[p],
                MMD_SA.MMD.motionManager.para_SA.motion_tracking,
                p
              );
            }
          }
          if (g.action.transform.gesture) {
            for (const p in g.action.transform.gesture) {
              transform_property(
                g.action.transform.gesture[p],
                para.json.XR_Animator_scene.on.gesture,
                p
              );
            }
          }
        ...
        if (g_parent.action.cooldown) {
          g_parent.action._cooldown_timestamp =
            RAF_timestamp + g?.action.cooldown;
          //System._browser.camera.DEBUG_show('cooldown:'+g_parent.action._cooldown_timestamp, 2)
        }

        if (g.action.motion_tracking) {
          if (
            g.action.motion_tracking.arm_tracking &&
            g.action.motion_tracking.arm_tracking.use_IK == null
          )
            g.action.motion_tracking.arm_tracking.use_IK = true;

          let motion_tracking =
            MMD_SA.MMD.motionManager.para_SA.motion_tracking;
          if (!motion_tracking)
            motion_tracking = MMD_SA.MMD.motionManager.para_SA.motion_tracking =
              {};
...
function restore_explorer_mode(e) {
  ...
  if (/^(Key[WASD]|Space)$/.test(ev.code)) {
    MMD_SA_options._motion_shuffle_list_default = [
      MMD_SA_options.motion_index_by_name["tsuna_standby"],
    ];
    ...
    System._browser.on_animation_update.add(
      () => {
        ...
        MMD_SA_options.Dungeon.para_by_grid_id[2].ground_y =
          para.json.XR_Animator_scene.settings?.explorer_mode?.ground_y || 0;
      },
      0,
      0
    );
...
function adjust_hip_y_offset() {
  THREE.MMD.getModels()[0].mesh.bones_by_name["センター"].position.y +=
    hip_y_offset;
}
...
function adjust_height_offset_by_bone() {
  ...
  height_offset_by_bone.forEach((config) => {
    for (const b in config.bone) {
      window.addEventListener(
        "SA_camera_poseNet_process_bones_onstart",
        () => {
          if (f._skin[b]) {
...
const gesture_plugin = (function () {
  ...
  function init() {
    ...
    finger1_horizontal.addDirection(
      fp.Finger.Index,
    ...
    for (let finger of [fp.Finger.Index, fp.Finger.Middle]) {
      ...
      finger2_horizontal.addDirection(
        finger,
        fp.FingerDirection.HorizontalLeft,
        1.0
      );
    ...
    GE = new fp.GestureEstimator(
      ge_list.map((ge) => mc.gestures.custom[ge[0]])
    );
    /*
...
*/
  ...
  const ge_list = [
    ...
    [
      "index_up",
      [
        [{ hand_facing: "sideway" }, ["finger1_up"]],
        [null, ["finger1_up"]],
      ],
    ],
    ["finger2_up"],
...
function load(p) {
  function onmotionchange(e) {
    ...
    height_offset_by_bone.forEach((config) => {
      for (const b in config.bone) {
        if (f._skin[b]) f._skin[b]._offset_y_ = null;
      }
    });
    ...
    const filename_new = e.detail.motion_new.para_SA._path
      .replace(/^.+[\/\\]/, "")
      .replace(/\.\w{3,4}$/, "");
    //DEBUG_show(filename_new)
    const motion_target_matched =
      motion_target_list.indexOf(filename_new) != -1;

    const _motion_blending =
      (!explorer_mode_locked &&
        MMD_SA_options.Dungeon_options.character_movement_disabled) ||
      motion_target_matched
        ? e.detail.motion_old.para_SA.motion_blending
        : null;
...
*/
    ...
    if (motion_tracking) {
      if (motion_tracking._default_) {
        ...
        if (!motion_tracking_new)
          motion_tracking_new = e.detail.motion_new.para_SA.motion_tracking =
            {};
  ...
  para.json.XR_Animator_scene.auto_fit_list?.forEach((list) =>
    list.forEach((af) => {
      if (af.motion_target) motion_target_list.push(...af.motion_target);
    })
  );
